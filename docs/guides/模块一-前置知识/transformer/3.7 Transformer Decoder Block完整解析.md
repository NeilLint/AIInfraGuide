...
**1. 能解释 Decoder-only 架构为什么在大模型时代胜出，至少说出三个理由**

<details>
<summary>参考答案</summary>

- **统一的训练范式**：训练目标就是"预测下一个 token"，任何语言任何任务信号统一，是利用数万亿无标注 token 最自然高效的方式；Encoder-Decoder 需构造"输入-输出"对，数据更复杂。
- **Scaling 更简单**：只有一种 Block 不断堆叠，扩大模型只需加层数或隐藏维度；Scaling Laws 表明其参数效率与 Encoder-Decoder 基本相当，但工程复杂度低得多。
- **推理效率优势**：只需维护一套 KV Cache，而 Encoder-Decoder 要维护 Encoder 输出和 Decoder KV Cache 两套数据，长上下文对话场景下简单的 KV Cache 管理意味着更高吞吐。
- **涌现能力的经验观察**：参数量超过阈值后，Decoder-only 在 few-shot/zero-shot 场景（如 chain-of-thought）表现出显著涌现能力。GPT-3、LLaMA、Mistral、Qwen、DeepSeek 全部采用此路线。

（对应正文第 1 节）

</details>

**2. 能在白板上画出完整的 Decoder Block 数据流图，标注 RMSNorm、Masked MHA、SwiGLU FFN、残差连接的位置和顺序**

<details>
<summary>参考答案</summary>

- Pre-Norm 结构，两个子层各带一条残差：第一子层 `h = x + MaskedMHA(RMSNorm(x))`，第二子层 `out = h + SwiGLUFFN(RMSNorm(h))`。
- Masked MHA 内部：$W_Q/W_K/W_V$ 投影 → 切分多头 → **RoPE 只旋转 Q 和 K**（V 承载内容信息，不需要位置匹配信号）→ 缩放点积 + 因果掩码 → softmax 加权求和 → $W_O$ 输出投影。
- SwiGLU FFN 内部：$W_{gate}$ 输出经 SiLU 激活作为门控，与 $W_{up}$ 输出逐元素相乘，再由 $W_{down}$ 降维。
- 画图关键：两条残差路径都从 Add 节点直接拉出、中间不经任何变换（残差路径"干净"），这是深层模型训练稳定的关键；归一化用 RMSNorm 而非 LayerNorm。

（对应正文第 2、4 节）

</details>

**3. 能画出因果掩码矩阵（给定序列长度 $N$），解释上三角 $-\infty$ 的作用**

<details>
<summary>参考答案</summary>

- 掩码是 $N \times N$ 矩阵：主对角线及以下为 0（保留），严格上三角（$j > i$）为 $-\infty$（屏蔽），代码上即 `torch.triu(full(-inf), diagonal=1)`，加到缩放后的注意力分数上。
- 作用机制：softmax 时 $e^{-\infty} = 0$，被屏蔽位置的注意力权重变为零，因此第 $i$ 个 token 只能看到位置 $0$ 到 $i$。
- 为什么需要：训练时整个序列并行送入模型，不加限制每个 token 都会"看到"全序列，预测下一个 token 就成了"开卷考试"；因果掩码在并行计算的同时强制执行"只能看过去"的约束。
- 高性能实现（如 FlashAttention）不显式构造 $N \times N$ 矩阵，而是在 tiled 计算中通过索引判断跳过上三角区域，省显存也省计算。

（对应正文第 3 节）

</details>

**4. 能手算 LLaMA-2-7B 和 13B 的参数量（误差不超过 5%），说清 FFN 和 Attention 的参数比例**

<details>
<summary>参考答案</summary>

- 单 Block 公式：$P_{block} = 4d^2 + 3 \times d \times d_{ff} + 2d$（MHA + SwiGLU + 两个 RMSNorm）；整模型再加 Embedding（$V \times d$）、最终 RMSNorm（$d$）和 LM Head（$V \times d$）。
- **7B**（$d=4096, d_{ff}=11008, L=32, V=32000$）：Attention $4 \times 4096^2 = 67.1M$，FFN $3 \times 4096 \times 11008 = 135.3M$，RMSNorm 8192，单 Block 约 202M；总计 $131M + 32 \times 202M + 4096 + 131M \approx 6.74B$。
- **13B**（$d=5120, d_{ff}=13824, L=40$）：Attention 104.9M + FFN 212.3M + Norm 10240，单 Block 约 317M；总计约 13.0B，与官方标注吻合。
- 参数比例：FFN 占单 Block 约 66.8%（约 2/3），Attention 约 33.2%（约 1/3），FFN 参数量约为 Attention 的 2 倍。

（对应正文第 6 节）

</details>

**5. 能用 "$6Ns$" 公式估算训练 FLOPs，用 "$2Ns$" 估算推理 FLOPs**

<details>
<summary>参考答案</summary>

- 前向传播每 token 约 $2N$ FLOPs（每个参数参与一次矩阵乘法，贡献一次乘法一次加法），处理 $s$ 个 token 即 $2Ns$；反向传播约为前向的 2 倍，故训练 $\approx 6Ns$，整个训练过程 $FLOPs_{total} = 6 \times N \times T$（$T$ 为训练 token 数）。
- 例：LLaMA-2-7B 用 2 万亿 token 训练，$6 \times 6.7\text{B} \times 2\text{T} = 8.04 \times 10^{22}$ FLOPs；1000 张 A100（312 TFLOPS，MFU=50%）约需 6 天，与公开数据量级相符。
- 注意 $2Ns$ 只算了线性层，忽略了 Attention 的 $4 \times L \times h \times s^2 \times d_k$ 项——短序列时可忽略，$s$ 很长（如 128K）时 Attention 计算量可能接近甚至超过线性层。
- 推理只有前向（每 token 约 $2N$），但 Decode 阶段每步只处理 1 个 token，GEMM 退化为 GEMV，瓶颈是显存带宽而非算力：7B 模型理论计算只需 0.043ms，实际 10–20ms，因为光搬运 13.4 GB 权重（HBM 2 TB/s）就要 6.7ms。

（对应正文第 7 节）

</details>

**6. 能计算给定模型在 FP16、INT8、INT4 下的权重显存**

<details>
<summary>参考答案</summary>

- 公式：显存 = 参数量 × 每参数字节数。FP32 为 4 Bytes，FP16/BF16 为 2 Bytes，INT8 为 1 Byte，INT4 为 0.5 Bytes。
- 7B 模型：FP16 约 13.4 GB，INT8 约 6.7 GB，INT4 约 3.35 GB；13B 模型：26 / 13 / 6.5 GB；70B 模型：140 / 70 / 35 GB。
- FP16 与 BF16 显存占用相同（都是 16 位）；BF16 指数位更多（8 位 vs 5 位）、数值范围更大，是训练主流。
- 注意：即使权重量化到 INT4，KV Cache 通常仍用 FP16（量化 KV Cache 对精度影响较大，需要 KIVI、KVQuant 等专门方案）。

（对应正文第 8.1、8.3 节）

</details>

**7. 能算出 Adam 优化器需要多少显存，并解释为什么需要 FP32 Master Weights**

<details>
<summary>参考答案</summary>

- Adam/AdamW 的优化器状态包含三样，各 $4N$ Bytes（FP32）：FP32 参数副本（Master Weights）、一阶动量 $m$（梯度的指数移动平均）、二阶动量 $v$（梯度平方的指数移动平均，实现自适应学习率），合计 $12N$ Bytes——是 BF16 权重（$2N$）的 6 倍。
- 需要 FP32 Master Weights 的原因：BF16 只有约 3–4 位有效数字，当学习率 × 梯度的增量很小（如 1e-5）时，BF16 精度不足以表示，更新会被"四舍五入"掉；FP32 有约 7 位有效数字，能捕捉这些微小更新。
- 训练态合计（7B 模型）：BF16 权重 13.4 + BF16 梯度 13.4 + 优化器状态 80.4 = 静态 107.2 GB，再加激活值约 25.8 GB（batch=4, seq=2048），共约 133 GB——超过单张 A100-80GB，所以 7B 训练也必须上分布式（ZeRO、张量并行等）。

（对应正文第 8.2 节）

</details>

**8. 给定一个模型配置和 GPU 型号，能完成完整的显存规划（权重 + KV Cache 或 权重 + 梯度 + 优化器 + Activation），判断是否能装下**

<details>
<summary>参考答案</summary>

- 推理态：显存 = 权重 + KV Cache + 框架开销（约 1–2 GB）。KV Cache 公式：$M_{kv} = 2 \times L \times h_{kv} \times d_k \times s \times b \times \text{bytes\_per\_element}$。
- 训练态：显存 = 权重 + 梯度 + 优化器状态 + Activation，其中 Activation $\approx s \times b \times d \times L \times k$（$k$ 约 10–14），Activation Checkpointing 可降到约 1/3–1/5，代价是约 33% 额外计算。
- 案例：A100-80GB 部署 LLaMA-2-13B（FP16，batch=8，seq=4096）——权重 26 GB + KV Cache 25.6 GB（单 token 800 KB）+ 开销 2 GB ≈ 53.6 GB < 80 GB，可以装下；batch 提到 16 时 KV Cache 翻倍至 51.2 GB，总计 79.2 GB，几乎无余量大概率 OOM。
- 装不下时的选项：INT8 量化权重（13B → 13 GB，总计 66.2 GB 可行）、换 GQA 模型减少 KV Cache、KV Cache 量化、减小最大序列长度——显存规划的价值就是精确算出每项开销、找到瓶颈、选对优化策略。
