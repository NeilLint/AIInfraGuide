...
**1. 能画出 7 个集合通信原语的"输入 → 输出"语义图**

<details>
<summary>参考答案</summary>

- **Broadcast**：根 rank 持有一份完整数据，操作后所有 rank 都拿到同一份拷贝。
- **Scatter**：根 rank 把大张量切成 $N$ 份，第 $i$ 份发给 rank $i$；**Gather** 是它的逆操作，把各卡数据收回根 rank 拼成完整张量。
- **Reduce**：所有卡的数据按元素规约（如求和），结果只放到根 rank，其余卡不变。
- **AllReduce**：规约后结果广播回每一个 rank，操作后所有卡内容完全一致。
- **ReduceScatter**：先按元素规约，再让第 $i$ 张卡只保留结果的第 $i$ 个分片。
- **AllGather**：每卡持一个分片，操作后每卡都拿到拼接后的完整数据（只拼接、不规约）。
- **All-to-All**：每个 rank 给其他每个 rank 发一份各不相同的数据，效果等价于数据矩阵的一次转置。

（对应正文第 3 节）

</details>

**2. 能说出 DDP、ZeRO、TP、MoE 各自主要依赖哪个通信原语**

<details>
<summary>参考答案</summary>

DDP 靠 **AllReduce** 同步梯度；ZeRO-2/3 与 FSDP 反向用 **ReduceScatter** 聚合并分片梯度、前向用 **AllGather** 重组参数分片；TP 用 **AllReduce** 合并切分矩阵乘的部分和；MoE 专家并行用 **All-to-All** 把 token 按路由结果发往各卡上的专家（Ulysses 序列并行也用它）。此外 Broadcast 用于初始化时同步参数，send/recv 用于流水线并行传激活值。（对应正文第 1、3 节的全景对照表）

</details>

**3. 能解释点对点阻塞收发为什么会死锁，并写出两种避免方法**

<details>
<summary>参考答案</summary>

如果两个进程都用阻塞式 `send` 先发后收，且底层缓冲区不足以暂存数据，双方就会都卡在 `send` 上等对方来 `recv`，互相等待形成死锁。两种避免方法：

- **错开顺序**：一方先发后收，另一方先收后发（按 rank 分支）。
- **用异步原语 `isend`/`irecv`**：先发起全部收发请求，再统一 `wait()`。

实际框架里更推荐 `dist.batch_isend_irecv()` 一次性提交一批收发请求，由后端统一调度，既避免死锁又能重叠通信——PP 的实现大量依赖它。（对应正文第 2 节）

</details>

**4. 能推导 Ring AllReduce 每卡通信量为 $2\frac{N-1}{N}\Psi \approx 2\Psi$ 且与卡数无关**

<details>
<summary>参考答案</summary>

把每卡大小为 $\Psi$ 的数据切成 $N$ 个分片，每片 $\frac{\Psi}{N}$。Reduce-Scatter 和 All-Gather 两个阶段各走 $N-1$ 步，每步每卡发送一个分片，故每卡发送总量为 $2(N-1)\cdot\frac{\Psi}{N} = 2\frac{N-1}{N}\Psi = 2\Psi(1 - \frac{1}{N})$，接收量与发送量相同。当 $N$ 较大时该值趋近 $2\Psi$，**与卡数 $N$ 无关**——无论 8 卡还是 1024 卡，每卡通信量都稳定在约 $2\Psi$，这正是 Ring AllReduce 能支撑大规模数据并行的根本原因（对比朴素中心化实现：中心节点通信量随 $N$ 线性增长）。（对应正文第 4.3 节）

</details>

**5. 能默写恒等式 $\text{AllReduce} = \text{ReduceScatter} + \text{AllGather}$ 并解释它在 ZeRO 中的意义**

<details>
<summary>参考答案</summary>

恒等式：$\text{AllReduce} = \text{ReduceScatter} + \text{AllGather}$。直观理解：先 ReduceScatter 让每卡拿到"自己那份的全局求和结果"，再 AllGather 把各卡分片拼回完整结果，合起来就完成了 AllReduce。它既是 Ring AllReduce 的实现蓝图（两个阶段正好对应两个半步），也是 ZeRO 能"用两个半步通信代替一次 AllReduce"的根本依据——ZeRO 反向只做 ReduceScatter（每卡只留自己负责的梯度分片），需要完整参数时才做 AllGather，每个半步的通信量各为 $\Psi$，通信账可直接与 DDP 的一次 AllReduce（$2\Psi$）对照。（对应正文第 3.8、4.2 节）

</details>

**6. 能用 $T \approx \alpha + \text{数据量}/B$ 估算一次通信耗时，并说明何时该用 Ring、何时该用 Tree**

<details>
<summary>参考答案</summary>

$\alpha$ 是每次通信的固定延迟（建立连接、同步），与数据量无关；$\text{数据量}/B$ 是带宽项。小数据量时延迟项主导，适合用步数 $O(\log N)$、延迟低的 **Tree** 算法（小张量、控制信号）；大数据量时带宽项主导，适合带宽最优的 **Ring**（如梯度同步等大块数据），代价是它有 $2(N-1)$ 步、延迟随 $N$ 线性增长。估算时务必代入正确带宽：机内 NVLink 数百 GB/s、机间 IB 约 50 GB/s，同一公式会差十倍——这定量解释了"为什么 TP 必须放机内"。实际中 NCCL 会根据数据量、卡数、拓扑自动选择算法。（对应正文第 4.4、5.3 节）

</details>

**7. 能解释为什么 FSDP 每步通信量是 $3\Psi$ 而 DDP 是 $2\Psi$**

<details>
<summary>参考答案</summary>

DDP 每步只在反向做一次梯度 AllReduce，每卡通信量约 $2\Psi$。FSDP（ZeRO-3）因为参数分片，前向要 AllGather 拼出完整参数（$\Psi$），反向要再 AllGather 一次（$\Psi$）加 ReduceScatter 同步梯度（$\Psi$），合计 $3\Psi$，比 DDP 多 50\%——这正是它省显存付出的通信代价。（对应正文第 5.2 节）
