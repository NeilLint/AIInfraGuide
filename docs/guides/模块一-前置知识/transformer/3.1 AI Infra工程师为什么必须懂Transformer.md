...
**1. 能用自己的语言解释什么是 AI Infra，它在 AI 技术栈中处于什么位置，上下游分别是什么**

<details>
<summary>参考答案</summary>

一句话定义：AI Infra 就是**用系统工程的方法，把硬件算力高效地"喂"给模型**——如果算法研究员是菜单的设计者，AI Infra 工程师就是搭建厨房、调度厨师、管理供应链的后勤总指挥。

在技术栈中，AI Infra 正好夹在"算法与模型层"（上游）和"硬件层"（下游）之间，扮演两层之间的**翻译官**：向上理解模型长什么样、每个模块需要多少计算和显存；向下理解 GPU 的存储层次、通信拓扑和并行能力，把上层模型的计算需求翻译成下层硬件能高效执行的指令。而这个"上层模型"，在当今几乎就等于 Transformer。（详见第 1.1–1.2 节）

</details>

**2. 能说清 RNN/LSTM 的两个核心弱点（串行、遗忘），以及 Transformer 如何用 Self-Attention 解决它们**
...
- **无法并行训练**：每一步计算依赖上一步输出，2048 个 token 的序列必须串行执行 2048 步，无法利用 GPU 数千核心的并行能力——像单车道公路，每次只能过一辆车
- **长距离依赖的遗忘问题**：信息在长距离传递时逐步衰减，处理数千 token 以上的长序列时效果明显下降

Self-Attention 的解法：让每个 token 直接和所有其他 token 计算注意力权重并聚合信息，第 1 个和第 2000 个 token 之间的信息传递一步到位。由此带来**天然的并行性**（注意力权重本质是一次矩阵乘法 $QK^T$，GPU 最擅长）和**全局信息访问**（不存在长距离衰减）。代价是 $O(N^2)$ 的复杂度，而这恰好成为 AI Infra 优化的核心战场。（详见第 2.1–2.2 节）

</details>

**3. 能列出至少 3 项 CUDA 算子优化技术及其对应的 Transformer 模块，并解释对应关系**

<details>
<summary>参考答案</summary>

- **FlashAttention → Self-Attention 的 $QK^T$ / $PV$ 矩阵乘法**：标准实现要把 $N \times N$ 注意力矩阵写入 HBM（显存 $O(N^2)$），FlashAttention 用 tiling 分块，让每个小块在片上 SRAM 中完成 $QK^T$、Softmax、$PV$ 的全部计算
- **Fused Softmax / Online Softmax → Attention 的 Softmax 归一化**：标准实现需三遍扫描（求 max、求指数和、归一化），Online Softmax 合并前两遍，与 tiling 结合后大幅减少中间结果的显存读写
- **LayerNorm kernel 融合 → 每个 Decoder Block 的归一化层**：LayerNorm 计算量小但需两遍扫描，是 Memory Bound 操作；把 Residual Add + LayerNorm 融合为一个 kernel 可省一次完整的 HBM 读写
- **RoPE 融合到 Attention kernel → 位置编码**：旋转变换计算量小但调用频繁，融合可避免单独 launch kernel 的开销（它作用于 QK 投影之后、$QK^T$ 之前）

（详见第 3.1 节）

</details>

**4. 能列出至少 3 项分布式训练技术及其对应的 Transformer 模块，并解释对应关系**

<details>
<summary>参考答案</summary>

- **张量并行（TP）→ Attention 多头 + FFN 矩阵**：32 个 Attention 头可均匀分到 4 张 GPU 各处理 8 个头，到输出投影前不需卡间通信；FFN 的升维矩阵 $W_{up}$ 按列切、降维矩阵 $W_{down}$ 按行切，中间激活值不需跨卡通信
- **流水线并行（PP）→ Decoder Block 的层级堆叠**：将几十层 Block 分段放到不同 GPU（如前 8 层在 GPU 0）。可行的根本原因是每个 Block 的输入输出维度完全相同（都是 $(N, d_{model})$），天然支持分段
- **ZeRO 显存优化 → 所有参数矩阵**：标准数据并行中每卡都存完整的参数、梯度、优化器状态，ZeRO 将它们按切片分摊到各卡；理解参数分布（FFN 约 2/3、Attention 约 1/3）对评估其显存节省和通信开销至关重要

（详见第 3.2 节）

</details>

**5. 能列出至少 3 项推理优化技术及其对应的 Transformer 模块，并解释对应关系**

<details>
<summary>参考答案</summary>

- **KV Cache → Self-Attention 的 K/V 矩阵**：自回归生成每步只输出一个 token，若不缓存则每步都要重算所有历史 token 的 K、V；KV Cache 把每层每步的 K、V 存显存复用
- **PagedAttention → KV Cache 的显存碎片**：不同请求序列长度不同，预分配会造成碎片；借鉴操作系统虚拟内存分页，把 KV Cache 切成固定大小的"页"按需分配
- **量化 → 所有权重矩阵和 KV Cache**：如 FP16 压到 INT4，显存直接降为 1/4；作用于 $W_Q/W_K/W_V/W_O/W_{gate}/W_{up}/W_{down}$ 及 KV Cache
- **Speculative Decoding → 自回归生成的串行瓶颈**：用小而快的 Draft 模型先"猜"多个 token，大模型一次性并行验证，用并行验证加速串行生成

（详见第 3.3 节）

</details>

**6. 给定一个 LLM 推理请求，能追踪数据流经 Transformer 各模块的过程，并指出每个环节对应哪些 AI Infra 优化**

<details>
<summary>参考答案</summary>

以 LLaMA-2-7B + vLLM 为例，完整旅程：

1. **Tokenize + Embedding**（CPU 分词 → $(10, 4096)$ 张量）：高并发下 CPU 端 tokenize 可能成瓶颈，用 Nsight Systems 查 GPU idle gap
2. **Prefill 阶段**（10 个 token 并行过 32 层）：RMSNorm（与 Residual Add 融合）→ QKV 投影（TP 切分点）→ RoPE（融合进 Attention kernel）→ Self-Attention（FlashAttention 全融合，Compute Bound）→ 缓存 K/V（PagedAttention 分页写入）→ FFN（参数大头，TP 切分与量化的主要对象）；耗时决定 TTFT
3. **Decode 阶段**（每步 1 个新 token）：矩阵乘退化为矩阵-向量乘，Memory Bound；对应优化最密集——Speculative Decoding（串行瓶颈）、FlashDecoding（KV 搬运）、PagedAttention（KV 增长）、KV Cache 量化（显存不够）、Continuous Batching（动态组批）、Prefill/Decode 解耦（尾延迟）

每个环节都是某项 AI Infra 优化的"着力点"。（详见第 4 节）

</details>

**7. 能回答"为什么 AI Infra 工程师必须懂 Transformer"这个问题，给出至少两个具体理由（而不是空泛的"因为很重要"）**

<details>
<summary>参考答案</summary>

- **理由一：不懂结构就无法定位和评估优化**。每项 AI Infra 技术都作用于 Transformer 的某个具体模块——FlashAttention 优化的是 Self-Attention 的 $QK^T$/Softmax 的计算与显存访问模式，张量并行沿多头和 FFN 矩阵切分，KV Cache 管理的是 Attention 中 K/V 的生命周期。不理解这些模块，就无法定位性能瓶颈出在哪、也看不懂 profiler 里的 kernel 在做什么
- **理由二：优化对象几年来几乎没变**。从 BERT、GPT 到 LLaMA、Mistral、DeepSeek，无论名字、规模、场景如何变化，底层计算单元始终是 Self-Attention、FFN、LayerNorm、位置编码这几个核心模块，变的只是规模和细节。懂了 Transformer，这份理解可以复用于 CUDA 算子、分布式训练、推理部署所有层级的工作
