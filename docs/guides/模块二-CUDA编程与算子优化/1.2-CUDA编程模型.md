...
**1. 能画出 Grid/Block/Thread 的层次结构示意图**

<details>
<summary>参考答案</summary>

- 三级层次自顶向下：**Grid（学校）** = 一次 Kernel 启动产生的所有线程的集合；**Block（班级）** = 可协作的线程组，Block 内线程可以使用共享内存和同步；**Thread（学生）** = 最小执行单位。
- 画图时按 Grid ⊃ Block ⊃ Thread 的包含关系嵌套方框：Grid 里排列若干 Block，每个 Block 里排列若干 Thread。
- Grid 和 Block 都可以是 1D/2D/3D，用 `dim3` 指定，如 `dim3 grid(16, 16); dim3 block(16, 16);` 表示 256 个 Block、每个 Block 256 个线程。
- 硬件限制：Block 内线程总数 `blockDim.x * blockDim.y * blockDim.z ≤ 1024`；Grid 的 x 维最多 $2^{31}-1$ 个 Block，y/z 维最多 65535。

（对应正文第 2 节）

</details>

**2. 能根据数据维度选择合适的 Grid 和 Block 维度**

<details>
<summary>参考答案</summary>

- 原则是让线程组织贴合数据形状：一维向量用 1D（`dim3 grid(256); dim3 block(256);`）；图像/矩阵用 2D（如 `dim3 block(16, 16)` 共 256 线程，`dim3 grid((width + 15) / 16, (height + 15) / 16)`）；体数据或批处理用 3D（第三维放 batch，如 `dim3 grid(..., ..., batchSize)`，Kernel 内用 `blockIdx.z` 取 batch 编号）。
- Block 数量按数据大小向上取整 `(N + blockSize - 1) / blockSize`，保证覆盖所有元素。
- N 不是 blockSize 整数倍时最后一个 Block 会有"越界"线程，Kernel 内必须做边界检查 `if (idx < N)`（2D 则是 `if (col < width && row < height)`）。
...
- 一维公式：`globalIdx = blockIdx.x * blockDim.x + threadIdx.x`，即全局线程 ID = Block 编号 × Block 大小 + Block 内线程编号。
- 例：`gridDim=4, blockDim=8` 时，Block 2 的线程 3 的全局索引 = 2 × 8 + 3 = 19（Block 2 覆盖 16~23）。
- 二维：`col = blockIdx.x * blockDim.x + threadIdx.x; row = blockIdx.y * blockDim.y + threadIdx.y;`，再按行主序转线性索引 `linearIdx = row * width + col`。
- 四个内置变量：`threadIdx`（线程在 Block 内的索引）、`blockIdx`（Block 在 Grid 内的索引）、`blockDim`（Block 的维度）、`gridDim`（Grid 的维度），类型都是 `dim3`。

（对应正文第 3 节）

</details>

**4. 能编写 Grid-stride loop 处理超大数组**

<details>
<summary>参考答案</summary>

当数据量大于线程总数时，让每个线程以总线程数为步长循环处理多个元素：
...
这是 CUDA 编程的最佳实践：既能正确处理任意大小的数据，又能让你自由选择 Grid 大小来优化性能。（对应正文 3.4 节）

</details>

**5. 能使用 Occupancy API 自动计算最优 Block 大小**

<details>
<summary>参考答案</summary>

- 调用 `cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, myKernel, 0, 0);`，它会自动计算使占用率最大化的 Block 大小。
- 五个参数依次是：最小 Grid 大小的输出、Block 大小的输出、kernel 函数指针、动态共享内存大小、Block 大小上限（0 = 不限制）。
- 拿到 blockSize 后照常计算 `gridSize = (N + blockSize - 1) / blockSize` 并启动。
- 另有 `cudaOccupancyMaxActiveBlocksPerMultiprocessor` API 可用于计算给定配置下的占用率。

（对应正文 5.3、8.3 节）

</details>

**6. 能识别 Warp 分歧并重构代码避免分歧**

<details>
<summary>参考答案</summary>

- Warp 是物理执行单位：32 个连续线程在同一时钟周期执行同一条指令（SIMT）；当 Warp 内线程走不同分支路径时发生 **Warp 分歧**，GPU 不得不串行执行两个分支。
- 识别：分支条件依赖 `threadIdx` 且在 Warp 内部变化就危险，典型如 `if (threadIdx.x % 2 == 0)` 让奇偶线程分家。
- 重构：让分支边界对齐 Warp，如 `if (threadIdx.x / 32 < 2)`——前两个 Warp 走 pathA、后续 Warp 走 pathB。
- 关键认知：分歧只发生在 Warp 内部；只要同一 Warp 的 32 个线程走相同分支就没有惩罚，不同 Warp 之间走不同分支完全没有代价。

（对应正文第 6 节）

</details>

**7. 能为矩阵运算和图像处理配置 2D Grid/Block**

<details>
<summary>参考答案</summary>

- 用 2D Block + 2D Grid 覆盖整幅图像/矩阵：`dim3 block(16, 16);`（256 线程）加 `dim3 grid((width + 15) / 16, (height + 15) / 16);`。
- Kernel 内计算 `col = blockIdx.x * blockDim.x + threadIdx.x` 和 `row = blockIdx.y * blockDim.y + threadIdx.y`，做 `if (col < width && row < height)` 双边界检查，再按行主序 `row * width + col` 访问数据——灰度转换和 matrixAdd 都是这个模式。
- 批处理场景再加第三维：`dim3 grid(..., ..., batchSize)`，用 `blockIdx.z` 作为 batch 编号，线性索引为 `batch * height * width + row * width + col`。

（对应正文第 7 节）

</details>

**8. 能根据 `--ptxas-options=-v` 的输出分析占用率瓶颈**

<details>
<summary>参考答案</summary>

- 编译 `nvcc --ptxas-options=-v mykernel.cu` 会输出形如 `ptxas info: Used 32 registers, 4096 bytes smem, 368 bytes cmem[0]` 的资源用量。
- 对照占用率的影响因素逐项分析：每线程寄存器多 → SM 能容纳的线程少；每 Block 共享内存多 → SM 能容纳的 Block 少；Block 太小 → 活跃 Warp 少；还受不同架构的最大 Block 数/线程数硬件限制。
- 把寄存器和共享内存用量代入 CUDA Occupancy Calculator 或 `cudaOccupancyMaxActiveBlocksPerMultiprocessor` API，即可算出占用率并找到限制项。
- 占用率 = SM 上实际活跃 Warp 数 / 理论最大 Warp 数；越高不一定性能越好，但过低通常意味着 GPU 资源闲置。
