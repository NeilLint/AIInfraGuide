...
**1. 基础概念：梯度消失和梯度爆炸的根本原因是什么？为什么深度与这个问题正相关？**

<details>
<summary>参考答案</summary>

- 反向传播本质是链式法则的递归应用，梯度是一连串雅可比矩阵的**连乘**。
- 设每层雅可比矩阵的谱范数（最大奇异值）为 $\sigma$：若 $\sigma < 1$，经 $L$ 层后梯度以 $\sigma^L$ 指数衰减（梯度消失）；若 $\sigma > 1$，以 $\sigma^L$ 指数增长（梯度爆炸）；只有 $\sigma$ 恰好等于 1 才能稳定传播，但这在实际训练中几乎不可能自然满足。
- 深度 $L$ 出现在指数上，所以层数越多问题恶化越剧烈——这就是深度与问题正相关的原因。
- 此外还有退化问题：更深的网络训练误差反而更高（如 56 层比 20 层差），因为让多余的层"学会恒等映射"本身很困难——这直接催生了残差连接。

（对应正文第 1 节）

</details>

**2. 残差连接：写出残差连接的反向传播公式，解释为什么它包含一个"恒等捷径"。**

<details>
<summary>参考答案</summary>

- 残差层的计算为 $x_{l+1} = x_l + F_l(x_l)$，对输入求导得 $\frac{\partial x_{l+1}}{\partial x_l} = I + \frac{\partial F_l}{\partial x_l}$。
- 从第 $L$ 层回传到第 $l$ 层的梯度为 $\frac{\partial L}{\partial x_l} = \frac{\partial L}{\partial x_L} \cdot \prod_{k=l}^{L-1}\left(I + \frac{\partial F_k}{\partial x_k}\right)$。
- 把连乘展开后，必然包含一项所有括号都取 $I$ 的乘积，即恒为 $I$——无论各层 $\partial F_k / \partial x_k$ 多小甚至趋近零，梯度都能通过这条"恒等通道"原封不动地传到任意浅层，这就是"梯度高速公路"。
- 本质上，残差连接把反向传播中的纯"乘法链"变成了"加法 + 乘法"的混合结构，加法给梯度提供了"保底值"，大幅提升数值稳定性。

（对应正文第 2.2 节）

</details>

**3. 归一化对比：BatchNorm 和 LayerNorm 的归一化维度有什么区别？为什么 BatchNorm 不适合 Transformer？**

<details>
<summary>参考答案</summary>

- **BatchNorm** 沿 batch 维度归一化：对每个特征通道，统计 batch 内所有样本在该通道上的均值和方差。
- **LayerNorm** 沿特征维度归一化：对每个 token 的 $d$ 维特征向量独立计算均值和方差，与 batch 中其他样本无关。
- BatchNorm 不适合 NLP/Transformer 的四个原因：① 序列长度不等，padding 位置的值污染 batch 统计量；② 大模型单卡 micro batch 很小（1–4），统计量本身方差大、不可靠；③ 训练用 batch 统计、推理用 running mean/variance，训练-推理不一致；④ 自回归生成时 batch size 为 1，batch 统计量完全没有意义。
- LayerNorm 完全独立于 batch，训练/推理行为一致，逐 token 生成友好，因此成为 Transformer 的标准配置。

（对应正文第 3.1–3.2 节）

</details>

**4. RMSNorm：RMSNorm 相比 LayerNorm 省略了什么？为什么在大模型中效果不受影响？**

<details>
<summary>参考答案</summary>

- 省略了两样东西：① 减均值这一步（不做 re-centering），只保留 $y_j = \gamma_j \cdot \frac{x_j}{\text{RMS}(x)}$，其中 $\text{RMS}(x) = \sqrt{\frac{1}{d}\sum_j x_j^2}$；② 偏移参数 $\beta$（没有 re-centering 也就不需要偏移）。
- 效果不受影响的原因：$\text{RMS}(x)^2 = \text{Var}(x) + \text{mean}(x)^2$，当均值接近 0 时 RMSNorm 近似等于 LayerNorm；实验表明深层 Transformer（尤其 Pre-Norm 架构）中各层特征均值确实通常接近零。
- 更本质地，归一化的核心作用是**控制激活值的尺度**，除以 RMS 已基本达成；去均值的效果可由各层可学习参数隐式处理。
- 工程收益：LayerNorm 需两遍扫描（sum 和 sum of squares 两个 reduction），RMSNorm 只需一遍扫描一个 reduction，在 memory-bound 的 GPU 场景中少一次 HBM 读取，kernel 更简单、数值问题更少。

（对应正文第 3.3、5.1 节）

</details>

**5. Pre-Norm vs Post-Norm：画出两种架构的数据流图，解释为什么 Pre-Norm 的梯度流更稳定。**

<details>
<summary>参考答案</summary>

- **Post-Norm**（原始 Transformer）：`output = LayerNorm(x + SubLayer(x))`，LayerNorm 位于残差加法**之后**，在残差主路径上。
- **Pre-Norm**（GPT-2 起的主流）：`output = x + SubLayer(LayerNorm(x))`，LayerNorm 移到子层**之前**，残差加法后没有任何操作。
- Post-Norm 中，梯度每过一个 Block 都要被 LayerNorm 的非平凡雅可比矩阵"调制"一次，深层累乘可能引发梯度爆炸，必须用小学习率 + 精心设计的 warmup（Xiong et al. 2020 证明其各层输出方差随深度线性增长）。
- Pre-Norm 中每个因子是 "$I$ + 某项" 的形式，展开后必然含恒等项——梯度可经恒等路径直达任意浅层、不经过任何 LayerNorm，即"残差路径保持纯净"。因此 Pre-Norm 对学习率更鲁棒，大规模训练（数十亿参数以上 Post-Norm 常出现 loss spike 甚至发散）几乎全部选择 Pre-Norm。

（对应正文第 4 节）

</details>

**6. 工程实践：为什么 Residual Add 和 LayerNorm 通常融合为一个 kernel？FP16 下归一化需要注意什么？**

<details>
<summary>参考答案</summary>

- 融合的原因：分开实现时，`h = x + SubLayer_output` 要先写回 HBM，LayerNorm kernel 再把 `h` 读出来——一次写一次读完全可以省掉。融合 kernel 在寄存器/shared memory 中完成加法后立即做 reduction 和归一化，只在最后写回。这是 Apex、Megatron-LM 的标准优化，实测可带来约 5–10% 的端到端推理加速。
- FP16 的两个精度陷阱：① $\sum x^2$ 的平方操作放大数值范围，可能溢出 FP16 上限 65504；② 方差公式 $\text{mean}(x^2) - \text{mean}(x)^2$ 在方差较小时是"大数减大数"，产生灾难性抵消。
- 标准解法：reduction（求均值/方差）时提升到 FP32 累加，算完再转回半精度做归一化——"输入输出半精度、中间计算全精度"的混合精度策略。
- BF16 指数位 8 位（FP16 只有 5 位），动态范围与 FP32 一致，不易溢出，因此越来越多大模型训练选择 BF16。
