...
**1. 能独立写出 CUDA 向量加法的完整代码（含内存管理和错误检查）**

<details>
<summary>参考答案</summary>

- 五步标准流程：`cudaMalloc` 分配设备内存 → `cudaMemcpy(..., HostToDevice)` 传入 A、B → `vectorAdd<<<gridSize, blockSize>>>(d_A, d_B, d_C, N)` 启动 Kernel → `cudaMemcpy(..., DeviceToHost)` 取回 C → `cudaFree`/`free` 释放。
- Kernel 三要素：全局索引 `int idx = blockIdx.x * blockDim.x + threadIdx.x;`、边界检查 `if (idx < N)`、并行计算 `C[idx] = A[idx] + B[idx];`（A、B 用 `const float*` 表示只读输入）。
- 启动配置：`int blockSize = 256; int gridSize = (N + blockSize - 1) / blockSize;`。
- 所有 API 调用用 `CUDA_CHECK` 宏包裹；Kernel 启动后接 `CUDA_CHECK(cudaGetLastError())` 与 `CUDA_CHECK(cudaDeviceSynchronize())`。
- 最后与 CPU 基线逐元素对比（容差 1e-5f）验证正确性。

（对应正文 2、3、4、6 节）

</details>

**2. 能解释为什么 Kernel 中需要 `if (idx < N)` 边界检查**

<details>
<summary>参考答案</summary>

- Grid 大小按 `(N + blockSize - 1) / blockSize` 向上取整，当 N 不是 blockSize 的整数倍时，最后一个 Block 会有多余线程。
- 例：N = 1000、Block 大小 = 256 时需要 4 个 Block，总线程数 4 × 256 = 1024，多出的 24 个线程索引为 1000~1023，超出数组范围。
- 没有 `if (idx < N)` 检查，这些线程会访问非法内存地址，导致未定义行为或程序崩溃（对应错误码 `cudaErrorIllegalAddress`）。

（对应正文 2.2、4.3 节）

</details>

**3. 能使用 CUDA Event 精确测量 Kernel 执行时间**

<details>
<summary>参考答案</summary>

- 流程：`cudaEventCreate` 创建 start/stop 两个事件 → `cudaEventRecord(start)` → 执行要计时的操作 → `cudaEventRecord(stop)` → `cudaEventSynchronize(stop)` 等待完成 → `cudaEventElapsedTime(&milliseconds, start, stop)` 得到毫秒数 → `cudaEventDestroy` 清理。
- CUDA Event 是 GPU 端精确计时的标准方法，精度达微秒级。
- 要测端到端时间，就把两次 `cudaMemcpy` 也夹在 start/stop 之间；可以用两对 Event（start/stop 和 kernel_start/kernel_stop）在一次运行中同时测"含传输"和"仅 Kernel"两个区间。

（对应正文 5.2、5.3 节）

</details>

**4. 能计算给定 Kernel 的有效内存带宽并与理论峰值对比**

<details>
<summary>参考答案</summary>

- 有效带宽 = 实际读写的总字节数 / Kernel 时间。向量加法每个元素读 2 个 float、写 1 个 float，共 12 bytes/element，总数据量 = 3 × size。
- 代码实现：`float bandwidth_gb = (3.0f * size) / (kernel_ms * 1e-3f) / 1e9f;`。
- 例：N = $2^{24}$ 时处理 $3 \times 2^{24} \times 4$ Bytes ≈ 201 MB 数据，Kernel 时间 0.3 ms → 有效带宽约 670 GB/s。
- 对比 RTX 4090 的理论峰值带宽 1008 GB/s，利用率约 66%——对如此简单的 Kernel 来说已经不错。

（对应正文 5.4、7.2 节）

</details>

**5. 能判断一个操作是计算密集型还是内存密集型**

<details>
<summary>参考答案</summary>

- 用**算术强度**（Arithmetic Intensity = FLOPs / Bytes）判断：计算量相对内存访问量越低，越偏内存密集型。
- 向量加法每 12 Bytes 只做 1 FLOP，AI ≈ 0.083 FLOP/Byte，极低——典型的内存密集型操作，性能受限于内存带宽而非计算能力。
- 矩阵乘法这类计算密集型操作则相反，是 GPU 优势最明显的场景。
- 实践意义：内存密集型操作的优化方向是提升带宽利用率、减少数据搬运（让数据留在 GPU 上），而不是堆算力。

（对应正文 7.1、7.4 节）

</details>

**6. 能解释为什么包含数据传输的端到端时间远大于纯 Kernel 时间**

<details>
<summary>参考答案</summary>

- 因为 PCIe 传输成为瓶颈。参考数据（RTX 4090、N = $2^{24}$）：H2D 传输（64MB × 2）约 5 ms、占 ~60%，Kernel 执行仅约 0.3 ms、占 ~4%，D2H 传输（64MB）约 3 ms、占 ~36%。
- Kernel 用的是显存带宽（仅 Kernel 时有效带宽 ~400 GB/s），而传输走 PCIe，端到端有效带宽掉到 ~15 GB/s 量级。
- 结论：对简单的逐元素操作，数据传输往往是主要瓶颈；实际应用中应尽量让数据"留在 GPU 上"——多个 Kernel 串联处理同一批数据，避免反复搬运。GPU 的优势在数据已经在显存上时最明显。

（对应正文 5.4、7.3 节）

</details>

**7. 能使用 Grid-stride loop 模式处理任意大小的数据**

<details>
<summary>参考答案</summary>

每个线程以总线程数为步长循环处理多个元素：
...
好处是启动配置与数据规模解耦：不再需要 `gridSize = (N + 255) / 256`，可以选择任意合理的 Grid 大小（如 `vectorAddStride<<<128, 256>>>`），同一个 Kernel 即可正确处理任意大小的数组。（对应正文 8.3 节）

</details>

**8. 能使用 `CUDA_CHECK` 宏编写健壮的 CUDA 代码**

<details>
<summary>参考答案</summary>

- 宏定义：用 `do { ... } while (0)` 包裹——接收 CUDA 调用，把返回值存入 `cudaError_t err`，若 `err != cudaSuccess` 就用 `cudaGetErrorString(err)` 连同 `__FILE__`、`__LINE__` 打印到 stderr 并 `exit(EXIT_FAILURE)`。
- 用法：`CUDA_CHECK(cudaMalloc(&d_A, size));`、`CUDA_CHECK(cudaMemcpy(...));`。
- Kernel 启动是异步的，错误检查需要两步：`CUDA_CHECK(cudaGetLastError())` 检查启动参数是否正确（如 Block 超过 1024 线程的 `cudaErrorInvalidConfiguration`），`CUDA_CHECK(cudaDeviceSynchronize())` 检查执行过程错误（如越界的 `cudaErrorIllegalAddress`）。
- 开发阶段建议每个 CUDA API 后都加检查；发布时可用编译宏控制，在 Release 模式下去掉检查开销。
