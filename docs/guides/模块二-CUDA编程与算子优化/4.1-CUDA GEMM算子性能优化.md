...
**1. 能解释 GEMM 分块后算术强度从 0.25 提升到 32 FLOPs/Byte 的推导过程**

<details>
<summary>参考答案</summary>

- 朴素实现：每个线程计算一个 $C[i][j]$，运算量 $2K$ FLOPs，需读 $A$ 的一行 + $B$ 的一列共 $8K$ 字节，算术强度 $= 2K / 8K = 0.25$ FLOPs/Byte，远低于平衡点（V100 约 17.4），严重带宽受限。
- Block Tiling 后：每次把 $BM \times BK$ 的 A 子块和 $BK \times BN$ 的 B 子块载入 Shared Memory，被整个 Block 复用，完成 $BM \times BN \times 2 \times BK$ FLOPs、搬运 $(BM \times BK + BK \times BN) \times 4$ 字节，约去 $BK$ 得算术强度 $= \frac{BM \times BN}{2(BM + BN)}$。
- 取 $BM = BN = 128$：$128 \times 128 / (2 \times 256) = 32$ FLOPs/Byte，远超平衡点。且由均值不等式，$BM + BN$ 固定时 $BM = BN$ 使该比值最大。

（对应正文 §3.2、§4.3）

</details>

**2. 能说出朴素实现、Shared Memory 分块、线程分块各自的性能瓶颈**

<details>
<summary>参考答案</summary>

- **朴素实现**：数据复用率几乎为零——$A$ 的同一行、$B$ 的同一列被不同线程重复从 Global Memory 读取，算术强度仅 0.25 FLOPs/Byte，实测约为 cuBLAS 的 6%~11%。
- **Shared Memory 分块**：Global 访存量降下来后，每次 FMA 仍伴随对 Shared Memory 的隐式读取，256 个线程同时高频访问 SMEM 造成 MIO Throttle Stall，约 25%~40% cuBLAS。
- **线程分块（寄存器外积）**：SMEM 压力缓解后（约 50%~65% cuBLAS），剩余瓶颈转向加载指令数（需 float4 向量化）、SMEM Bank Conflict，以及未被隐藏的 Global Memory 延迟（需双缓冲/流水线）。

（对应正文 §3.2、§5.1、§12.2）

</details>

**3. 能独立实现一个包含 Shared Memory Tiling + Thread Tiling 的 SGEMM kernel**

<details>
<summary>参考答案</summary>

核心结构（典型参数 $BM=BN=128$、$BK=8$、$TM=TN=8$、Block=256）：

- 声明 `__shared__ float As[BM][BK]`、`Bs[BK][BN]`，寄存器数组 `a_frag[TM]`、`b_frag[TN]`、`c_frag[TM][TN]`；
- 外层 K-Loop 以 $BK$ 为步长：Block 内 256 个线程按合并访问的排布协作加载 tileA/tileB 到 Shared Memory，`__syncthreads()`；
- 内层对 $k = 0..BK$：每线程先把 `As[thread_row + i][k]` 的 $TM$ 个值和 `Bs[k][thread_col + j]` 的 $TN$ 个值读入寄存器，再在寄存器上做 $TM \times TN$ 次外积 FMA 累加到 `c_frag`；
- 再次 `__syncthreads()`（防止下一轮加载覆盖正在使用的数据）后进入下一 K 步；K-Loop 结束后把 `c_frag` 写回 $C$ 中该线程负责的 $TM \times TN$ 子块。

（对应正文 §4.5、§5.3）

</details>

**4. 能解释外积（Outer Product）相比内积为何能提高 Shared Memory 复用率**

<details>
<summary>参考答案</summary>

内积写法（K 在内层）每完成 1 次 FMA 就要从 SMEM 读 $A$、$B$ 各一个元素，按元素算比值为 1。外积把 K 放到最外层：每轮 K 迭代先把 $A$ 的 $TM$ 个、$B$ 的 $TN$ 个元素批量读入寄存器（共 $TM + TN$ 次 SMEM 读取），再在寄存器上完成 $TM \times TN$ 次 FMA。算术强度 $= \frac{TM \times TN}{2(TM + TN)}$，形式与 Block 分块完全一致——外积本质是把分块思想下沉到 Register 层级。$TM = TN = 8$ 时按元素算比值为 $64/16 = 4$，比内积提升 4 倍：原本 4 次 SMEM 访问才能驱动的计算，现在只需 1 次。

（对应正文 §5.2）

</details>

**5. 能画出双缓冲流水线中 LDG / FFMA / STS 的时序重叠关系**

<details>
<summary>参考答案</summary>

准备两套 SMEM buffer（`As[2][BK][BM]`、`Bs[2][BK][BN]`），主循环每轮的指令顺序：

1. 发射 **LDG**（异步加载下一 tile 的数据到寄存器暂存 `ldg_a/ldg_b`）；
2. 执行 **FFMA**（用当前 buffer 的数据做外积计算）——LDG 发射后线程继续执行，300+ cycle 的 Global Memory 延迟被这段计算隐藏；
3. 执行 **STS**（把暂存寄存器写入另一个 buffer）；
4. 一次 `__syncthreads()` 切换 buffer——双缓冲使主循环只需一次同步而非两次。

p-Loop 内还有第二级双缓冲：`a_frag`/`b_frag` 各两组交替，预取第 $k+1$ 步的同时计算第 $k$ 步；LDS 与 FFMA 分属不同执行单元，硬件可并行调度。

（对应正文 §9.2~§9.6）

</details>

**6. 能使用 Nsight Compute 识别 Stall Long Scoreboard vs MIO Throttle 的含义**

<details>
<summary>参考答案</summary>

- **Stall Long Scoreboard**：warp 停在等待 Global Memory 加载结果上，说明 Global 延迟没有被隐藏——对策是引入双缓冲/多级流水线（cp.async），让数据搬运与计算重叠。
- **Stall MIO Throttle**：Shared Memory 指令发射压力过大——对策是做寄存器级数据复用（Thread Tiling 外积）和 float4 向量化，减少 LDS 指令数量。
- 相关指标还有 Stall Short Scoreboard（Shared Memory 延迟本身）和 Stall Barrier（同步等待）。

（对应正文 §5.1、§12.3）

</details>

**7. 能解释 Shared Memory Bank Conflict 的产生条件和 3 种解决方案**

<details>
<summary>参考答案</summary>

产生条件：SMEM 分 32 个 bank（各 4 字节宽，第 $i$ 字节属于 bank $\lfloor i/4 \rfloor \bmod 32$）。同一 warp 内（float4 访存时是同一 quarter-warp / memory transaction 内）多个线程访问**同一 bank 的不同地址**时串行化为 N-way conflict；访问同一地址则触发广播不冲突；不同 warp 之间不存在 bank conflict。

解决方案：

1. **Padding**：声明 `As[BK][BM + 4]` 错开 bank 映射，缺点是破坏 float4 对齐，可能无法向量化读取；
2. **XOR Swizzle**：写入与读取用相同的异或规则（如 `row ^ (k * 4)`）打乱地址，不增加空间且保持 float4 对齐；
3. **调整计算区域/线程映射**：把每线程 $8 \times 8$ 计算区域拆为 4 个 $4 \times 4$ 子块，让 quarter-warp 的读取恰好铺满一层 bank（warp transactions 从 10 降到 6）；再配合 Z-order 线程映射（$2 \times 4$ 的 quarter-warp），让 tileA/tileB 的读取都触发广播并满足跨 QW 合并条件，进一步降到 4 个 transactions。

（对应正文 §8.1、§8.5）

</details>

**8. 能说明为什么 4×8 的 Warp 形状优于 1×32**

<details>
<summary>参考答案</summary>

设 warp 在 M、N 方向排成 $x \times y$（$xy = 32$），利用 SMEM 广播，一次 p-Loop 只需从 tileA 读 $x \cdot TM$、从 tileB 读 $y \cdot TN$ 个不同元素，却完成 $32 \times TM \times TN \times 2$ FLOPs。$TM = TN$ 时计算访存比 $= \frac{16 \times TM}{x + y}$，与 $x + y$ 成反比。由均值不等式 $x + y \geq 2\sqrt{32} \approx 11.3$，整数解中 $4 \times 8$ / $8 \times 4$（$x + y = 12$）最优；而 $1 \times 32$（$x + y = 33$）读 $A$ 时虽可广播，读 $B$ 却需要 32 份不同数据，两侧访问严重失衡，比值最差。

（对应正文 §6.1、§6.2）

</details>

**9. 能计算给定 TM/TN/BM/BN 参数下的寄存器用量和 occupancy**

<details>
<summary>参考答案</summary>

单线程寄存器 ≈ `c_frag`（$TM \times TN$ 个）+ `a_frag`/`b_frag`（$TM + TN$ 个）+ 索引和临时变量（约 30 个）。$TM = TN = 8$ 时约 $64 + 16 + 30 \approx 110$~$128$ 个。以 128 寄存器/线程、Block = 256 线程计：一个 Block 用 32768 个寄存器，V100 每 SM 65536 个寄存器可驻留 2 个 Block（512 线程 = 16 warps），occupancy = 25%。这看似很低，但 GEMM 靠 ILP（每线程 64 条独立 FMA 链）而非高 occupancy 隐藏延迟；若取 $TM = TN = 16$，仅 `c_frag` 就要 256 个寄存器，会溢出到 Local Memory 反而劣化性能。

（对应正文 §5.2、§5.4）

</details>

**10. 能解释 cp.async 相比传统 LDG+STS 的优势**

<details>
<summary>参考答案</summary>

传统路径是 Global → 寄存器（LDG）→ Shared Memory（STS），需要线程寄存器中转。SM80（Ampere）引入的 `cp.async` 支持 Global → Shared 的异步直接拷贝：

- 节省用于中转的寄存器（约 8 个/线程）；
- 硬件流水线更深，延迟隐藏效果更好；
- 配合 `cp.async.commit_group` / `cp.async.wait_group N` 可实现多级流水线（加载 stage $i+2$ 的数据、同时计算 stage $i$），CUTLASS 在 Ampere 上默认使用 3~4 级 Shared Memory buffer，3 stage 通常是 SMEM 用量与延迟隐藏的最佳平衡点。
