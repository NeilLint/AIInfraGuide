...
**1. 能解释 `is` 与 `==`、浅拷贝与深拷贝的区别**

<details>
<summary>参考答案</summary>

- `is` 比较对象身份（是否指向同一个对象），适合写 `x is None`；`==` 调用值比较逻辑，比较内容是否相等。`b = a` 只是名字绑定，不复制对象，因此 `a is b` 与 `a == b` 都为 True。
- 浅拷贝（如 `src.copy()`）只复制最外层容器，内层对象仍被共享，修改 `shallow[0]` 会影响 `src`；深拷贝（`copy.deepcopy`）递归复制内部对象，彼此独立。
- 深拷贝并非默认答案：对大型张量或模型状态做无意复制可能产生巨大的内存和时间开销，应先明确哪些数据需要独占、哪些可以只读共享。（详见第 2.1、2.2 节）

</details>

**2. 能说明默认可变参数为什么会跨调用共享**

<details>
<summary>参考答案</summary>

- 默认参数在函数定义时求值一次，而不是每次调用时求值。因此 `def collect_bad(x, result=[])` 中的列表是所有调用共享的同一个对象，多次调用会不断累积内容。
- 正确做法是用 `None` 表示“未提供”：在函数体内写 `if result is None: result = []`，让每次调用都创建新列表。（详见第 2.2 节）

</details>

**3. 能写一个保留函数元信息的装饰器和一个上下文管理器**

<details>
<summary>参考答案</summary>

- 装饰器接收一个可调用对象并返回另一个可调用对象，`@timed` 等价于 `preprocess = timed(preprocess)`。内层 `wrapper` 必须用 `@functools.wraps(fn)` 装饰，才能保留原函数的函数名、文档字符串等元信息，这对日志、测试和框架反射很重要。
- 上下文管理器可用 `@contextlib.contextmanager` 加生成器实现：`yield` 之前获取资源，`try/finally` 的 `finally` 中释放；`with` 保证即使发生异常，清理逻辑也会执行。
- 它和 C++ 的 RAII 解决的是同一个问题：资源生命周期必须有清晰边界。（详见第 2.6、2.7 节）

</details>

**4. 能根据 I/O 密集、CPU 密集选择线程、进程或异步 I/O**

<details>
<summary>参考答案</summary>

- I/O 密集（请求服务、读大量小文件）：用线程或 `asyncio`——阻塞 I/O 时解释器会释放 GIL，协程在 `await` 处主动让出控制权。
- 纯 Python CPU 密集：用多进程或原生扩展绕开 GIL；但多进程有进程创建、序列化和数据搬运成本，任务粒度要足够大，且要用 `if __name__ == "__main__":` 保护入口。
- 原生算子密集（NumPy/PyTorch/CUDA）：由底层线程池和 GPU 并行，Python 只负责调度。
- 注意在事件循环中不能直接执行阻塞 I/O 或长时间 CPU 计算，否则会卡住所有协程；应改用异步库或放到线程/进程执行器。（详见第 3.1–3.5 节）

</details>

**5. 知道 GPU 异步执行为什么会让普通 CPU 计时失真**

<details>
<summary>参考答案</summary>

- GPU Kernel 启动通常是异步的：CPU 提交任务后立即返回，CPU 计时器只量到“提交任务”的时间，而不是 Kernel 真正执行完的时间。
- 正确做法是在计时边界同步，或使用框架提供的 GPU Event；此外要先 warmup、多次重复并取中位数。
- 同步本身会改变流水线行为，因此端到端吞吐和单算子延迟应分别测量。（详见第 3.6 节）

</details>

**6. 能区分 C++ 指针、引用、值和 `const` 引用的接口语义**

<details>
<summary>参考答案</summary>

- 按值 `T`：函数获得自己的独立对象；对小型标量（`int`、`float`、指针）按值传递通常最清楚。
- 指针 `T*`：可以是 `nullptr`、可以改指向，常表示可选对象、数组或非拥有观察。
- 引用 `T&`：语义上必须绑定对象、不可重新绑定，表示函数会修改调用方对象（可变借用）。
- 常量引用 `const T&`：只读借用，避免复制且不允许修改。（详见第 4.1 节）

</details>

**7. 能说明 RAII、`unique_ptr`、`shared_ptr` 各自解决什么问题**

<details>
<summary>参考答案</summary>

- RAII 在构造时获取资源、析构时释放资源；无论函数正常返回还是抛出异常，局部对象的析构函数都会执行，因此能系统性避免手工 `new`/`delete` 带来的内存泄漏、重复释放、悬空指针和异常路径泄漏。
- `std::unique_ptr`：唯一所有权，可移动不可复制，成本接近裸指针，是默认首选。
- `std::shared_ptr`：引用计数共享所有权，有计数与控制块成本，循环引用需配合 `weak_ptr`；没有共享需求时不要用它，“避免思考所有权”不是共享所有权的合理理由。（详见第 4.4、4.5 节）

</details>

**8. 能解释预处理、编译、汇编、链接和动态装载阶段**

<details>
<summary>参考答案</summary>

- 预处理：展开 `#include`、宏和条件编译；编译：语法/类型检查并生成汇编；汇编：生成目标文件 `.o`；链接：解析符号并生成可执行文件或库；运行时装载：加载动态库并解析依赖。
- 三类错误对应不同阶段：编译错误（`not declared`、类型不匹配）、链接错误（`undefined reference`，常见原因是只声明未定义、漏链接库）、运行时装载错误（`cannot open shared object file`，动态库不在搜索路径）。
- 可用 `g++ -E/-S/-c` 逐步观察各阶段，用 `nm -C`、`ldd`、`readelf -d` 定位符号和依赖。（详见第 5.1、5.2 节）

</details>

**9. 能用 CMake 构建一个最小 C++ 库**

<details>
<summary>参考答案</summary>

- 最小项目包含 `cmake_minimum_required`、`project(... LANGUAGES CXX)`，用 `add_library(vector_ops src/vector_ops.cpp)` 定义库目标。
- 现代 CMake 的核心是 target：用 `target_include_directories`、`target_compile_features`、`target_link_libraries` 描述目标的头文件路径、语言标准和依赖，少用全局的 `include_directories` 和手工拼接编译命令。
- 构建命令：`cmake -S . -B build -DCMAKE_BUILD_TYPE=Release` 再 `cmake --build build --parallel`。（详见第 5.7 节）

</details>

**10. 能解释 shape、stride、dtype、device 和 lifetime 为什么属于跨语言接口**

<details>
<summary>参考答案</summary>

- 所谓零拷贝只是共享同一块底层内存，它要求双方对完整契约达成一致：地址 + 元素类型（dtype）+ 形状（shape）+ 步长（stride）+ 设备（device）+ 生命周期 + 可变性。
- 任何一项不匹配都可能导致隐式复制、错误结果或越界访问；例如转置后的 NumPy 数组可能不是 C 连续布局，只拿首地址按连续数组读取会得到错误数据。
- 核心原则：把数据布局和所有权当作接口的一部分，而不是实现细节。（详见第 6.4 节）

</details>

**11. 能在 Linux 上查进程、资源占用、端口和动态库依赖**

<details>
<summary>参考答案</summary>

- 进程：`ps -ef`、`pgrep -af`、`top`/`htop`。
- 资源占用：进程内存用 `pmap -x PID`，系统内存用 `free -h`，磁盘容量用 `df -h`，目录占用用 `du -sh`，文件 I/O 用 `iostat`/`iotop`。
- 端口：`ss -lntp` 查监听端口，`curl -v`、`nc -vz` 测连接。
- 动态库依赖：Linux 用 `ldd`，macOS 用 `otool -L`；`strace` 可观察程序实际在找哪个配置文件或动态库。（详见第 7.4、7.5、7.8 节）

</details>

**12. 能区分 NVIDIA Driver、CUDA Toolkit、CUDA Runtime 与 cuDNN**
...
- NVIDIA Driver：操作系统与 GPU 通信的基础，`nvidia-smi` 主要反映驱动状态。
- CUDA Toolkit：开发工具集合，包含 `nvcc`、头文件、调试/分析工具和库。
- CUDA Runtime 与数学库：应用运行时加载的用户态组件（cudart、cuBLAS 等）。
- cuDNN：面向深度学习算子的独立加速库，不等于整个 CUDA。
- `nvidia-smi` 显示的 “CUDA Version” 是驱动可支持的最高兼容级别，不证明本机装了同版本 `nvcc`；运行预编译框架也未必需要 `nvcc`。（详见第 8.3 节）

</details>

**13. 能创建小而清晰的 Git 分支和提交，并在提交前检查 diff**

<details>
<summary>参考答案</summary>

- 用 `git switch -c <branch>` 从主干新建主题分支；一个提交只包含一个清晰主题，不要把格式化整个仓库、编辑器配置和目标改动混在一起。
- 提交前同时查看 `git diff`（工作区 vs 暂存区）和 `git diff --staged`（暂存区 vs HEAD），配合 `git status --short`，避免漏提交或把无关文件带入。
- 个人尚未共享的功能分支可以 rebase；公共分支不要随意改写历史。（详见第 9.1–9.3 节）

</details>

**14. 能用“最小复现 + 环境快照 + 单变量实验”定位问题**

<details>
<summary>参考答案</summary>

- 按固定顺序记录：期望行为 → 实际行为（完整报错、退出码）→ 最小复现（最短命令、最小输入）→ 环境快照（代码提交、依赖版本、系统、硬件）→ 边界定位（数据、Python、C++、CUDA、驱动、网络中的哪一层）→ 单变量实验（每次只改变一项并记录结果）。
- 好的问题描述是“提交 abc123、batch size 8、序列长度 4096，在第 23 step 分配 1.2 GiB 时 CUDA OOM，batch size 4 不复现”，而不是“偶尔 OOM”。（详见第 9.4 节）
