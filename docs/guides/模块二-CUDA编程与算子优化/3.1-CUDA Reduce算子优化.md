...
**1. 能解释 Warp Divergence 产生的原因，以及 V1（交错寻址）和 V2（步长反转）如何分别减少/消除它**

<details>
<summary>参考答案</summary>

- GPU 以 Warp（32 线程）为调度单位，Warp 内所有线程必须执行相同指令；当 `if` 让同一 Warp 内部分线程满足条件、部分不满足时，GPU 分两次执行两条路径，实际吞吐减半，这就是 Warp Divergence。V0 的 `tid % (2*step) == 0` 使每一轮都产生分化，且随 step 增大利用率越来越差（50% → 25% → ...）。
- V1 改用 strided index：每个线程计算 `index = threadIdx.x * 2 * s`，只判断 `index < blockDim.x`，让活跃线程集中在连续的低 tid 区间——前几轮完整 Warp 整体进入或跳过分支，仅最后 5 轮（s=8~128，活跃线程不足 32 个）在 Warp 0 内部有分化。
- V2 反转步长方向（从 `blockDim.x/2` 逐轮减半）并用 `if (tid < s)` 让低编号线程始终活跃：每个 Warp 要么整体工作、要么整体空闲，彻底消除分化。

（对应正文 §2.3、§3、§4.3）

</details>

**2. 能描述 Shared Memory Bank Conflict 的概念，并说明 V2 为什么能同时消除 Bank Conflict**

<details>
<summary>参考答案</summary>

- Shared Memory 被划分为 32 个 Bank（每个宽 4 字节），相邻 4 字节地址依次落在 Bank 0~31 循环排布。同一 Warp 内多个线程访问**同一 Bank 的不同地址**时，访问被串行化，即 Bank Conflict；多个线程访问同一地址则由硬件广播，不产生冲突。
- V1 存在严重冲突：s=1 时 tid 0 访问 smem[0]、tid 16 访问 smem[32]，两者同落 Bank 0 → 2 路冲突；s=2 时升级为 4 路、s=4 时 8 路，逐轮恶化。
- V2 中活跃线程访问 smem[tid] 与 smem[tid+s]：如 step=128 时 Warp 0 的 32 个线程分别访问 smem[0..31]（及 smem[128..159]），恰好覆盖 32 个不同 Bank → 无冲突；step 更小时剩余线程的访问也完全落在不同 Bank 上。

（对应正文 §4.1、§4.2、§4.3）

</details>

**3. 能解释 V3 中"每线程处理 2 个元素"相比"每线程 1 个元素"的性能收益来源**

<details>
<summary>参考答案</summary>

V2 中每线程只加载 1 个元素，规约第一轮（step = blockDim.x/2）后一半线程彻底闲置，它们的唯一贡献就是把数据搬进 Shared Memory。V3 让每线程在加载阶段读取 2 个相距 `blockDim.x` 的元素并预先求和：同样 256 个线程可处理 512 个数据，Block 数从 524,288 减半到 262,144，GPU 调度压力降低；加载阶段每线程多做 1 次"有用"的加法，而规约轮数保持 8 轮不变；全局内存访问模式也更规整（连续两段数据由同一 Block 处理）。带宽利用率由约 40% 提升到约 45%。

（对应正文 §5）

</details>

**4. 能解释 V4 为什么在最后一个 Warp 中可以省去 `__syncthreads()`**

<details>
<summary>参考答案</summary>

当规约进行到 step ≤ 32 时，只剩 1 个 Warp（tid 0~31）在工作，而同一 Warp 内的线程执行相同指令路径时本身就是同步的，不需要额外屏障。V4 将规约循环只执行到 s > 32，把后面的轮次展开为 `warpReduce` 直线代码，省去 5 次多余的 `__syncthreads()` 调用。注意 `warpReduce` 中的 `smem` 必须声明为 `volatile`，防止编译器把中间结果缓存到寄存器、导致其他线程读到旧值。带宽利用率由约 45% 提升到约 52%。

（对应正文 §6）

</details>

**5. 能解释为什么使用模板参数（`template <int BLOCK_SIZE>`）能提升性能**

<details>
<summary>参考答案</summary>

运行时循环的每轮迭代都带有边界判断（`s > 32`）、分支跳转和循环变量更新（`s >>= 1`）的开销。把 Block 大小作为模板参数后，编译器在编译期就确定了所有循环的迭代次数：`if (BLOCK_SIZE >= 512)` 这类条件在编译期求值，不满足的分支整块删除、不生成任何指令，最终机器码是一段没有循环跳转、没有死代码的紧凑直线代码。调用侧需按实际 block_size 用 switch-case 选择对应的模板实例化。带宽利用率由约 52% 提升到约 62%。

（对应正文 §7）

</details>

**6. 能写出使用 `__shfl_down_sync` 实现 Warp 内规约的代码**
...
要点：`0xffffffff` 表示 Warp 内全部 32 个线程参与；每轮每个线程读取 `lane_id + offset` 处线程的寄存器值并累加；offset 从 16 减半到 1，5 轮后 lane 0 持有 Warp 内总和。整个过程在寄存器间完成，无地址计算、无 Bank Conflict、延迟更低。

（对应正文 §8.1、§8.3）

</details>

**7. 能解释两级规约（Warp 内 + Warp 间）的完整流程**

<details>
<summary>参考答案</summary>

Warp Shuffle 只能在单个 Warp（32 线程）内交换数据，256/512 线程的 Block 需要两级规约：

1. **Warp 内规约**：每个 Warp 用 `__shfl_down_sync` 把 32 个值规约为 1 个（结果在各 Warp 的 lane 0）；
2. **写入 Shared Memory**：各 Warp 的 lane 0 把局部结果写入 `warp_results[wid]`（最多 32 项），随后 `__syncthreads()`；
3. **Warp 间规约**：由 Warp 0 读取这些中间值（`lane < num_warps` 时取值，否则补 0），再做一次 Warp 内规约；
4. tid 0 把该 Block 的结果写回全局内存。

（对应正文 §8.2、§8.3）

</details>

**8. 能说明 `float4` 向量化加载相比逐元素加载的优势**

<details>
<summary>参考答案</summary>

GPU 内存系统以事务（通常 128 字节）为粒度传输，而每条加载指令的调度开销是固定的。`float4` 让每个线程一次加载 16 字节（4 个 float）：每条 `ld.global.v4.f32` 指令的数据吞吐是 `ld.global.f32` 的 4 倍，相同数据量下循环迭代次数等效减少 4 倍，并提升指令级并行（ILP），让访存流水线更饱和。若 n 不是 4 的倍数，需对尾部元素做逐元素兜底处理。

（对应正文 §9.1、§9.3）

</details>

**9. 能描述 Grid Stride Loop 的工作方式及其适用场景**

<details>
<summary>参考答案</summary>

工作方式：固定 Grid 大小（如 SM 数 × 4，A100 上为 432），每个线程从 `blockIdx.x * blockDim.x + tid` 开始，以 `gridDim.x * blockDim.x` 为步长循环处理多段数据，直到覆盖整个数组。适用场景与好处：Grid 大小可设置为恰好填满 GPU，避免尾部 Block 浪费；对超出最大 Grid 限制的超大数组同样适用；每个线程处理更多数据，充分摊销 Kernel 启动和规约的固定开销。

（对应正文 §9.2、§9.3）

</details>

**10. 能使用 Nsight Compute 的 Memory Throughput 指标验证各版本的带宽利用率**

<details>
<summary>参考答案</summary>

带宽利用率 = Kernel 实测内存吞吐 ÷ 理论峰值带宽（A100 为 2039 GB/s）。用 Nsight Compute 读取各版本的 Memory Throughput，对照本文的实测阶梯：V0 ~300 GB/s（15%）→ V1 ~653（32%）→ V2 ~816（40%）→ V3 ~918（45%）→ V4 ~1060（52%）→ V5 ~1265（62%）→ V6 ~1468（72%）→ V7 ~1733 GB/s（85%）。受 ECC、时钟波动等影响，85~90% 已是 Reduce 的合理目标上限；生产环境可再对比 CUB 的 `cub::DeviceReduce`（通常 90% 以上）。
