...
**1. 能列出 CUDA 五种主要内存类型及其容量、延迟、作用域**

<details>
<summary>参考答案</summary>

- **寄存器**：SM 芯片上，约 256KB/SM，1 cycle，单线程私有，生命周期同线程——最快的存储。
- **共享内存**：SM 芯片上，48~228KB/SM（因架构而异），约 20 cycles，Block 内共享，生命周期同 Block。
- **全局内存（HBM）**：芯片外，16~80GB，约 400 cycles，带宽 1~3.4 TB/s，全局作用域，应用生命周期。
- **常量内存**：芯片外 + 专用缓存，64KB，命中缓存时极快（1~400 cycles），全局只读。
- **统一内存**：编程抽象而非独立硬件——单一地址空间，CPU/GPU 用同一指针访问，系统自动做页面迁移。
- 中间还有自动管理的 L1（约 30 cycles）/ L2（约 200 cycles）缓存；寄存器用超会溢出到局部内存。

（对应正文 2.2、8 节）

</details>

**2. 能解释合并访问的条件，并判断给定访问模式是否合并**

<details>
<summary>参考答案</summary>

- 条件：**一个 Warp 内 32 个线程访问的地址连续且对齐**。GPU 以 32B/64B/128B 的粒度做内存事务，地址连续时一次事务就能满足所有线程。
- 量化影响（Warp 访问 32 个 float）：完美合并只需 1 次 128B 事务（带宽利用率 100%）；连续但未对齐要 2 次 128B 事务（50%）；随机散列最多 32 次 32B 事务（约 3%）。
- 判断方法：看相邻线程访问地址的间隔——`data[threadIdx.x + blockIdx.x * blockDim.x]` 相邻线程访问相邻地址，是合并访问；乘上 stride 的跨步访问（Thread 0 → data[0]、Thread 1 → data[stride]）不合并。

（对应正文 3.2、3.3 节）

</details>

**3. 能将一个 AoS 数据结构改造为 SoA 以实现合并访问**

<details>
<summary>参考答案</summary>

- AoS（Array of Structures）把每个对象的所有字段放在一起：`struct Particle { float x, y, z, vx, vy, vz; }` 的数组中，Thread i 访问 `particles[i].x` 时地址间隔为 `sizeof(Particle)` = 24 bytes，相邻线程访问不连续，无法合并。
- SoA（Structure of Arrays）按字段组织：`struct Particles { float x[N], y[N], z[N], vx[N], vy[N], vz[N]; }`，Thread i 访问 `p.x[i]` 时间隔仅 `sizeof(float)` = 4 bytes，相邻线程访问连续地址，完美合并。
- 改造方法：把"对象数组"拆成"每个字段一个数组"。设计数据结构时优先选 SoA，因为它更容易实现合并访问。

（对应正文 3.4 节）

</details>

**4. 能正确使用 `__shared__` 声明共享内存并配合 `__syncthreads()`**

<details>
<summary>参考答案</summary>

- 静态声明（编译时确定大小）：Kernel 内写 `__shared__ float tile[256];`。
- 动态声明（运行时确定大小）：`extern __shared__ float sharedData[];`，并在启动时用第三个参数传入字节数——`dynamicSharedMem<<<grid, block, sharedMemSize>>>(data, N);`。
- 标准使用模式：先把全局内存数据加载到共享内存（`tile[threadIdx.x] = input[idx];`），然后调用 `__syncthreads()` 确保 Block 内**所有线程**都完成加载，之后才能安全读取其他线程写入的数据。
- 共享内存位于 SM 芯片上、Block 内所有线程共享，像一个团队的白板——成员都能快速读写，其他团队（Block）看不到。

（对应正文 4.1 节）

</details>

**5. 能识别 Bank Conflict 的成因并使用 padding 解决**

<details>
<summary>参考答案</summary>

- 成因：共享内存被划分为 32 个 Bank（对应 32 个 Warp 线程），每个 Bank 宽 4 bytes，`Bank 编号 = (地址 / 4 bytes) % 32`；当一个 Warp 中多个线程同时访问**同一个 Bank 的不同地址**时，访问必须串行化。
- 判断：`shared[threadIdx.x]` 无冲突（线程 i → Bank i）；`shared[threadIdx.x * 2]` 是 2-way conflict（线程 0 和线程 16 都落在 Bank 0）；所有线程读同一地址 `shared[0]` 是广播，无冲突。
- 解决：矩阵转置等列访问场景用 padding——把 `__shared__ float tile[32][32];` 改成 `__shared__ float tile[32][32 + 1];`，多加一列错开 Bank 对齐。

（对应正文 4.2、4.3 节）

</details>

**6. 能用 Tiling 模式优化矩阵乘法 Kernel**

<details>
<summary>参考答案</summary>

- 核心思想是"搬一次数据，用多次"：把全局内存数据分块加载到共享内存，在共享内存上做多次计算，用少量全局内存访问换取大量快速的共享内存访问。
- 实现：声明 `__shared__ float As[TILE][TILE]; __shared__ float Bs[TILE][TILE];`，沿 K 维分块迭代——每轮 Block 内线程协作把 A、B 的一个 Tile 加载进共享内存（越界补 0），`__syncthreads()` 后在共享内存上做该块乘加并累积到寄存器 `sum`，再次 `__syncthreads()` 后滑动到下一个 Tile；循环结束写回 `C[row * N + col]`。
- 两次 `__syncthreads()` 缺一不可：第一次保证 Tile 加载完毕才开始计算，第二次防止下一轮加载覆盖还在使用的数据。
- 这一范式在 FlashAttention、GEMM 优化等高级场景反复出现，是 GPU 编程最重要的优化范式之一。

（对应正文 9.2 节）

</details>

**7. 能根据场景选择正确的内存类型（常量/共享/全局/统一）**

<details>
<summary>参考答案</summary>

- 按决策树走：数据只读且所有线程读同一值、≤64KB → **常量内存**；只读但各线程读不同地址 → **全局内存 + `__ldg()`**（走只读缓存路径）；读写且需要 Block 内共享/交换 → **共享内存 + `__syncthreads()`**（同 Warp 内可用 Warp Shuffle）；读写但无需共享 → **全局内存**（保证合并访问）。
- 局部累加器/计数器 → 寄存器；链表、树等复杂指针结构或快速原型、CPU/GPU 交替访问 → **统一内存**。
- 注意两点：Warp 内不同线程访问常量内存不同地址会被串行化为 32 次读取，比全局内存还慢；统一内存编程简便但性能次优，性能敏感的生产代码用显式 `cudaMemcpy` 精确控制传输时机。
- 要减少全局内存事务 → Tiling + 共享内存。

（对应正文第 9 节）

</details>

**8. 能使用 `cudaMemPrefetchAsync` 优化统一内存的性能**

<details>
<summary>参考答案</summary>

- 背景：统一内存靠**按需页面迁移**工作——GPU Kernel 首次访问某页时触发缺页中断，驱动才把页面从系统内存迁移到显存，这个自动迁移有延迟开销。
- 优化：在 Kernel 启动前显式预取到 GPU——`cudaGetDevice(&device); cudaMemPrefetchAsync(data, N * sizeof(float), device);`，让迁移在计算开始前完成。
- Kernel 完成后再把结果预取回 CPU：`cudaMemPrefetchAsync(data, N * sizeof(float), cudaCpuDeviceId);`。
- 这样避免了 Kernel 执行过程中被缺页中断反复打断，是统一内存场景下最直接的性能优化手段。
