...
description: "梳理优化器从 SGD 到 AdamW 的演进，算清优化器状态的显存开销与 FP32 主权重的必要性，并介绍大 Batch 场景下的 LARS/LAMB"
pubDate: 2026-07-06
...
**1. 能说清 SGD → Momentum → Adam → AdamW 每一步各解决了什么问题、多存了几份状态**

<details>
<summary>参考答案</summary>

- **SGD**：只用当前梯度更新（$\theta \leftarrow \theta - \eta g$），无额外状态（0 份），但容易在狭长山谷里左右震荡。
- **Momentum**：加一阶动量 $m$（惯性）平滑震荡、加速收敛，每参数多存 1 份状态。
- **Adam**：同时维护一阶动量 $m$（管"往哪走"）和二阶动量 $v$（梯度平方的指数移动平均，管自适应步长）——历史梯度大的参数分母大、步子收着点，共 2 份状态。
- **AdamW**：解决原始 Adam 把权重衰减塞进梯度、与自适应学习率纠缠（正则化力度被 $\sqrt{\hat{v}}$ 扭曲）的问题，把权重衰减解耦成独立的参数缩放步骤，状态仍是 2 份，是当今大模型训练的事实标准。

（对应正文第 2 节）

</details>

**2. 能手算任意规模模型在 AdamW + 混合精度下的优化器状态显存，并解释它为什么约占静态显存的 75\%**

<details>
<summary>参考答案</summary>

混合精度下优化器内部一律用 FP32（每个数 4 字节）：AdamW 每参数要存 FP32 主权重 + FP32 $m$ + FP32 $v$，共 **12 字节**。7B 模型即 $12 \times 7 \times 10^9 \approx 84$ GB。完整静态账本还要加上 BF16 参数 $2 \times 7 \times 10^9 = 14$ GB 和 BF16 梯度 14 GB，合计约 112 GB，其中优化器状态占 $84/112 = 75\%$（BF16 参数、梯度各占 12.5\%）。这正是 ZeRO-1 第一刀砍向优化器状态的原因——它是显存占比最大的一块。（对应正文第 3.1、3.2 节）

</details>

**3. 能解释混合精度训练中优化器为什么必须保留 FP32 主权重，而不能直接用 BF16**

<details>
<summary>参考答案</summary>

每步参数更新量约等于学习率 × 梯度，量级常在 $10^{-4}$ 甚至更小；BF16 只有 7 位尾数、相对精度很粗，这么小的更新量加到较大的参数上等于没加（大数吃小数）。而且训练要跑成千上万步，每步的舍入误差会不断累积，用低精度存参数最终会导致训练发散。所以优化器必须保存一份 FP32 的主权重（master weights），每步用 FP32 精度完成更新，再转成 BF16 供下一次前向——这份副本（7B 约 28 GB）省不掉。（对应正文第 3.3 节）

</details>

**4. 能写出统计 PyTorch 优化器 `state` 元素数的代码，验证 Adam 每参数存 2 份、SGD 存 0 份**

<details>
<summary>参考答案</summary>

思路：构造一个参数量已知的小模型（如 `nn.Linear(1000, 1000)`，1,001,000 个参数），先跑一步 `loss.backward()` + `optimizer.step()`（Adam 此时才真正分配状态），再遍历 `optimizer.state.items()`，把其中张量条目的 `numel()` 累加。运行会看到 Adam 存了 `exp_avg`（一阶动量 $m$）和 `exp_avg_sq`（二阶动量 $v$）两份，各自元素数都等于参数量，总状态元素约等于 2 × 参数量；换成不带 momentum 的 `torch.optim.SGD`，`state` 几乎是空的（0 份）——这就是 SGD 省显存的原因。（对应正文第 5 节）

</details>

**5. 能回答"数据并行把 Batch 放大 10 倍，学习率该怎么处理"，并说清 LARS/LAMB 在什么场景下才有必要**

<details>
<summary>参考答案</summary>

先试两招简单的工程手段：**线性缩放规则**（Batch 放大 $k$ 倍，学习率也放大 $k$ 倍）配合**学习率 warmup**（前几百到几千步让学习率从很小线性爬到目标值，避开初期剧烈震荡），能覆盖大多数中等规模场景。只有当数据并行卡数很多（64+）、有效 Batch 极大、AdamW 已经压不住训练不稳时，才有必要换逐层自适应的优化器：LAMB 是 Adam + 逐层信赖域缩放，曾把 BERT 预训练的 Batch 拉到 65536 仍稳定收敛、训练从 3 天压到 76 分钟；LARS 基于 SGD + Momentum，按"参数范数/梯度范数"给每层定步长，主要用于 ResNet 等 CV 大 Batch 场景。常规场景老老实实用 AdamW 即可。（对应正文第 4 节）
