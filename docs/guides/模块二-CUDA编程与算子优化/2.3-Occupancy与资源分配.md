...
**1. 能从寄存器数、共享内存大小和 Block 大小三个维度手算 Occupancy**

<details>
<summary>参考答案</summary>

- 定义：Occupancy = 每 SM 活跃 Warp 数 / 每 SM 最大 Warp 数（A100 为 64 Warp = 2048 线程）。
- 分别算四个上限再取最小值：① 寄存器：每 Warp 分配量 = 每线程寄存器数 × 32（按 256 个一组的粒度向上对齐），SM 可容纳 Warp = ⌊65536 / 每 Warp 寄存器⌋；② 共享内存：SM 可容纳 Block = ⌊164KB / 每 Block 共享内存⌋，再乘每 Block 的 Warp 数；③ 每 SM 最大 Block 数（A100 为 32）；④ 每 SM 最大线程数 2048（= 64 Warp）。
- 别忘了 Block 是整体分配的：最终 Warp 数要向下取整到每 Block Warp 数的整数倍。
- 正文示例：40 寄存器/线程、256 线程/Block、16KB 共享内存 → min(51, 80, 64) = 51 → ⌊51/8⌋ × 8 = 48 Warp → Occupancy = 48/64 = 75%。

（对应正文第 1.1、3、4.1 节）

</details>

**2. 能解释为什么 Occupancy 100% 的 Kernel 不一定比 50% 的快**

<details>
<summary>参考答案</summary>

- Occupancy 只衡量延迟隐藏能力，性能是 Occupancy、ILP、数据复用率、访存效率等多因素的函数。
- 低 Occupancy 的 Kernel 可以用更多寄存器和共享内存**增加数据复用**，减少总的全局内存访问量。正文对比：Version A（32 寄存器、8KB smem、100% Occupancy）只有 800 GFLOPS，Version B（96 寄存器、48KB smem、33% Occupancy）达到 1200 GFLOPS。
- 盲目追求高 Occupancy 的三个代价：寄存器溢出到局部内存、Tile 变小导致复用不足、过多活跃 Warp 争抢 L1/L2 引起缓存抖动。
- 正确做法：不设固定 Occupancy 目标，用 profiler 实测吞吐量。

（对应正文第 5.1、5.2、5.5 节）

</details>

**3. 能使用 `cudaOccupancyMaxPotentialBlockSize` 自动选择 Block 大小**

<details>
<summary>参考答案</summary>

- `cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, kernel, dynamicSmemSize, blockSizeLimit)` 自动计算使 Occupancy 最大的 Block 大小（blockSizeLimit 传 0 表示无上限）。
- 配合 `cudaOccupancyMaxActiveBlocksPerMultiprocessor(&maxActiveBlocks, kernel, blockSize, dynamicSmem)` 查询给定配置下每 SM 的活跃 Block 数。
- Occupancy = maxActiveBlocks × blockSize / prop.maxThreadsPerMultiProcessor（设备属性通过 `cudaGetDeviceProperties` 获取）。

（对应正文第 4.2 节）

</details>

**4. 能用 `__launch_bounds__` 限制寄存器用量并理解其副作用（spilling）**

<details>
<summary>参考答案</summary>

- `__launch_bounds__(maxThreadsPerBlock, minBlocksPerMultiprocessor)` 向编译器承诺启动配置：如 `(256, 2)` 表示需要 256 × 2 = 512 线程同时驻留，编译器据此把每线程寄存器上限压到 65536 / 512 = 128 个（通常还会更激进压缩）。
- 使用原则：访存密集（带宽瓶颈）用较大的 minBlocks 提升 Occupancy；计算密集（FLOP 瓶颈）用 1 保留寄存器。另一方式是编译时全局限制 `nvcc -maxrregcount=32`。
- 副作用：放不下的变量 spill 到 Local Memory（显存），可能反而降低性能，必须用 profiler 实际验证。

（对应正文第 3.1 节）

</details>

**5. 能解释 Register Spilling 发生的条件及对性能的影响**

<details>
<summary>参考答案</summary>

- 发生条件：Kernel 实际需要的寄存器超过了编译器允许的上限——通常是为提升 Occupancy 用 `maxrregcount` 或 `__launch_bounds__` 强制压低了每线程寄存器数。
- 溢出的变量被放到局部内存，其实质是全局内存 + L1 缓存，访问速度比寄存器慢数十倍。
- 影响：为了更高 Occupancy 而触发 spilling 往往得不偿失；调优流程中若性能不升反降（寄存器溢出等），应回退并接受当前 Occupancy。

（对应正文第 3.1、5.2、6.3 节）

</details>

**6. 能判断一个 Kernel 是 Memory-bound 还是 Compute-bound，并据此决定 Occupancy 优先级**

<details>
<summary>参考答案</summary>

- Occupancy 对**内存访问密集且复用度低**的 Kernel 最关键：Elementwise（向量加法、激活函数）、归约（每个元素只读一次）、简单 Stencil——它们几乎无数据复用，纯靠 Warp 切换隐藏延迟，应优先追求高 Occupancy。
- 低 Occupancy 在以下场景是合理的：GEMM/矩阵乘法（大量数据复用）、FlashAttention（用更多共享内存减少 HBM 访问）、计算密集型 Kernel（瓶颈在计算而非访存）。
- 判断依据与验证：用 Nsight Compute 实测——Memory-bound 且 Occupancy < 50% 时尝试提升 Occupancy；Compute-bound 或性能已足够时优化其他方面。

（对应正文第 5.3~5.5、6.3 节）

</details>

**7. 能从 Nsight Compute 报告中读出 Occupancy 限制因素**

<details>
<summary>参考答案</summary>

- 关键指标：`launch__occupancy`（理论 Occupancy）、`sm__warps_active.avg.pct_of_peak_sustained_active`（实际活跃 Warp 占比）、`launch__registers_per_thread`、`launch__shared_mem_per_block_allocated`。
- 报告的 Occupancy Limiters 部分直接列出各因素各自允许的 Occupancy（如 Registers: 50% ← 瓶颈、Shared Memory: 75%、Block Size: 100%），取最小者即当前瓶颈。
- 编译期辅助：`nvcc -Xptxas -v` 输出每个 Kernel 的寄存器数、smem、cmem 用量（如 "Used 40 registers, 16384 bytes smem"）。

（对应正文第 6.1、6.2 节）

</details>

**8. 能对 GEMM 类 Kernel 合理权衡 Tile 大小与 Occupancy**

<details>
<summary>参考答案</summary>

- 小 Tile（TILE_M=TILE_N=64，每线程算 4×4 = 16 个输出）：约 48 寄存器、4KB 共享内存，Occupancy 约 75%。
- 大 Tile（TILE_M=TILE_N=128，每线程算 8×8 = 64 个输出）：约 128 寄存器、8KB 共享内存，Occupancy 约 25%。
- 但大 Tile 每次从 Global 加载的数据被复用 8 次（小 Tile 只有 4 次），每线程计算量更大、ILP 更充分，实际性能通常大 Tile 更高。
- 结论：GEMM 属于"用寄存器和共享内存换数据复用"的场景，低 Occupancy 是合理取舍。

（对应正文第 7.1、5.4 节）

</details>

**9. 能使用动态共享内存和 `cudaFuncSetAttribute` 灵活配置共享内存**

<details>
<summary>参考答案</summary>

- Kernel 内声明 `extern __shared__ float smem[];`，launch 时通过第三个配置参数 `<<<grid, block, smem_size>>>` 按需指定大小，而非编译期写死。
- 需要更大动态共享内存时（Volta+），先调用 `cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size)` 提升上限。
- 好处是可在运行时按设备能力和 Occupancy 结果选择配置：如 16KB 下 Occupancy > 0.5 就用 16KB，否则退到 8KB。

（对应正文第 7.2 节）

</details>

**10. 能为不同类型的 Kernel 选择合适的 Block 大小**

<details>
<summary>参考答案</summary>

- 太小的 Block 会被"每 SM 最大 Block 数"限制：A100 上 Block=32 线程（1 Warp）时最多 32 个 Block = 32 Warp，Occupancy 只有 50%；64 线程起即可达 100%。
- 通用建议：至少 128 或 256，**256 是不错的默认值**，既避开 Block 数限制又给编译器优化空间。
- 按场景：Memory-bound → 256 或 512（需要高 Occupancy 隐藏延迟）；Compute-bound → 128 或 256（够用即可，优先保证寄存器）；大量共享内存 → 128 或 256（小 Block 让 SM 容纳更多 Block）；归约 → 256 或 512（更多线程参与，减少归约步骤）。
