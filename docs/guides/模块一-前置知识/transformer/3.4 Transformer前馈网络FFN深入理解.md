...
- [3. 激活函数详解](#3-激活函数详解)
- [4. SwiGLU：当前大模型的主流选择](#4-swiglu当前大模型的主流选择)
...
激活函数是 FFN 中至关重要的组件。没有激活函数，两层线性变换的复合仍然是线性变换（$xW_1 W_2$ 等价于一个 $x(W_1 W_2)$ 的单层线性变换），FFN 就失去了存在的意义。激活函数引入的非线性是模型表达能力的关键来源。
...
这里引入了两个独立的线性投影：
- $xW_{up}$：生成"候选内容"——这些信息准备通过
- $\sigma(xW_{gate})$：生成"门控信号"——每个维度的通过概率（0 到 1 之间）
...
$$
\text{SwiGLU}(x) = (\text{Swish}(xW_{gate})) \otimes (xW_{up})
$$

完整的 SwiGLU FFN 包含三个权重矩阵：

$$
\text{FFN}_{SwiGLU}(x) = \left[\text{Swish}(xW_\text{gate}) \otimes (xW_\text{up})\right] W_\text{down}
$$
...
这是因为（行向量视角，$h$ 拆分列维度、$W_{down}$ 拆分行维度）：

$$
h W_{down} = \begin{bmatrix} h_0 & h_1 \end{bmatrix} \begin{bmatrix} W_{down,0} \\ W_{down,1} \end{bmatrix} = h_0 W_{down,0} + h_1 W_{down,1}
$$
...
1. kernel A: $x_{\text{gate}} = x W_{gate}$，写入 HBM
2. kernel B: $\text{gate} = \text{Swish}(x_{\text{gate}})$，从 HBM 读出 $x_{\text{gate}}$ 并写回 gate
...
**1. 能说清 FFN 在 Transformer Block 中的角色——为什么 Attention 之后还需要 FFN，FFN 负责什么**

<details>
<summary>参考答案</summary>

- Self-Attention 是 **token 间**的运算，负责信息交互，本质是加权求和（线性运算）；仅靠线性的信息汇聚，模型表达能力会受到严重限制。
- FFN 是 **token 内**的运算：对每个 token 的表示向量独立施加相同的非线性变换，token 之间不再交互，形成"先交互、后加工"的两步设计。
- FFN 引入的非线性赋予模型逼近任意连续函数的能力（万能近似定理：带非线性激活的两层前馈网络可逼近任意连续函数）。
- 近年研究发现 FFN 还扮演"知识库"角色：大量事实性知识编码在 FFN 参数中，Attention 层更多负责组织和提取——这也解释了 FFN 为什么占单个 Block 约 2/3 的参数量。

（对应正文第 1 节）

</details>

**2. 能在白板上画出标准 FFN 和 SwiGLU FFN 的结构图，标注每个矩阵的维度和数据流方向**

<details>
<summary>参考答案</summary>

- 标准 FFN 是单条链路：输入 $(N, d_{model})$ → $W_1$（形状 $(d_{model}, d_{ff})$，升维）→ 激活函数 $\sigma$ → $W_2$（形状 $(d_{ff}, d_{model})$，降维）→ 输出 $(N, d_{model})$，即 $\text{FFN}(x) = \sigma(xW_1)W_2$。
- SwiGLU FFN 是两条并行路径再汇合：门控路径 $\text{Swish}(xW_{gate})$ 与内容路径 $xW_{up}$（两个矩阵形状均为 $(d_{model}, d_{ff})$）逐元素相乘，再经 $W_{down}$（形状 $(d_{ff}, d_{model})$）压缩回原始维度。
- 以 $d_{model}=4096$ 为例：标准 FFN 的 $d_{ff}=16384$（4 倍），SwiGLU 的 $d_{ff}=11008$（约 8/3 倍）。
- 画图要点：标准 FFN 两次 GEMM，SwiGLU 三次 GEMM；激活在标准 FFN 中施加于升维结果，在 SwiGLU 中只施加于门控路径。

（对应正文第 2、4 节）

</details>

**3. 能手算标准 FFN 和 SwiGLU FFN 的参数量，并解释为什么两者参数量几乎相同**

<details>
<summary>参考答案</summary>

- 标准 FFN 两个矩阵：$2 \times d_{model} \times 4d_{model} = 8 \times d_{model}^2$。以 $d_{model}=4096$、$d_{ff}=16384$ 计算，$W_{up}$ 和 $W_{down}$ 各 67,108,864，合计约 134M。
- SwiGLU FFN 三个矩阵，但中间维度缩小为 $\frac{8}{3} \times d_{model}$：$3 \times d_{model} \times \frac{8}{3}d_{model} = 8 \times d_{model}^2$。以 $d_{ff}=11008$ 计算，三个矩阵各 45,088,768，合计约 135M。
- 两者几乎相同的原因：SwiGLU 正是通过把中间维度从 $4\times$ 降到 $\frac{8}{3}\times$，抵消了多出的第三个矩阵，刻意保持总参数量为 $8 \times d_{model}^2$ 不变。
- 代价是前向传播需要 3 次 GEMM 而非 2 次，计算量略有增加。

（对应正文第 5 节）

</details>

**4. 能解释为什么 FFN 的中间维度选择 4x（标准）和 8/3x（SwiGLU），以及 LLaMA 中 11008 这个数值的由来**

<details>
<summary>参考答案</summary>

- $4\times$ 来自原始 Transformer 论文（$d_{model}=512, d_{ff}=2048$），是经验性的平衡点：太小（如 2x）高维空间表达能力受限，太大（如 8x）参数膨胀但收益边际递减。Scaling Laws 相关研究也表明该比值在 4 附近近似最优。
- SwiGLU 有 3 个矩阵，若沿用 $4\times$，总参数量变为 $12 \times d_{model}^2$，比标准 FFN 多 50%。为保持 $8 \times d_{model}^2$ 不变，解方程 $3 \times d_{model} \times d_{ff} = 8 \times d_{model}^2$ 得 $d_{ff} = \frac{8}{3} \times d_{model}$。
- LLaMA-2-7B：$\frac{8}{3} \times 4096 = 10922.67$，工程上取整到 256 的倍数得 $11008 = 43 \times 256$，以满足 GPU Tensor Core 对矩阵维度的对齐要求，获得更好的利用率。

（对应正文第 6 节）

</details>

**5. 能分别说清 ReLU、GELU、Swish 三种激活函数的数学定义、直觉含义和各自优缺点**

<details>
<summary>参考答案</summary>

- **ReLU**：$\max(0, x)$，"单向阀门"式硬开关。优点是计算最省（一次比较）、正半轴梯度恒为 1、带来稀疏性；缺点是零点不可微，且存在神经元死亡问题。输出范围 $[0, +\infty)$。
- **GELU**：$x \cdot \Phi(x)$（$\Phi$ 为标准正态 CDF），直觉是"以概率 $\Phi(x)$ 保留 $x$"的概率化软门控。处处光滑可微，无神经元死亡，BERT、GPT-2 采用；缺点是计算量比 ReLU 大（erf 或 tanh 近似）。输出范围约 $[-0.17, +\infty)$。
- **Swish**：$x \cdot \sigma(\beta x)$（$\beta=1$ 时即 SiLU），"自门控"——用输入自身控制通过量。处处光滑、非单调，负半轴有最小值约 $-0.278$ 的小凹坑；平滑的损失曲面使优化更稳定，深层网络（>40 层）表现一致优于 ReLU。LLaMA 在 SwiGLU 中使用。

（对应正文第 3 节）

</details>

**6. 能解释 ReLU 的神经元死亡问题为什么会发生，以及 GELU 和 Swish 如何避免这个问题**

<details>
<summary>参考答案</summary>

- 当某神经元的输入落入负区间时，ReLU 输出为 0 且梯度为 0；零梯度意味着反向传播时权重得不到任何更新信号，下一轮输入大概率仍为负——神经元就此永久"死亡"，不再对计算做贡献。
- 学习率较大时尤为严重：一次大幅度更新可能把权重推到对所有样本输出都为负的"坏"区域。实际训练中有时超过 40% 的 ReLU 神经元处于死亡状态，模型有效容量大幅缩水。
- LeakyReLU 给负半轴一个微小斜率（如 0.01x）只是打补丁，没有解决零点不光滑的问题。
- GELU 和 Swish 在负半轴的输出和梯度均非零（软门控），神经元在任何输入下都能收到更新信号，因此不存在死亡问题。

（对应正文第 3.1–3.3 节）

</details>

**7. 能解释 SwiGLU 的门控机制——$W_{gate}$ 和 $W_{up}$ 各自的作用，为什么门控比统一激活更有效**

<details>
<summary>参考答案</summary>

- $W_{up}$ 是内容投影，生成"候选内容"；$W_{gate}$ 是门控投影，其输出经 Swish 激活后产生"门控信号"。两者逐元素相乘：$\text{FFN}_{SwiGLU}(x) = \left[\text{Swish}(xW_{gate}) \otimes (xW_{up})\right] W_{down}$，门控值大的维度保留信息，门控值小的维度抑制信息。
- 更精细的信息筛选：标准 FFN 对所有维度施加同一个全局非线性；门控允许模型对每个维度学习不同的"通过/抑制"决策，实现条件化计算。
- 更丰富的梯度信号：门控路径和内容路径是两个独立的线性投影，梯度可沿两条路径分别回传，与残差连接的"梯度高速公路"思想异曲同工。
- 实验验证：Shazeer 论文在相同参数量下 SwiGLU 一致取得最佳表现，LLaMA、Mistral、Qwen、DeepSeek 等主流大模型均采用。

（对应正文第 4 节）

</details>

**8. 能描述 Megatron-LM 对 FFN 做张量并行的切分方式——列切分 $W_{up}$/$W_{gate}$，行切分 $W_{down}$，为什么这样切分只需要一次 AllReduce**

<details>
<summary>参考答案</summary>

- $W_{up}$（和 $W_{gate}$）按列切分：每张 GPU 持有 $(d_{model}, d_{ff}/k)$ 的一部分，各自算出中间结果的一段。激活函数是逐元素操作，可以在切分后的分片上独立施加，不需要看到完整向量。
- $W_{down}$ 按行切分：每张 GPU 用自己的中间分片乘以自己的 $(d_{ff}/k, d_{model})$ 部分，得到形状完整但数值不完整的部分输出。
- 由矩阵分块乘法 $hW_{down} = h_0 W_{down,0} + h_1 W_{down,1}$，各卡部分输出求和即为最终结果——所以整个 FFN 前向只在最后做一次 AllReduce，中间的三次 GEMM 和逐元素操作全部本地完成（每个 Block 中 Attention 也贡献一次，共两次 AllReduce）。
- 工程上 $d_{ff}$ 需能被 TP 卡数整除：如 11008 在 TP=2/4/8 时每卡分别为 5504/2752/1376。

（对应正文第 7 节）

</details>

**9. 能概述 MoE 的基本思想——多个专家 FFN、路由机制、负载均衡，以及 Expert Parallelism 的通信模式**

<details>
<summary>参考答案</summary>

- 核心思想：把一个巨大 FFN 拆成多个较小的专家 FFN，每个 token 只激活少数专家（如 64 个专家选 Top-2），实现"参数量大但每 token 计算量小"。
- 路由机制：路由器 $g(x) = \text{softmax}(xW_r)$ 把 token 映射到专家概率分布，选出 Top-K 个专家，输出为被选中专家输出的加权和（权重 re-normalize 为和为 1）。
- 负载均衡：防止"明星专家"垄断 token 的手段包括辅助损失 $L_{aux} = \alpha \cdot n_{experts} \cdot \sum_i f_i \cdot p_i$、容量因子（超出容量的 token 丢弃或重路由）、Expert Choice 路由（专家主动选 token，天然均衡）。
- Expert Parallelism：不同专家放在不同 GPU 上，通信模式是 **All-to-All**（每卡把 token 发给目标专家所在卡并接收本地专家的 token），与张量并行的 AllReduce 截然不同，是 MoE 系统设计的核心挑战。

（对应正文第 8 节）

</details>

**10. 能举出至少两个 FFN 中 CUDA kernel 融合的具体场景**

<details>
<summary>参考答案</summary>

- 场景一：**Swish 激活 + 门控乘法融合**——把 $\text{Swish}(\text{gate}) \times \text{up}$ 合为一个 kernel，gate 的中间结果驻留寄存器，省去写入 HBM 再读回的开销。
- 场景二：**GEMM epilogue fusion**——在 GEMM kernel 的收尾阶段直接在寄存器中对输出施加激活函数再写回 HBM，cuBLAS/CUTLASS 均提供接口。
- 场景三：**$W_{gate}$ 和 $W_{up}$ 的 GEMM 合并**——两者输入相同，可拼接为 $(d_{model}, 2 \times d_{ff})$ 的大矩阵做一次 GEMM，减少 launch 开销且大 GEMM 利用率更高。
- 场景四：**偏置/残差加法 + LayerNorm 融合**——FFN 输出、残差输入一次性读入片上完成加法和归一化。
- 收益：Decode 阶段（memory bound、kernel launch 开销占比高）融合可带来 20–40% 加速；Prefill 阶段 GEMM 计算占主导，收益约 5–15%。
