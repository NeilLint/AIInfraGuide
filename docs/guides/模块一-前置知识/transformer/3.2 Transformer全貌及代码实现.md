...
- [9. PyTorch 代码实现：从零搭建 Transformer](#9-pytorch-代码实现从零搭建-transformer)
- [总结](#-总结)
- [自我检验清单](#-自我检验清单)
- [参考资料](#-参考资料)
...
$$
P(\text{next\_token} \mid x_1, x_2, \ldots, x_n) = \text{Softmax}\left(\text{RMSNorm}\left(\text{DecoderBlocks}^{(L)}(\text{Embed}(x))\right) W_{lm}\right)
$$
...
$$
\text{Params} \approx L \times (12 \cdot d_{model}^2) + 2 \cdot V \cdot d_{model}
$$

其中 $12 \cdot d_{model}^2$ 是单层 Decoder Block 的参数量（4 个 Attention 矩阵 + SwiGLU 的 3 个矩阵约等于 $4d^2 + 3 \times \frac{8}{3}d^2 = 12d^2$），$2 \cdot V \cdot d_{model}$ 是 Embedding 与 LM Head 的参数量（LLaMA 系列两者不共享权重；若像 GPT-2、Gemma 那样 tie embedding，则只算一份 $V \cdot d_{model}$）。
...
**1. 能画出原始 Transformer 的 Encoder-Decoder 整体结构图，标注三种注意力的位置和 Q/K/V 来源**

<details>
<summary>参考答案</summary>

整体结构：左侧 Encoder $\times 6$ 层（每层 = Self-Attention + FFN），右侧 Decoder $\times 6$ 层（每层 = Masked Self-Attention + Cross-Attention + FFN），每个子层外包残差连接 + LayerNorm（Post-Norm）。原论文配置：$d_{model}=512$、$h=8$、$d_{ff}=2048$。

三种注意力的位置与 Q/K/V 来源：

| 注意力类型 | 所在位置 | Q 来自 | K、V 来自 |
|-----------|---------|--------|----------|
| Encoder Self-Attention | Encoder 层 | 输入序列 | 输入序列 |
| Masked Decoder Self-Attention | Decoder 层 | 已生成序列 | 已生成序列 |
| Cross-Attention | Decoder 层 | 已生成序列（Decoder 当前层输出） | Encoder 最后一层输出 |

（详见第 2 节）

</details>

**2. 能解释 Encoder Self-Attention、Masked Decoder Self-Attention、Cross-Attention 各自的作用和区别**

<details>
<summary>参考答案</summary>

三者底层计算相同（都是 Scaled Dot-Product Attention），区别在 Q/K/V 来源和掩码：

- **Encoder Self-Attention**：双向，输入 token 之间互相交换信息，注意力矩阵全 1 无遮挡——像读原文时反复揣摩每个词与其他词的关系
- **Masked Decoder Self-Attention**：带因果掩码，已生成的 token 之间交换信息但不能看未来（下三角矩阵）——像写译文时回顾自己已写的内容
- **Cross-Attention**：Q 来自 Decoder，K/V 来自 Encoder 输出，生成过程中"回头查看"输入序列——像写译文时对照原文；注意力矩阵形状是 $(N_{tgt} \times N_{src})$，不是方阵，且不需要因果掩码（翻译时需要看到完整原文）

（详见第 2.3、3.3、4.2–4.3 节）

</details>

**3. 能说清 Encoder-only、Encoder-Decoder、Decoder-only 三种架构分别保留和砍掉了原始 Transformer 的哪些组件**

<details>
<summary>参考答案</summary>

- **Encoder-only（BERT 路线）**：保留双向 Self-Attention + FFN + 残差 + LayerNorm；砍掉因果掩码、Cross-Attention、自回归生成。适合分类、NER 等"理解"任务
- **Encoder-Decoder（T5 路线）**：保留完整的 Encoder + Decoder + Cross-Attention；改进是把所有 NLP 任务统一为 text-to-text 格式
- **Decoder-only（GPT 路线）**：保留因果 Self-Attention + FFN + 残差 + LayerNorm；砍掉 Encoder、Cross-Attention、双向注意力，用纯"预测下一个 token"训练，最终成为大模型时代的绝对主流

（详见第 5 节）

</details>

**4. 能从头到尾描述 Decoder-only LLM 的完整推理数据流：token ID → Embedding → N 层 Block → Norm → LM Head → Softmax → 采样**

<details>
<summary>参考答案</summary>

1. **Tokenizer** 把输入文本切成 token ID 序列（如 5 个 token）
2. **Token Embedding** 查表 $W_{emb}$，得到 $(5, d_{model})$
3. **L 层 Decoder Block**：每层做两段——`RMSNorm → Masked Self-Attention（含 RoPE）→ 残差相加`，再 `RMSNorm → SwiGLU FFN → 残差相加`；维度始终保持 $(5, d_{model})$
4. **Final RMSNorm** 稳定数值范围
5. **LM Head**：$(5, d_{model}) \times (d_{model}, V)$ 得 logits $(5, V)$
6. **取最后一个位置做 Softmax**，得到词表上的概率分布，按采样策略选出下一个 token
7. **自回归循环**：新 token 拼接到输入末尾，重复步骤 2–6，直到结束

（详见第 6.1 节）

</details>

**5. 能默写 Decoder-only 模型的五大组件（Embedding、Block 堆叠、Final Norm、LM Head、Softmax）及各自职责**

<details>
<summary>参考答案</summary>

1. **Token Embedding**："单词-向量"对照字典，嵌入矩阵 $W_{emb} \in \mathbb{R}^{V \times d_{model}}$（LLaMA-2-7B 约 131M 参数，仅占总量约 2%）
2. **Decoder Block 堆叠**：模型主体，占 95%+ 的参数和计算量；三个全局性质——维度守恒（输入输出都是 $(N, d_{model})$，可任意堆叠）、层间参数不共享、层数决定深度
3. **Final LayerNorm**：最后一层 Block 输出后、送入 LM Head 之前稳定数值范围，防止深层数值漂移导致输出分布不稳定
4. **LM Head**：线性层 $W_{lm} \in \mathbb{R}^{d_{model} \times V}$，把隐藏状态映射为词表大小的 logits
5. **Softmax + 采样**：logits 转为概率分布，按 Greedy / Top-k / Top-p / Temperature 等策略选出下一个 token

（详见第 6.2 节）

</details>

**6. 能根据给定的配置参数（$d_{model}$、$n_{layers}$、vocab_size 等）估算模型的总参数量**
...
其中 $12 d_{model}^2$ 是单层 Block 的参数量（Attention 4 个矩阵 $4d^2$ + SwiGLU 3 个矩阵 $3 \times \frac{8}{3}d^2 = 8d^2$），$2 V d_{model}$ 是 Embedding 与 LM Head（若 weight tying 则只算一份）。

以 LLaMA-2-7B 验证（$d_{model}=4096$、$L=32$、$V=32000$、$d_{ff}=11008$）：单层约 201M（4 个 16.8M 的 Attention 矩阵 + 3 个 45.1M 的 FFN 矩阵），32 层约 6,432M，加 Embedding 和 LM Head 各 131M，总计约 6.7B。参数量对宽度是二次方关系、对深度是线性关系。（详见第 7.2–7.3 节）

</details>

**7. 能解释 Weight Tying 的含义和工程价值**

<details>
<summary>参考答案</summary>

Weight Tying（权重共享）指让 LM Head 的权重直接复用 Token Embedding 的权重矩阵（转置后使用），即 $W_{lm} = W_{emb}^\top$。

价值有二：一是**省参数**——以 LLaMA-2-7B 的规模（$V=32000$、$d_{model}=4096$）计算，可减少约 131M 参数；二是**语义上合理**——"把向量映射回 token"和"把 token 映射为向量"应该是互逆操作，共享权重符合这一直觉。（详见第 6.2 节）

</details>

**8. 能从工程角度说出至少三个 Decoder-only 对 AI Infra 更友好的原因**

<details>
<summary>参考答案</summary>

- **架构统一性**：只有一种 Block 结构反复堆叠——CUDA kernel 只需优化一套、并行策略只需设计一次、任意一层的特性代表所有层，性能分析和瓶颈定位更简单
- **KV Cache 管理简单**：只有一套 KV Cache，PagedAttention 等优化可直接应用；不像 Encoder-Decoder 要同时处理 Encoder 输出缓存和 Decoder KV Cache 两套数据
- **Scaling 路径清晰**：加层数、加宽度、加头数三个旋钮，效果可预测，不需要纠结 Encoder 和 Decoder 的比例分配
- **训练数据利用率高**："预测下一个 token"让每个 token 都产生训练信号（loss），数据利用率接近 100%；Encoder-Decoder 的训练信号只在 Decoder 部分产生

（详见第 8.3 节）

</details>

**9. 能说清三种 Mask 的名称、使用场景和 shape（`(b, q_len, k_len)`）**
...
| Mask 类型 | 使用位置 | 形状 | 目的 |
|-----------|---------|------|------|
| 长度 Mask | Encoder 自注意力 | `(b, enc_len, enc_len)` | 屏蔽 Encoder padding 位置 |
| Causal Mask | Decoder 自注意力 | `(b, dec_len, dec_len)` | 防止看到未来 token |
| Cross-Attn Mask | Decoder Cross-Attention | `(b, dec_len, enc_len)` | 屏蔽 Encoder 输出中的 padding |

三者都符合 `(b, q_len, k_len)` 的统一形状——q_len 是 Query 侧序列长度，k_len 是 Key 侧序列长度（Cross-Attn 的两侧长度不同，所以不是方阵）。实现上 `True` 表示需要屏蔽，用 `masked_fill_(mask, -1e4)` 把分数填为大负数，softmax 后权重趋近 0。（详见第 9.1 节）

</details>

**10. 能解释 Causal Mask 为什么用上三角矩阵，以及 `torch.triu(..., diagonal=1)` 的含义**

<details>
<summary>参考答案</summary>

训练时为了效率，整个目标序列一次性并行输入 Decoder（Teacher Forcing），但预测位置 $i$ 只能看到位置 $0$ 到 $i$——若不遮蔽，模型预测第 3 个词时就能看到第 4、5 个词，等于"开卷考试"，学不到预测能力。

注意力矩阵中第 $i$ 行第 $j$ 列表示"位置 $i$ 关注位置 $j$"，需要屏蔽的正是 $j > i$ 的未来位置——它们恰好构成**严格上三角**区域，因此掩码是上三角矩阵（可关注区域是下三角）。

`torch.triu(torch.ones(...), diagonal=1)` 保留严格上三角（$j > i$ 的位置为 1/True），`diagonal=1` 表示从主对角线上方第一条对角线开始保留——这样对角线本身（自己看自己）不被屏蔽，第 $i$ 行只能 attend 到第 $0$ 到 $i$ 列。（详见第 4.2、9.1 节）

</details>

**11. 能描述 Multi-Head Attention 的四个步骤：线性投影 → 拆分多头 → Scaled Dot-Product Attention → 合并输出**

<details>
<summary>参考答案</summary>

1. **线性投影**：$W_Q / W_K / W_V$ 把输入 `(N, q_len, d_model)` 投影为 `d_k * num_heads` 维
2. **拆分多头**：`view(N, -1, num_heads, d_k).transpose(1, 2)` 变为 `(N, num_heads, q_len, d_k)`，让每个头在自己的子空间独立计算；mask 通过 `unsqueeze(1).repeat(...)` 广播到 head 维度
3. **Scaled Dot-Product Attention**：`scores = Q @ K^T / sqrt(d_k)`，被屏蔽位置 `masked_fill_(mask, -1e4)`，softmax 得注意力权重后乘 V
4. **合并输出**：`transpose(1, 2).reshape(N, -1, d_v * num_heads)` 把多头拼接回去，最后过输出投影 `W_out` 映射回 `d_model`

原论文标准配置 $d_{model}=512$、8 头、$d_k=d_v=64$，此时 $d_{model} = d_k \times \text{num\_heads}$，输入输出维度一致。（详见第 9.2 节）

</details>

**12. 能写出正弦位置编码的公式，并解释为什么 Transformer 需要显式位置编码而 RNN 不需要**
...
生成的编码直接**加**到 token embedding 上：$\text{Input} = \text{TokenEmbed}(x) + \text{PosEncode}(\text{position})$，实现中通过 `nn.Embedding.from_pretrained(..., freeze=True)` 固定为不可学习参数。

为什么 Transformer 需要而 RNN 不需要：Attention 机制本身不包含位置信息——打乱 token 顺序，$QK^T$ 加权求和的计算结果不变，所以必须显式注入位置信号；而 RNN 按顺序逐个处理序列元素，位置信息天然隐含在逐步递推的计算顺序中，无需额外编码。（详见第 3.1、9.3 节）

</details>

**13. 能运行完整的前向验证代码，并解释输出 `logits` 的维度含义**

<details>
<summary>参考答案</summary>

前向验证用随机 dummy 数据（batch_size=16、Encoder 输入最长 100、Decoder 目标最长 50、词表 26），组装 `Transformer(frontend, encoder, decoder, hidden_dim, vocab_size)` 后执行 `model(fbank_feature, feat_lens, labels)`。

输出 `logits` 形状为 `(batch_size, dec_len, vocab_size)`，即 `torch.Size([16, 50, 26])`——含义是 batch 中每个样本、每个解码位置上，在整个词表（26 个候选 token）上的得分分布；对应每个解码步骤在词表上的概率分布（过 softmax 后），接上交叉熵损失即可训练。（详见第 9.8 节）
