...
**1. 能推导数据并行的平均梯度为什么严格等于单卡全局 batch 梯度，并解释梯度为何要"求平均"而非"求和"**

<details>
<summary>参考答案</summary>

设全局 batch 为 $B$、卡数 $N$，损失是样本损失的平均，故全局梯度 $g$ 带 $\frac{1}{B}$ 系数。每卡的本地梯度在自己的 local batch（$\frac{B}{N}$ 个样本）内已经做过一次平均，带 $\frac{N}{B}$ 系数；对 $N$ 个本地梯度再取平均，系数恰好变回 $\frac{1}{B}$，精确还原出单卡在全局 batch 上的梯度 $g$——数学上严格等价。如果改成"求和"，等价于把学习率放大了 $N$ 倍。注意等价性的前提：同步梯度、且各卡 local batch 大小相同（数据不均或异步更新会打破它）。（对应正文第 2 节）

</details>

**2. 能说清 DP 的三个致命缺陷，以及 DDP 分别是如何针对性解决的**

<details>
<summary>参考答案</summary>

DP 三个致命缺陷：**GIL 限制**（单进程多线程，Python 全局解释器锁让多线程无法真正并行调度）；**负载不均**（主卡额外承担 Scatter/Gather、loss 计算、梯度汇总，常见"主卡 OOM 其他卡还很空"）；**通信低效**（每步重新复制模型、数据经主卡中转挤在 PCIe 上）。DDP 的针对性解法：每卡一个独立进程，消除 GIL 争抢；去中心化 AllReduce，各卡地位对等、无主卡热点；模型只在初始化时 Broadcast 一次，梯度同步走点对点 Ring、充分利用 NVLink。病根总结：DP 是"单进程 + 主卡中心化"，DDP 把这两个前提都拆掉了。（对应正文第 3.2、4.1 节）

</details>

**3. 能解释 DDP 的 Bucket 机制如何实现计算与通信的重叠，以及 `find_unused_parameters` 的作用**

<details>
<summary>参考答案</summary>

反向传播是从最后一层往前逐层算的，靠后的层梯度先就绪。DDP 把参数梯度按反向计算顺序打包成若干 Bucket（默认约 25 MB 一个）：某个 Bucket 里的梯度全部就绪，就**立即**对它发起 AllReduce，同时更靠前的层继续算反向——通信与计算就重叠起来了，通信几乎被"藏进"计算里。`find_unused_parameters=True` 用于模型中有参数未参与前向的场景（如条件分支）：这些参数的梯度永远不就绪，对应 Bucket 永远等不满，AllReduce 会卡住死锁；该选项让 DDP 主动标记未使用的参数，但有额外开销，能避免则避免。（对应正文第 4.2 节）

</details>

**4. 能写出 AllReduce = ReduceScatter + AllGather 的等价关系，并推导 Ring AllReduce 单卡通信量约 $2\Psi$**

<details>
<summary>参考答案</summary>

一次 AllReduce 在实现上等于一次 ReduceScatter（每卡数据分成 $N$ 块，跨卡对相同位置的块求和，每卡拿到一块聚合完成的结果）加一次 AllGather（每卡把自己那块广播出去，所有卡集齐完整结果）。Ring 实现下，数据切成 $N$ 份，两个阶段各需 $N-1$ 步、每步每卡向邻居发送 $\frac{\Psi}{N}$，故每卡发送总量为 $2 \times (N-1) \times \frac{\Psi}{N} = 2\Psi \cdot \frac{N-1}{N} \approx 2\Psi$（$N$ 大时），**与卡数无关**。对比中心化汇总：主卡收发量 $2(N-1)\Psi$ 随 $N$ 线性膨胀、立刻成为瓶颈。（对应正文第 5 节）

</details>

**5. 能说出 ZeRO 三个阶段各分片了什么，分别对应 FSDP 的哪种 `sharding_strategy`**

<details>
<summary>参考答案</summary>

- **Stage 1**：分片优化器状态，每卡显存 $2\Psi + 2\Psi + \frac{12\Psi}{N}$，包含在 FSDP 的 `SHARD_GRAD_OP` 内。
- **Stage 2**：再分片梯度，每卡 $2\Psi + \frac{14\Psi}{N}$，对应 `SHARD_GRAD_OP`。
- **Stage 3**：再分片参数，每卡 $\frac{16\Psi}{N}$，对应 `FULL_SHARD`。

另有两个策略：`NO_SHARD`（不分片，等价 DDP，用于调试对照）和 `HYBRID_SHARD`（机内 FULL_SHARD + 机间数据并行，多机训练首选）。（对应正文第 6.1、6.3 节）

</details>

**6. 能画出 FSDP 一个 unit 前向 + 反向的通信流程（AllGather / ReduceScatter 出现在哪些位置）**

<details>
<summary>参考答案</summary>

初始：每卡只存 $\frac{1}{N}$ 参数分片 → **前向 AllGather** 拼出完整参数 → 用完整参数做前向计算 → 立即释放非本地分片、只留自己的 $\frac{1}{N}$ → **反向再次 AllGather** 拼出完整参数（因为前向后已释放）→ 算出完整梯度 → **反向 ReduceScatter** 一步同时完成梯度聚合与分片回各卡 → 释放完整参数、每卡只留自己那片梯度 → 更新时各卡只更新自己负责的 $\frac{1}{N}$ 参数和优化器状态。即每个 unit：前向 1 次 AllGather，反向 1 次 AllGather + 1 次 ReduceScatter。（对应正文第 6.2 节）

</details>

**7. 能解释 FSDP 反向为什么用 ReduceScatter 而不是 AllReduce**

<details>
<summary>参考答案</summary>

因为 FSDP 每卡只负责更新自己的参数分片，只需要拿到"自己那片"的聚合梯度就够了，不必像 DDP 那样让每卡都持有完整梯度。ReduceScatter 一步同时完成"梯度聚合"和"分片回各卡"，单卡通信量只有 $\Psi$（AllReduce 的一半），正好匹配"分片更新"的需求。这个差异直接决定了 DDP（$2\Psi$）与 FSDP（$3\Psi$）通信量的不同。（对应正文第 6.2 节）

</details>

**8. 能算出 DDP 和 FSDP 各分片策略下的单卡显存，并说明优化器状态为何是最大头**

<details>
<summary>参考答案</summary>

混合精度 + Adam 下模型状态共 $16\Psi$：FP16 参数 $2\Psi$ + FP16 梯度 $2\Psi$ + FP32 优化器状态 $12\Psi$（master 权重 $4\Psi$ + 一阶动量 $4\Psi$ + 二阶动量 $4\Psi$）。各方案单卡显存：**DDP** 恒定 $16\Psi$（7B 约 112 GB，与卡数无关）；**SHARD_GRAD_OP**（ZeRO-2）$2\Psi + \frac{14\Psi}{N}$（7B、8 卡约 26 GB）；**FULL_SHARD**（ZeRO-3）$\frac{16\Psi}{N}$（约 14 GB，随卡数线性下降）。优化器状态 $12\Psi$ 占 75\%、是最大头——所以 ZeRO-1/2 只分片优化器状态和梯度就能省掉大半，性价比很高。注意这笔账不含激活值。（对应正文第 7 节）

</details>

**9. 能解释 FSDP 每步通信量 $3\Psi$ 比 DDP 的 $2\Psi$ 多 50\% 的来源**

<details>
<summary>参考答案</summary>

DDP 每步一次梯度 AllReduce，即 $2\Psi$。FSDP 每步拆成三块：前向 AllGather 拼参数（$\Psi$）+ 反向 AllGather 再拼一次参数（前向后已释放，$\Psi$）+ 反向 ReduceScatter 同步并分片梯度（$\Psi$），合计 $3\Psi$。原因在于 AllReduce 本身 = ReduceScatter + AllGather（两个 $\Psi$ 操作的组合），FSDP 把这两半拆开用，又额外多付一次前向 AllGather，所以是 $\Psi + \Psi + \Psi = 3\Psi$，比 DDP 多 50\%。这笔额外通信在高带宽机内基本能被 prefetch 重叠掉，低带宽跨机则可能暴露成瓶颈——这正是 `HYBRID_SHARD` 存在的意义。（对应正文第 8 节）

</details>

**10. 能根据"单卡是否装得下 / 是否多机"给出 DDP、`SHARD_GRAD_OP`、`FULL_SHARD`、`HYBRID_SHARD` 的选型决策**

<details>
<summary>参考答案</summary>

- **单卡装得下完整模型状态** → 用 DDP，简单高效、通信最省；别为"显得高级"上 FSDP，白付 50\% 通信。
- **单机、差一点装不下** → FSDP `SHARD_GRAD_OP`（ZeRO-2），只分片梯度 + 优化器，通信比 FULL_SHARD 省。
- **单机、差很多** → FSDP `FULL_SHARD`（ZeRO-3），显存最省。
- **多机 + 大模型** → FSDP `HYBRID_SHARD`：通信密集的分片关在机内高带宽 NVLink 上，机间慢速链路只跑低频数据并行。
- **单个层都装不下**（如超大 Embedding 或 FFN）→ 数据并行维度到头，需叠加张量并行（TP）、流水线并行（PP），走向 3D 并行。
