...
**1. 能解释长 Prefill 为什么会拖慢同批 Decode 请求的 TPOT**

<details>
<summary>参考答案</summary>

- Prefill 是 Compute Bound：一次要并行处理整段 Prompt 的所有 Token，Prompt 越长这一步的矩阵运算量越大、耗时越久；Decode 是 Memory Bound，每步只处理 1 个 Token，本身很快，出字速度（TPOT）靠它一步步稳定推进。
- Continuous Batching 是**每一步重新组批**的：若某一步把一个 4000 Token 长 Prompt 的 Prefill 和一批 Decode 请求塞进同一个 step，这一步的耗时会被庞大的 Prefill 计算**整体拉长**——同批所有 Decode 请求的这个 Token 都得等 Prefill 算完才能一起出来。
- 用户端表现为：出字很流畅，突然卡顿一下，再恢复流畅——TPOT 出现尖刺（P99 抖动）。

（对应正文第 1 节）

</details>

**2. 能描述 Chunked Prefill 如何把计算尖峰削平成多个小峰**

<details>
<summary>参考答案</summary>

- 核心思路：不要求一个长 Prompt 在一步里 Prefill 完，而是把它切成若干固定大小的 Chunk，分散到多个 step 逐块处理——比如 4000 Token 的 Prompt 按每块 512 Token 切成 8 个 Chunk，每步只处理一块。
- 每一步的计算量被控制在可预期范围内，不会被超长 Prefill 突然撑爆；剩下的算力留给同批的 Decode 请求正常出字。
- Decode 请求得以在长 Prefill 进行的同时**持续、平稳地推进**，TPOT 尖刺被抹平——本质是把一次性的计算尖峰削平成多个可控的小峰。

（对应正文第 2 节）

</details>

**3. 能说清 Chunked Prefill 用什么代价（TTFT）换来什么收益（TPOT 稳定）**

<details>
<summary>参考答案</summary>

- **代价**：长 Prompt 要跨多步才 Prefill 完，首 Token 出得稍晚，该请求的 TTFT 略微变大。
- **收益**：同批 Decode 请求的 TPOT 平稳、尾延迟被稳住。
- 这是一个刻意的权衡——牺牲少量单请求 TTFT，换取整批 Decode 的 TPOT 稳定；在追求平稳出字体验的服务里，这笔交易通常很划算。
- 另外它与 Prefix Cache 是天然搭档：前缀命中先砍掉一部分 Prefill 计算，剩下未命中的部分再切块掺入 Decode，长 Prompt 场景下 TTFT 和 TPOT 能同时得到照顾。

（对应正文第 2、7 节）

</details>

**4. 能解释 Token Budget 为什么能统一度量 Prefill 和 Decode**

<details>
<summary>参考答案</summary>

- 核心洞察：**无论 Prefill 还是 Decode，对 GPU 来说本质都是"处理若干个 Token 的前向计算"**。
- 一个做 Decode 的请求这一步贡献 1 个 Token；一个 Prefill 请求（或它的一个 Chunk）这一步贡献 N 个 Token（这一块的长度）。
- 调度器给每步设一个总的 Token 预算上限（比如 2048），往里装请求：先保证正在生成的 Decode 请求（各占 1 个额度），剩余预算再分给 Prefill 的 Chunk，预算用完就发车——约束即 $\sum \text{Decode} + \sum \text{Prefill Chunk} \leq \text{token\_budget}$。
- 有了这把统一的尺子，"Chunk 切多大、一步塞几个 Decode、长 Prefill 与 Decode 怎么共处"都归结成同一个装箱问题：预算内尽量装满 GPU，同时优先保证 Decode 不断流。

（对应正文第 3 节）

</details>

**5. 能说明 vLLM V1 用 `{request_id: num_tokens}` 统一表示带来的好处**

<details>
<summary>参考答案</summary>

- V0 里 Prefill 和 Decode 是两类被区别对待的阶段，调度逻辑要分别处理，代码复杂且切换时易产生空隙；V1 彻底取消这个二分——调度决策就是一个字典 `{request_id: num_tokens}`，约束是所有 value 之和不超过 `token_budget`。
- 这一个统一表示同时涵盖了 **Chunked Prefill**（较大的值）、**Decode**（1）、**Speculative Decoding 验证**（若干候选 Token）——它们本质都是"某个请求这一步要处理多少 Token"的不同取值，一套调度框架优雅统一所有情况。
- 也因此 V1 能把 Chunked Prefill 做成**默认行为**：调度天然以 Token 为单位、不区分阶段，长 Prompt 自然被预算约束切成 Chunk，无需额外的特殊逻辑。

（对应正文第 4 节）

</details>

**6. 能从源码角度说清"预算截断即切块"以及 `num_computed_tokens` 如何续块**

<details>
<summary>参考答案</summary>

- 调度器给每个请求计算"这一步处理多少 Token"时，用剩余预算截断：`num_tokens = min(need, token_budget)`。一个 4000 Token 的 Prompt，若这一步只剩 512 预算，就只 Prefill 512，剩下 3488 留到后续步——它自动成了一个 Chunk。**切块不是一个独立的"切块函数"，而是预算截断的自然结果。**
- 续块靠 `num_computed_tokens` 记账：请求"已计算到第几个 Token"被持久记录，下一步从断点继续；追上 `num_prompt_tokens`（Prefill 完成）后，自然转入每步 +1 的 Decode——这正是"不分 Prefill/Decode 阶段"在数据上的体现。
- 一句话精髓：`num_computed_tokens` 一路推向 `num_tokens_with_spec`，每步推进多少由 `min(剩余需求, 剩余预算)` 决定。

（对应正文第 5 节）

</details>

**7. 能描述 `max_num_batched_tokens` 调大/调小分别如何影响吞吐与 TPOT**

<details>
<summary>参考答案</summary>

- `max_num_batched_tokens` 就是一步能处理的最大 Token 数，即 Token Budget；`max_num_seqs` 限制一步最多容纳的请求数，两者共同决定实际批大小。
- **调大（偏吞吐）**：一步塞更多 Token，GPU 装得更满，系统吞吐更高；但长 Prefill 占的比重上升、单步耗时变长，Decode 的 TPOT 稳定性下降。
- **调小（偏延迟稳定）**：每步计算量更可控，Decode 出字更平稳、TPOT 尾延迟更低；但单步能干的活变少，峰值吞吐可能受限。
- 没有普适最优值——要在目标负载（真实 Prompt 长度分布、并发量）下压测，观察 TTFT、TPOT P95/P99 和吞吐三条曲线，找满足 SLO 的最大吞吐点。

（对应正文第 6 节）

</details>

**8. 能判断自己的负载是否适合开启 Chunked Prefill**

<details>
<summary>参考答案</summary>

- **收益明显**：Prompt 长度差异大、有超长 Prompt 混入（长文档问答、长上下文 RAG）；对出字流畅度敏感、要压 TPOT 尾延迟的在线服务（如实时对话）；Prefill 和 Decode 混合的高并发在线负载。
- **收益有限**：纯离线批量推理、只追求总吞吐而不在乎单请求延迟抖动（切块的调度开销未必划算）；Prompt 都很短、本来就没有长 Prefill 独占一步的情况。
- 判断标准两条：你的服务**是否在乎 TPOT 的稳定性**，以及负载里**有没有长 Prompt**——Chunked Prefill 是"平稳性"优化而非"峰值吞吐"优化。
