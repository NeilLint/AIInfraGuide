...
**1. 能解释标准 Attention 为什么是 Memory-Bound 而非 Compute-Bound**

<details>
<summary>参考答案</summary>

- 标准 Attention 的算术强度为 $\text{AI} = \frac{O(N^2 d)}{O(N^2 + Nd)} \approx O(d)$，而 $d$ 通常只有 64 或 128，算术强度很低。
- 中间矩阵 $S$ 和 $P$ 大小均为 $N \times N$，需要反复读写 HBM，HBM 读写量为 $O(N^2)$；当 $N = 4096$、$d = 128$ 时，单个 head 的 $S$ 矩阵就需要 32 MB（FP16），多 head、多 batch 下开销更可观。
- 因此瓶颈在 HBM 与 SRAM 之间的数据搬运（显存带宽）而非算力：大量时间花在搬数据上，GPU 计算单元处于饥饿状态——这正是 Memory-Bound 的典型特征。

（对应正文 1.2、1.3 节）

</details>

**2. 能手动推导 Online Softmax 的递推更新公式**

<details>
<summary>参考答案</summary>

维护局部最大值 $m^{(j-1)}$ 与局部指数和 $l^{(j-1)}$，当第 $j$ 块数据 $x^{(j)}$ 到来时按三步递推：

- 更新最大值：$m^{(j)} = \max(m^{(j-1)}, \max(x^{(j)}))$
- 修正并更新指数和：$l^{(j)} = l^{(j-1)} \cdot e^{m^{(j-1)} - m^{(j)}} + \sum_i e^{x^{(j)}_i - m^{(j)}}$
- 修正已有输出：$O^{(j)} = O^{(j-1)} \cdot \frac{l^{(j-1)} \cdot e^{m^{(j-1)} - m^{(j)}}}{l^{(j)}} + \frac{\sum_i e^{x^{(j)}_i - m^{(j)}} \cdot V^{(j)}_i}{l^{(j)}}$

正确性来自恒等式 $e^{x_i - m_{\text{new}}} = e^{x_i - m_{\text{old}}} \cdot e^{m_{\text{old}} - m_{\text{new}}}$：每当最大值更新，只需给旧的指数项乘上修正因子 $e^{m_{\text{old}} - m_{\text{new}}}$，结果与标准 Softmax 数学上完全等价，**没有任何近似**。

（对应正文 3.2、3.3 节）

</details>

**3. 能说明 Tiling 块大小 $B_r$、$B_c$ 如何根据 SRAM 容量确定**

<details>
<summary>参考答案</summary>

- 块大小公式：$B_c = \lceil M/(4d) \rceil$，$B_r = \min(\lceil M/(4d) \rceil, d)$，其中 $M$ 是 SRAM 容量（元素个数）、$d$ 是 head dimension。
- 分母 $4d$ 的来源：计算一个块时 SRAM 需同时驻留 $Q_i, K_j, V_j, O_i$ 四块约 $B \times d$ 的张量，合计约 $4Bd$ 个元素，由 $4Bd \le M$ 解出每块最多 $M/(4d)$ 行；相对较小的 $S_{ij}$（$B_r \times B_c$）和统计量 $m_i, l_i$（$2B_r$）不主导 SRAM 占用，可忽略。
- $B_r$ 再与 $d$ 取 $\min$：Online Softmax 的修正系数是逐行标量，原论文为简化统计量管理约束 $B_r \le d$，该约束在 V2 中被放宽。

（对应正文 4.1、4.2 节）

</details>

**4. 能描述 FlashAttention 前向传播中外循环和内循环各遍历什么**

<details>
<summary>参考答案</summary>

- V1 采用**外循环遍历 K/V 块、内循环遍历 Q 块**的结构：外循环每次把 $K_j, V_j$ 从 HBM 加载到 SRAM 一次。
- 内循环每次从 HBM 加载 $Q_i, O_i, m_i, l_i$，计算局部分数 $S_{ij} = Q_i K_j^\top / \sqrt{d}$，用 Online Softmax 更新统计量和输出，再把更新后的 $O_i, m_i, l_i$ 写回 HBM。
- 后果：每个 K/V 块只从 HBM 加载一次，但 Q 和 O（及 $m, l$）会随 $T_c$ 次外循环被反复加载和写回——这是 V2 调换循环顺序要改进的点。

（对应正文 4.3、5.2 节）

</details>

**5. 能解释为什么重计算策略不会让 wall-clock 时间变长**

<details>
<summary>参考答案</summary>

- 反向传播不保存 $S$ 和 $P$，只保存 $Q, K, V, O, m, l$，反向时重新计算 $S_{ij}$ 并用 $P_{ij} = \text{diag}(l_i)^{-1} \cdot e^{S_{ij} - m_i}$ 恢复 $P$。
- 重计算约增加 50\% 前向计算量的额外 FLOPs，但换来显存从 $O(N^2)$ 降到 $O(N)$、HBM 访问大幅减少。
- 由于 Attention 是 Memory-Bound 的，时间瓶颈在访存而非算力，额外的计算被 HBM 访问的减少所"隐藏"，实际 wall-clock 时间反而更短。

（对应正文 6.2、6.3 节）

</details>

**6. 能推导 FlashAttention 的 HBM IO 复杂度为 $O(N^2 d^2 / M)$**

<details>
<summary>参考答案</summary>

- 由块大小约束 $B_c = O(M/d)$，外循环共 $T_c = N/B_c = O(Nd/M)$ 次。
- 每次外循环要遍历完整的 $Q$ 和 $O$（共 $O(Nd)$ 个元素），故 $Q$ 和 $O$ 的总访问量为 $T_c \cdot Nd = O(N^2 d^2 / M)$；$K, V$ 整体只读一次（$O(Nd)$），可忽略。
- 由于 $d^2 < M$（典型 $d = 128$ 时 $d^2 = 16384$，而 $M$ 约为 98304 个 FP16 元素），该量远小于标准实现的 $O(N^2)$；直观上 SRAM 越大、块越大、外循环越少、对 $Q/O$ 的重复读写越少。
- 论文还证明了任何精确 Attention 算法的 HBM 访问下界为 $\Omega(N^2 d^2 / M)$，故 FlashAttention 在渐近意义上 IO 最优。

（对应正文 7.2、7.3 节）

</details>

**7. 能用 Triton 或 CUDA 实现简化版 FlashAttention 前向 Kernel**
...
- 并行划分：grid 取 `(batch, head)`，每个 program 串行扫完该 `(batch, head)` 切片的所有 K/V 块（外循环）与 Q 块（内循环）；块大小由 `BLOCK_M`（$B_r$）、`BLOCK_N`（$B_c$）、`BLOCK_D` 决定。
- 数据布局：$K_j, V_j$ 外循环加载一次常驻 SRAM/寄存器，被所有 Q 块复用；因不同外循环迭代要共享 Q 行的统计量，必须在 HBM 开辟 `[B, H, N]` 的辅助张量 `M, L` 持久化 $m_i, l_i$（$O_i$ 同样每轮读出写回）。
- 正确性关键：K/V 越界列要用 `tl.where(kv_mask, s, -inf)` 屏蔽，否则假 0 会污染 softmax 分母；`m, l` 初值取 `-inf` 和 `0`，配合 `alpha = exp(-inf - m_new) = 0` 让首次迭代自然退化为正常累加。
- 内循环核心即 5.1 节公式：`m_new = max(m_i, rowmax(S))`、`l_new = alpha * l_i + rowsum(exp(S - m_new))`、输出按 `alpha * l_i / l_new` 修正后累加 `P @ V`。

（对应正文 8.2、8.3 节）

</details>

**8. 能说明 FlashAttention V1 中外循环遍历 K/V 的局限性**

<details>
<summary>参考答案</summary>

- 并行度不足：grid 只有 $B \times H$，推理场景常只有几十个 program（如 batch=1、heads=32 时仅 32 个），而现代 GPU 有 80~140 个 SM（A100=108，H100=132），SM 利用率只有约 30\%。
- HBM 流量大：单个 program 装不下所有 Q 块的 $m/l/O$，必须借助 HBM 辅助张量 `M, L` 在外循环迭代之间持久化，导致 Q/O/m/l 被 $T_c$ 次外循环反复读写。
- 这两点正是 V2 出现的动因：V2 把 Q 提到 grid 上并行、调换内外循环，让 $m/l/O$ 全程留在寄存器中，消除这部分 HBM 访问。
