...
**1. 能写出 world_size 的乘法分解，说清五个维度各切什么、谁不省显存、谁直接省激活**

<details>
<summary>参考答案</summary>

$\text{world\_size} = TP \times PP \times DP\,(\times EP \times CP)$——五个维度切互不相同的东西，所以可以正交叠加：DP 切 batch 维、TP 切单个权重矩阵、PP 切层（深度维）、EP 切 MoE 专家集合、CP 切序列维。只有纯 DP 不省显存（要靠 ZeRO-1/2/3 逐级分掉优化器/梯度/参数），只有 CP 直接砍激活。通信频率排序：TP ≈ EP ≈ CP（每层多次）> PP（每 micro-batch）> DP（每 step 一次）。（对应第 1 节）

</details>

**2. 能用带宽与消息数字论证"TP 进节点、PP 跨节点、DP 最外"**

<details>
<summary>参考答案</summary>

节点内 NVLink 约 600 GB/s，节点间 IB 每卡约 12.5～50 GB/s，差一个数量级以上，排布的本质是把通信需求金字塔对齐到带宽金字塔。TP 每层前向 2 次 + 反向 2 次激活 AllReduce，单条消息 $s \times b \times h$（$s{=}4096, h{=}8192, b{=}1$ 时 64 MB），串行卡在计算路径、几乎无法重叠，只有 NVLink 咽得下；PP 每 micro-batch 每边界只传一次同量级消息，频率低一个数量级且可与计算重叠，IB 承受得起；DP 每 step 一次梯度同步、能被 Bucket 机制藏进反向，放最慢链路损失最小。（对应第 2.1 节；另见 Q2）

</details>

**3. 能解释 Megatron 的 rank 编排顺序为什么让"TP 落 NVLink 域"自动成立**

<details>
<summary>参考答案</summary>

`initialize_model_parallel()` 默认按 tp → cp → ep → dp → pp 从"变化最快"到"变化最慢"排布：TP 维变化最快，同一 TP 组的 rank 物理相邻（rank 0-7 就是节点 0 的 8 张卡），天然落在 NVLink 域内；PP 维变化最慢，同一 pipeline 的不同 stage 分布在不同节点，stage 间 P2P 走 IB。"TP 放节点内"是 rank 排序的**结果**而非框架硬编码——排查分布式性能问题的第一件事，就是确认逻辑通信组与物理拓扑对齐。（对应第 2.2 节）

</details>

**4. 能算出 64 卡训 70B 的模型状态总账，并推出 TP=8 后为什么仍必须叠加 PP**

<details>
<summary>参考答案</summary>

混合精度 + Adam 的模型状态是 $16\Psi$ 字节（BF16 参数 $2\Psi$ + BF16 梯度 $2\Psi$ + FP32 master 权重与两个动量 $12\Psi$），$\Psi = 70$B 即 1120 GB——纯 DP 每卡一份完整 $16\Psi$，单卡 80 GB 直接出局。TP 满配 8 后每卡仍分到 $1120/8 = 140$ GB $> 80$ GB，必须叠加 PP。最终落点是 TP8 / PP2 / DP4 + ZeRO-1 + 选择性重计算，激活吃紧时退到 PP4/DP2。（对应第 3.1、3.2、3.5 节）

</details>

**5. 能解释 3D 并行里为什么几乎只见 ZeRO-1、PP 配 ZeRO-3 会发生什么**

<details>
<summary>参考答案</summary>

优化器状态 $12\Psi$ 是 $16\Psi$ 里的大头，ZeRO-1 把它沿 DP 维再切一刀成本极低（只在 step 末多一次通信重排），所以是 PP 的标准搭档；而 ZeRO-3 按层 AllGather 参数的模式会与 PP 的 micro-batch 循环相乘，通信爆炸。Megatron 的 distributed optimizer 与 ZeRO-1 同理。（对应第 3.3 节；另见 Q7）

</details>

**6. 能用激活公式判断一个配置要不要开重计算**

<details>
<summary>参考答案</summary>

单层单 micro-batch 激活 $\approx \frac{sbh(34 + \frac{5as}{h})}{t}$，再乘"每 stage 层数 × 1F1B 在途 micro-batch 数（第一个 stage 为 $p$）"。案例中 PP4 方案不做重计算：$0.81 \times 20 \times 4 \approx 65$ GB，加 21.9 GB 模型状态约 87 GB $> 80$ GB 必 OOM；改选择性重计算（只重算 attention 内部大激活）后单层降到约 $\frac{34sbh}{t} \approx 0.14$ GB，激活约 11.4 GB，再预留 CUDA context、NCCL 缓冲与 logits 峰值约 10 GB，稳定在 45 GB 以内，方案成立。（对应第 3.4 节）

</details>

**7. 能写出 $\text{GBS} = DP \times b_{\text{micro}} \times m$ 并说清三个量各自的角色**

<details>
<summary>参考答案</summary>

GBS 是算法量，由收敛特性决定（70B 级预训练常见 4M token 量级），不应随并行配置变动或扩卡而改变——加 DP 卡数时变的是 $m$，不是 GBS；$b_{\text{micro}}$ 是显存旋钮；$m$ 是被动算出的梯度累积步数，PP 下同时决定气泡率、必须 $m \gg p$。梯度累积与一次大 batch 严格等价还要求：只在最后一个 micro-batch 做梯度同步（DDP 的 `no_sync`）、loss 按 $m$ 归一、模型中没有 BatchNorm 这类跨样本统计层。（对应第 4 节）

</details>

**8. 能复述 DeepSeek-V3 的并行配置，并给出"全程无 TP"的三条理由**

<details>
<summary>参考答案</summary>

2048 张 H800 = 16 PP（DualPipe 双向流水线）× 128 DP（ZeRO-1），EP=64 嵌在 DP 维内（专家分布在 8 个节点、参数冗余度 $128/64 = 2$），全程无 TP。三条理由：一是 MLA 压缩注意力参数、细粒度专家把 FFN 拆成中间维仅 2048 的小矩阵，没有矩阵大到必须切开，EP 已承担参数分摊；二是 FP8 训练、RMSNorm/MLA 上投影重计算、EMA 放 CPU、MTP 共享 Embedding 等组合拳省出了 TP 的显存份额；三是省掉每层 4 次激活 AllReduce，把节点内 NVLink 带宽全部让给 EP 的 All-to-All 转发。（对应第 6 节）
...
- TP > 8 或 TP 组跨节点——每层 AllReduce 走 IB，吞吐崩塌；
- $m$ 接近甚至小于 $p$——气泡率逼近甚至超过 50%；
- PP 各 stage 均分层数——首末 stage 因 Embedding、LM head 与 logits 先 OOM；
- PP 搭配 ZeRO-3——参数 AllGather 次数乘 micro-batch 数，通信爆炸；
- DP=1 还开 ZeRO——分片对象只有自己，纯开销；
- EP 组横跨过多节点且无路由约束——All-to-All 打满 IB，计算等通信。（对应第 7.1 节）

</details>

**10. 能按优先级排出 OOM 时的旋钮顺序，并先判断 OOM 发生的时机**

<details>
<summary>参考答案</summary>

顺序的逻辑是"先动便宜的、再动贵的"：mbs 降到 1 → 激活重计算（先选择性、再全量）→ ZeRO-1 / distributed optimizer → 加深 PP → 加大 TP（上限 8）→ 长序列场景改上 CP → CPU/NVMe offload 兜底。动手前先定位时机：第一个 step 就炸多半是 $16\Psi$ 模型状态账没算对；训练中途偶发的炸更可能是激活峰值（logits、最长序列样本）、显存碎片（可试 `expandable_segments`）或 MoE 负载尖峰。（对应第 7.2 节；另见 Q8）
