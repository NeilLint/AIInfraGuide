...
**1. 能解释为什么 Decode 阶段 CPU 的 Kernel 启动开销会成为瓶颈**

<details>
<summary>参考答案</summary>

- GPU 每做一个操作（矩阵乘、LayerNorm、激活……）都要 CPU 通过一次"启动（launch）"命令告知；跑一遍 Transformer 有成百上千个小操作，对应成百上千次 CPU→GPU 启动调用，每次有微秒级的固定 CPU 开销。
- Prefill 一步处理成千 Token，每个 Kernel 一跑就是几百微秒甚至毫秒，启动开销被淹没；但 **Decode 每步只处理 1 个（或很少几个）Token**，每个 Kernel 实际计算可能就几微秒——CPU 发射 Kernel 的时间比 GPU 执行它的时间还长。
- 结果是 GPU 干几微秒就停下来等 CPU 发下一条命令——算力闲置，这次不是等数据而是**等指令**。Decode 因此有两重等待：等数据（Memory Bound）和等指令（CPU launch 开销），小模型、小 Batch 时后者尤其突出。

（对应正文第 1 节）

</details>

**2. 能说出至少三种 vLLM Attention 后端及各自的适用场景**

<details>
<summary>参考答案</summary>

- **FlashAttention**：IO 感知的经典高性能实现（支持到 FlashAttention 3），主流 NVIDIA GPU 的通用首选。
- **FlashInfer**：面向推理优化，KV Cache 处理灵活、支持多种数据类型，适合高吞吐服务和需要灵活 KV 布局的场景。
- **FlashMLA**：针对 MLA（Multi-head Latent Attention）优化，适合 DeepSeek 等采用 MLA 结构的模型。
- **Triton**：用 Triton 语言实现、可移植性好，非 NVIDIA 硬件或需自定义时的兜底。
- 多数情况下不用手动指定——vLLM 会根据 GPU 架构、模型类型和数据精度自动挑选；后端支持矩阵随版本演进快，以所用版本官方文档为准。

（对应正文第 2 节）

</details>

**3. 能解释"数学等价、工程不同"的 Attention 后端意味着什么**

<details>
<summary>参考答案</summary>

- 各 Attention 后端是**同一个数学操作的不同工程实现**：它们算出的结果在数学上等价，换后端不改变模型输出的正确性。
- 区别在于**访存模式、并行策略、对特定硬件指令和模型结构的适配程度**。
- 因此选后端是纯粹的性能工程问题：选对后端，同样的卡能跑出明显不同的吞吐；只有做深度性能调优或跑特殊模型结构（如 MLA）时才需要显式干预。

（对应正文第 2 节）

</details>

**4. 能说明为什么统一调度器需要后端支持 Prefill/Decode 混合批次**

<details>
<summary>参考答案</summary>

- V1 的统一调度器会把 Prefill 的 Chunk 和 Decode 请求塞进**同一步**，但两者计算形态不同：Prefill 部分要算"一整段新 Token 之间 + 对历史的注意力"，Decode 部分只算"1 个新 Token 对全部历史的注意力"。
- 这就要求 Attention 后端**原生支持混合批次（mixed prefill/decode batch）**：在一次 Kernel 调用里同时正确处理两类请求。FlashAttention 3 等现代后端正是为此设计——能接受变长的、混合阶段的输入，用统一 Kernel 高效算完，不必拆成两次 Kernel 调用。
- "统一调度器"（软件层抹平 Prefill/Decode）和"支持混合批次的后端"（Kernel 层抹平）是**配套的**：没有后者，V1 的 Token 预算调度落不了地——这也是 V1 对后端能力有要求、早期不是所有后端都支持的原因。

（对应正文第 3 节）

</details>

**5. 能描述 CUDA Graph 通过"录制—重放"消除 CPU 开销的原理及其形状固定的前提**

<details>
<summary>参考答案</summary>

- 原理：把一连串固定的 GPU 操作**录制（capture）**成一张"图"，之后只需一条**重放（replay）**命令，GPU 就依次执行录好的所有 Kernel——成百上千次 CPU 启动被压缩成一次，CPU 开销几乎归零，GPU 不再空等指令。
- 硬性前提：**录制的操作序列和张量形状必须固定**。这与 Decode 是天作之合——每步都是"处理固定 Batch 个 Token"，形状稳定；而多变的 Prefill 较难直接套用。
- 实践上 vLLM 只为一组预设的 Batch Size 捕获图（`cudagraph_capture_sizes`）：实际 Batch Size 命中已捕获尺寸才走重放，否则回退到逐 Kernel 启动。

（对应正文第 4 节）

</details>

**6. 能解释 Piecewise CUDA Graph 为什么在 Attention 处切图，以及这样做的好处**

<details>
<summary>参考答案</summary>

- 矛盾在于：CUDA Graph 要求整段操作形状固定，而 **Attention 恰恰是最难兼容的部分**——涉及变长的 KV、复杂的 Mask，形状不规整；但因为 Attention 不好处理就放弃整张图又太可惜。
- vLLM V1 的解法：把前向图**在 Attention 算子处切开**——Attention 被包装成"不透明"的自定义算子，**保持 eager（即时）执行**，保留处理变长 KV 的灵活性；Attention 之外的规整 token-wise 部分（LayerNorm、GEMM、激活、残差……）用 CUDA Graph 捕获、重放。
- 好处是各取所长：CPU launch 开销的大头（成百上千个小算子）被 CUDA Graph 消灭，而 Attention 的灵活性（混合批次、变长 KV）不受损——这是 V1 相比 V0 在 Decode 上提速的关键工程之一。
- 兼容后端下还可用 Full CUDA Graph（把 Attention 也纳入捕获），在小模型或 MoE 的 Decode 等场景进一步提速；Piecewise 是通用默认，Full 是进阶选项。

（对应正文第 5 节）

</details>

**7. 能说清为什么切图后只产生三种子图，以及 `cudagraph_capture_sizes` 的作用**

<details>
<summary>参考答案</summary>

- Attention 被注册成自定义算子 `torch.ops.vllm.unified_attention_with_output`，Dynamo 追踪 `forward` 后按 `splitting_ops`（通常就是 Attention）把图切开。
- 由于 Transformer 是**同构层的堆叠**，切完只得到三种子图：**首层**（第一个 Attention 之前）、**中间重复层**（相邻两个 Attention 之间——被所有中间层复用，只需编译一次）、**末层**（最后一个 Attention 之后到输出）。中间层同构、子图可复用，几十层的模型也只编译三种子图，编译成本被摊得很低。
- `cudagraph_capture_sizes` 控制为哪些 Batch Size 捕获 CUDA Graph：命中列表才走重放，否则回退逐 Kernel 启动；`compile_sizes` 可选地让 Inductor 为特定静态形状自动调优（运行更快但首次编译更慢，默认关闭）。

（对应正文第 6 节）

</details>

**8. 能说清这些图优化的代价（启动、显存、调试）与生产环境的取舍**

<details>
<summary>参考答案</summary>

- **启动变慢**：服务启动时要做编译和 CUDA Graph 捕获，首次拉起耗时增加——vLLM 用编译缓存缓解，缓存目录可在部署间直接拷贝以省掉重复编译。
- **显存占用**：捕获多个 Batch Size 的 CUDA Graph 需要额外显存。
- **调试复杂度**：编译后的图不如 eager 模式好调试；排查数值异常时可临时关闭 `torch.compile`/CUDA Graph 用 eager 对比，定位完再打开。
- 取舍结论：收益是 Decode 阶段 CPU 开销大幅下降、小模型/小 Batch 下吞吐和延迟明显改善，这些优化默认开启、自动生效——**生产环境务必保持开启**，否则白白损失 Decode 性能。
