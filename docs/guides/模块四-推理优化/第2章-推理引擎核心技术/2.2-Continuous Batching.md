...
**1. 能从 Memory Bound 的角度解释批处理为什么能提升吞吐**

<details>
<summary>参考答案</summary>

- Decode 阶段是 Memory Bound：瓶颈在于把整个模型的权重从显存搬到计算单元，搬一次权重只服务 1 个请求的 1 个 Token 太亏。
- 批处理把多个请求的计算合并成一次大的矩阵运算，让一次权重搬运同时服务多个请求的 Token——算术强度成倍上升，闲置算力被利用起来，权重搬运成本被摊薄。
- 而且几乎不牺牲单请求延迟：因为瓶颈在搬权重，多算几个 Token 的计算量对 Memory Bound 的 Decode 来说近乎"免费"。

（对应正文第 1 节）

</details>

**2. 能说清 Static Batching 中短请求被长请求拖累的具体机制**

<details>
<summary>参考答案</summary>

- Static Batching 攒够一批请求一起送进模型，等这一批**全部生成完**才开始下一批；但请求的生成长度天差地别。
- 具体机制：请求 A 第 20 步就生成完了，却必须待在批次里干等最慢的请求 B 跑完 500 步。A 早已完成的那 480 步里，它在 Batch 中占的槽位纯粹空转——GPU 在为一个已经没有意义的位置做无用计算。
- 同时新请求必须等整批结束才能进来，排队延迟也被拉长。
- 结果是 GPU 利用率经常只有 30% 左右，请求长度差异越大浪费越严重。

（对应正文第 2 节）

</details>

**3. 能解释 Iteration-level Scheduling 与批级调度的本质区别**

<details>
<summary>参考答案</summary>

- 批级调度以"一批请求的完整生命周期"为调度单位：整批同进同出，中途不调整。
- Iteration-level Scheduling（迭代级调度）把粒度细化到**一个迭代步**：每生成一步（一个 Token）就重新调度一次——生成了 EOS 或达到长度上限的请求**立即移出批次、释放显存**；腾出空间就**立即从等待队列拉新请求**加入下一步计算。
- 于是批次构成是动态流动的：请求随到随拼、完成即退，GPU 几乎每一步都在为"真正还在生成"的请求满负荷干活，利用率从约 30% 提到 80% 以上。

（对应正文第 3 节）

</details>

**4. 能描述 vLLM 一次调度 step 中"退出完成请求、补入新请求"的流程**
...
1. **收新请求**：新到达的请求放进 Waiting 队列。
2. **调度决策**：决定这一步谁上车——兼顾 Running 队列的 Decode 请求和等待队列里能塞进来的新请求（做 Prefill），是否有空间取决于空闲 KV Block 数量（PagedAttention 提供的信息）。
3. **前向计算一步**：Decode 请求各产出 1 个新 Token，Prefill 请求产出首 Token。
4. **处理产出**：新 Token 追加进 KV Cache；触发 EOS 或长度上限的请求标记完成，**释放其 KV Block 归还空闲池**。
5. **循环**回到第 1 步。显存不够时调度器还会抢占：把部分请求的 KV 换出到 CPU 或丢弃后重算。

（对应正文第 4 节）

</details>

**5. 能说明为什么 Continuous Batching 必须依赖 PagedAttention 的细粒度显存管理**

<details>
<summary>参考答案</summary>

- "请求随退随补"的前提是**能随时、以细粒度分配和回收显存**。
- 如果还是传统的连续大块分配，一个请求退出后留下的空隙未必能恰好塞下新请求，动态补位就无从谈起。
- PagedAttention 的**固定大小块 + 按需分配 + 即时回收**恰好提供了这种弹性：请求完成，块立刻回到空闲池；新请求进来，从空闲池按需取块。
- 二者分工：PagedAttention 管"显存怎么放"，Continuous Batching 管"请求怎么调度"，两者相乘才有 vLLM 的高吞吐，单有其一都不够。

（对应正文第 5 节）

</details>

**6. 能说清 V1 Scheduler 中 `waiting`/`running` 两队列与 `schedule()` 的决策顺序**

<details>
<summary>参考答案</summary>

- `self.running` 是列表，装当前正在计算的请求；`self.waiting` 是优先级队列（策略有 `FCFS` 和 `PRIORITY` 两种），装排队等待的请求。
- `schedule()` 给定一个 **token 预算（`token_budget`）**，决策顺序是：**第一优先级**先遍历 `running` 给在途请求分配 Token 和 KV 块；预算有富余**且没发生抢占**时，才从 `waiting` 拉新请求进来做 Prefill——在途请求优先，避免"新请求不断插队、老请求饥饿"。
- 当 `allocate_slots` 返回 `None`（申请不到 KV 块）时触发抢占：FCFS 策略踢 `running` 里最后一个（最晚来的），PRIORITY 策略踢优先级最低的。
- 调度结果打包成 `SchedulerOutput` 交给模型执行；模型输出后由 `update_from_output()` 追加 Token、检查停止条件、释放已完成请求的资源。

（对应正文第 6.1、6.2 节）

</details>

**7. 能解释为什么 V1 调度器"不区分 Prefill/Decode 阶段"，以及默认抢占为何用重计算**

<details>
<summary>参考答案</summary>

- V1 源码注释明确写了 "no 'decoding phase' nor 'prefill phase'"：调度器眼里每个请求只是不停地把 `num_computed_tokens`（已计算 Token 数）推向 `num_tokens_with_spec`（含投机 Token 的总目标数）。这种统一视角正是 Chunked Prefill 能把 Prefill 和 Decode 混在一步里跑的前提。
- 被抢占的请求在 `_preempt_request()` 里释放 KV 块和编码器缓存、状态置为 `PREEMPTED`、**`num_computed_tokens` 归零**、重新塞回 `waiting` 队列的队头。
- `num_computed_tokens` 归零意味着默认恢复策略是**重计算（Recompute）**：下次调度时从头把已生成的 Token 当 Prompt 重新 Prefill，而不是从 CPU 换回 KV。重算实现简单、不占 CPU 内存，对多数场景是划算的默认选择。

（对应正文第 6 节）

</details>

**8. 能分析批次大小对吞吐与 TPOT 的相反影响**

<details>
<summary>参考答案</summary>

- 一步里塞进的请求越多，系统吞吐越高（权重搬运成本被摊得更薄），但每步的前向计算耗时也越长，单请求的 TPOT 会略微变差——批次大小需要在两者间取平衡。
- Continuous Batching 还让延迟指标变得**对负载敏感**：低负载时批次小、延迟低；高负载时批次变大、排队变长，P95/P99 尾延迟明显上升。
- 因此容量规划一定要在**目标并发下**压测延迟，而不是看空载数据。

（对应正文第 7 节）

</details>

**9. 能解释为什么长 Prompt 的 Prefill 会干扰同批 Decode，并知道该用什么技术缓解**

<details>
<summary>参考答案</summary>

- 新请求的 Prefill 是 Compute Bound 的重活：如果某一步塞进一个超长 Prompt 的 Prefill，这一步的耗时会被它整体拉长，同批 Decode 请求的出字速度明显变慢，表现为 TPOT 抖动。
- 缓解技术是 **Chunked Prefill**：把长 Prefill 切成小块分散到多步，避免它一次性霸占整个 step（详见 2.4 节）。
