...
**1. 能默写混合精度 + Adam 下模型状态 $16\Psi$ 的三部分构成，并说出优化器状态为什么是切分的第一目标**

<details>
<summary>参考答案</summary>

FP16 参数 $2\Psi$ + FP16 梯度 $2\Psi$ + FP32 优化器状态 $12\Psi$（主参数、一阶动量、二阶动量各 $4\Psi$，即 $K\Psi$、Adam 时 $K=12$）$= 16\Psi$。优化器状态占整份账单的 75%，而参数本体只占 12.5%——最大头又天然逐参数独立、最容易切，所以 ZeRO-1 先切它，收益立竿见影（简算另见 Q2）。（对应正文第 1 节）

</details>

**2. 能用一句话概括 ZeRO 的"切分-聚合"范式，并解释三类状态为什么按"优化器状态 → 梯度 → 参数"的顺序递进**

<details>
<summary>参考答案</summary>

范式：每类状态沿参数维度均匀切成 $N$ 份，rank $k$ 只持久保存第 $k$ 份；需要完整数据时通过集合通信临时重建，用完立即释放。递进顺序由"聚合难度"决定：优化器状态从不需要完整数据（每个参数的更新只依赖自己的动量）；梯度只需保证负责第 $k$ 片参数的 rank 拿到第 $k$ 片聚合梯度；参数每次前向和反向都需要完整数据，切了就必须反复聚合，代价最高。（对应正文第 2.2 节）

</details>

**3. 能写出 AllReduce 的等价拆解式，并说出 Ring 实现下两个原语各自的单卡通信量**

<details>
<summary>参考答案</summary>

AllReduce = ReduceScatter + AllGather。Ring 实现下两者的单卡通信量都是 $(N-1) \times \frac{\Psi}{N} \approx \Psi$，故 AllReduce $\approx 2\Psi$。这是全部通信分析的钥匙：ZeRO-2 把 AllReduce 拆成两半分别用在梯度和参数上（总量不变），ZeRO-3 再多付一次前向参数 AllGather。（对应正文第 3 节）

</details>

**4. 能说清"ZeRO-1 通信量与 DDP 相同"成立的前提，以及论文口径与部分实现口径差出的那 $\Psi$ 从哪来**

<details>
<summary>参考答案</summary>

前提是梯度同步用 ReduceScatter：rank $k$ 只需第 $k$ 片聚合梯度，ReduceScatter（$\Psi$）+ 更新后参数 AllGather（$\Psi$）$= 2\Psi$，与 DDP 持平。部分早期实现图省事仍做完整 AllReduce（$2\Psi$），加上参数 AllGather 约 $3\Psi$——差距的根源是 AllReduce 让每卡都拿到完整梯度，而 ZeRO-1 每卡其实只用得上 $\frac{1}{N}$（另见 Q4）。（对应正文第 4.2 节）

</details>

**5. 能推导 ZeRO-2 为什么被称为"几乎免费的午餐"**

<details>
<summary>参考答案</summary>

显存降到 $2\Psi + \frac{14\Psi}{N}$，通信却与 DDP 持平：反向按 bucket 做梯度 ReduceScatter（$\Psi$，与反向计算重叠）+ 更新后参数 AllGather（$\Psi$）$= 2\Psi$。本质是 DDP 的 AllReduce 被拆成两半用——ReduceScatter 那一半仍作用在梯度上，AllGather 那一半从梯度聚合的收尾挪到了参数更新的收尾；各卡不再拥有完整梯度，但训练本就不需要。所以只要参数 $2\Psi$ 单卡装得下，就没有理由停在 ZeRO-1。（对应正文第 5 节）

</details>

**6. 能拆出 ZeRO-3 通信量 $3\Psi$ 的三笔构成，并解释 ZeRO-2 那次"更新后参数 AllGather"为什么消失了**

<details>
<summary>参考答案</summary>

$3\Psi$ = 前向参数 AllGather（$\Psi$）+ 反向参数 AllGather（$\Psi$，前向后已释放、必须重新凑）+ 反向梯度 ReduceScatter（$\Psi$），比 DDP/ZeRO-2 多 50%。更新后不再需要 AllGather，因为参数本来就以分片形式存储，各 rank 更新完自己的分片即可；下一步前向的逐层 AllGather 天然承担了"把新参数发给所有人"的职责。每卡持久显存 $\frac{16\Psi}{N}$，实现理想线性缩放。（对应正文第 6 节）

</details>

**7. 能说明 ZeRO-3 的 Prefetch 如何把 AllGather 藏进计算，以及三个 `stage3_*` 旋钮各控制什么**

<details>
<summary>参考答案</summary>

首次迭代记录模块实际执行顺序（trace），之后每步在计算第 $i$ 层的同时，在独立通信流上异步发起第 $i+1$、$i+2$ 层的 AllGather；反向同理，梯度 ReduceScatter 也按 bucket 重叠。三个旋钮：`stage3_prefetch_bucket_size` 控制单次预取参数量上限（越大重叠越充分、峰值显存越高）；`stage3_max_live_parameters` 限制同时保持完整形态的参数总量；`stage3_param_persistence_threshold` 让小参数不切分、常驻各卡，避免为 LayerNorm 等小张量付 latency-bound 的 AllGather 延迟。（对应正文第 7 节）

</details>

**8. 能分别说出 ZeRO++ 三项技术各作用于哪次通信、用什么手段**

<details>
<summary>参考答案</summary>

qwZ：前向参数 AllGather 前把 FP16 权重做 INT8 分块量化再传输，通信减半（$\Psi \to 0.5\Psi$）；hpZ：每台机器内部再维护一份机内二级分片，反向参数 AllGather 只在机内高带宽域完成，跨机流量归零（用少量显存换通信）；qgZ：梯度量化到 INT4 后走基于 All-to-All 的分层归约，归约始终在高精度上做、只在传输段用低精度，一次量化一次反量化、误差不随卡数放大。官方称相比 ZeRO-3 通信量最多可减少约 4 倍。（对应正文第 8 节）

</details>

**9. 能说清 ZeRO-Offload 的 GPU/CPU 分工依据与瓶颈所在，以及 Offload/Infinity 与 ZeRO 阶段的搭配关系**

<details>
<summary>参考答案</summary>

分工是刻意算过的：CPU 保存全部 $12\Psi$ 的 FP32 优化器状态并执行 Adam 更新——$O(\Psi)$ 的逐元素运算、访存密集计算稀疏，放 CPU 不亏；GPU 保留 FP16 参数做前向反向——$O(\Psi \cdot B)$ 的矩阵乘必须留在 GPU。瓶颈在 PCIe：每步往返 $4\Psi$ Bytes（梯度下行 $2\Psi$ + 参数上行 $2\Psi$），而 PCIe 4.0 x16 单向约 32 GB/s，比 HBM 低一到两个数量级，靠 pinned memory + 延迟一步更新做重叠。搭配关系：Offload 通常配 Stage 2，Infinity 必须配 Stage 3（参数可分片才能分块换入换出）。（对应正文第 9 节）

</details>

**10. 能按"装不装得下、通信扛不扛得住"两问走完选型决策**

<details>
<summary>参考答案</summary>

$16\Psi$ + 激活单卡装得下 → DDP / ZeRO-1（通信最省）；装不下但完整参数 $2\Psi$ 装得下 → ZeRO-2 / SHARD_GRAD_OP（通信零额外代价，优先选）；参数也装不下 → ZeRO-3 / FULL_SHARD（多付 50% 通信），跨机低带宽再叠加 ZeRO++ 或 HYBRID_SHARD；只有 1~4 张卡 → ZeRO-Offload，还不够上 Infinity（NVMe）；单层参数都装不下 → 叠加张量并行。核心原则：装得下就别切参数。（对应正文第 12 节）
