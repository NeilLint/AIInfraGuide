---
title: "2.4 同步与原子操作"
description: "正确的同步机制是编写无 Bug 并行程序的基础"
pubDate: 2026-05-13
...
**1. 能正确使用 `__syncthreads()` 并避免死锁陷阱（条件分支、不等循环）**

<details>
<summary>参考答案</summary>

- 语义：Block 内所有线程都到达屏障后才能继续，且屏障前的所有内存写入对 Block 内所有线程可见；典型用于共享内存"写入 → 同步 → 读取"两个阶段之间。
- 陷阱一（条件分支）：把 `__syncthreads()` 放在 `if (threadIdx.x < 128)` 这类分支内，部分线程永远到不了屏障 → 死锁或未定义行为；同步必须放在所有线程都会执行的位置。
- 陷阱二（不等循环）：循环内使用时各线程的循环次数必须相同；正确做法是统一循环到最大次数，循环体内用 `if (i < array_len[threadIdx.x])` 屏蔽。
- 代价意识：屏障本身开销很低（几个周期），真正代价是等待最慢线程和打断编译器指令重排，应尽量合并、减少同步次数。

（对应正文第 2.1~2.4 节）

</details>

**2. 能解释 `__syncwarp()` 在 Volta+ 架构中为何必要**

<details>
<summary>参考答案</summary>

- Volta 之前 Warp 内 32 线程以 lockstep 方式执行，天然同步（依赖这一点通信属于未定义行为，不推荐）；那时 `__syncwarp()` 通常是空操作。
- Volta 引入独立线程调度后，同一 Warp 内的线程**可能不再步调一致**，因此 Warp 内通过共享内存通信时，写与读之间必须显式插入 `__syncwarp(mask)`（通常传 `0xFFFFFFFF`，也可用更精确的掩码）。
- 例外：Warp Shuffle 和 Warp Vote 自带同步语义，前后不需要额外 `__syncwarp()`。

（对应正文第 3.1~3.3 节）

</details>

**3. 能区分 `__threadfence_block()`、`__threadfence()`、`__threadfence_system()` 的适用范围**

<details>
<summary>参考答案</summary>

- 三者都是 Memory Fence：**不阻塞线程执行**，只保证 Fence 之前的内存写入在 Fence 之后对目标范围内的其他线程可见——这与 `__syncthreads()` 的执行屏障语义不同。
- `__threadfence_block()`：写入对同一 Block 内的线程可见。
- `__threadfence()`：写入对同一 GPU 上所有线程可见，用于跨 Block 通信。
- `__threadfence_system()`：写入对系统内所有设备（包括 CPU、其他 GPU）可见。

（对应正文第 4.1、4.2 节）

</details>

**4. 能使用原子操作 + Memory Fence 实现跨 Block 的全局归约**

<details>
<summary>参考答案</summary>

- 模式：每个 Block 先做局部归约，由 thread 0 把结果写入 `partial[blockIdx.x]`；然后 `__threadfence()` 确保这次写入对其他 Block 全局可见；再 `atomicInc(&block_counter, gridDim.x)` 领取一个序号。
- 拿到 `ticket == gridDim.x - 1` 的 Block 是最后一个完成者，此时可以安全读取所有 `partial` 值做最终归约、写出结果并重置计数器。
- 关键点：`__threadfence()` 保证 `partial` 的写入在 `atomicInc` 之前全局可见；缺了这道 Fence，最后一个 Block 可能读到其他 Block 的旧值。

（对应正文第 4.3 节）

</details>

**5. 能正确使用 `atomicCAS` 实现自定义原子操作**

<details>
<summary>参考答案</summary>

- 语义：`old = *addr; if (old == compare) *addr = val; return old;`——它是最底层的原子原语，其他所有原子操作都可以用它实现。
- 标准写法是重试循环：`do { assumed = old; old = atomicCAS(addr, assumed, 计算出的新值); } while (assumed != old);`——CAS 返回值与 assumed 不等说明其他线程改了值，需要重试。
- 典型应用：旧架构不直接支持 double 的 `atomicAdd`，可把 double 位转换为 unsigned long long（`__double_as_longlong` / `__longlong_as_double`），用 CAS 循环实现。
- 注意：高竞争下 CAS 循环可能重试多次，性能急剧下降，这正是要尽量减少原子冲突的根本原因。

（对应正文第 5.4 节）

</details>

**6. 能将"所有线程 atomicAdd 到同一地址"优化为分层归约方案**

<details>
<summary>参考答案</summary>

- 三层聚合：① Warp 内先用 `__shfl_down_sync` 归约，完全不需要原子操作；② 每个 Warp 的 lane 0（`threadIdx.x % 32 == 0`）对共享内存的 `block_sum` 做 `atomicAdd`，每 Block 只有 blockDim.x/32 个线程竞争；③ `__syncthreads()` 后由每个 Block 的 thread 0 对全局结果做一次 `atomicAdd`，全局只有 gridDim.x 个线程竞争。
- 效果：冲突从"N 个线程竞争 1 个地址"降到"gridDim.x 个线程竞争"，且共享内存原子远快于全局内存原子。

（对应正文第 6.1 节）

</details>

**7. 能使用共享内存原子操作实现高效直方图**

<details>
<summary>参考答案</summary>

- 朴素版每个线程直接对全局 `hist` 做 `atomicAdd`：全局内存原子延迟约 400~600 cycles，且大量线程竞争同一批 bin，很慢。
- 优化：每个 Block 在共享内存维护 `local_hist`（先初始化并 `__syncthreads()`），线程先对共享内存做原子加（约 20~100 cycles，在 SM 内部完成）；最后每个 bin 只做一次全局 `atomicAdd` 合并回去，全局原子操作次数从 N 降到 numBins × Block 数。
- bin 数很少时可进一步私有化（Privatization）：每线程用寄存器数组计数（纯寄存器操作、零冲突），最终统一合并。
...
- 原子操作是不可分割的读-改-写：一个线程的原子操作完成前，其他线程对同一地址的访问被排队等候，代价取决于**冲突程度**。
- 所有线程操作不同地址 → 接近非原子操作的速度；同一 Warp 内多线程竞争同一地址 → 串行化，最差 32x 慢；跨 Block 大量线程竞争同一地址 → 极慢，可能成为严重瓶颈。
- 存储位置也影响延迟：全局内存原子约 400~600 cycles（要走到 L2 或 DRAM），共享内存原子约 20~100 cycles。
- 对应的优化手段：分层归约、共享内存原子、私有化、Warp 聚合原子（Warp 内先归约再由 leader 做一次原子操作）。

（对应正文第 5.1、5.3、6 节）

</details>

**9. 能使用 Cooperative Groups 实现灵活粒度的同步**

<details>
<summary>参考答案</summary>

- Cooperative Groups（CUDA 9+）把固定粒度的 `__syncthreads()`/`__syncwarp()` 泛化为任意线程组：`cg::this_thread_block()`（当前 Block）、`cg::coalesced_threads()`（当前活跃线程）、`cg::tiled_partition<N>(block)`（固定大小 Tile，N 必须是 2 的幂且 ≤ 32）。
- 在任意组上调用 `.sync()`：`block.sync()` 等价于 `__syncthreads()`，`tile16.sync()` 只同步 16 线程的 Tile。
- Tile 还提供 `shfl_down` 等成员函数，可写出与组大小解耦的归约代码（循环从 `warp.size() / 2` 开始减半）。

（对应正文第 7.1~7.3 节）

</details>

**10. 能判断 Grid Sync 的适用条件和 Block 数量限制**

<details>
<summary>参考答案</summary>

- `cg::this_grid().sync()` 提供跨所有 Block 的 Grid 级同步，适合"全 Grid 完成第一阶段后才能进入第二阶段"的场景（如先全体写入、再读取其他 Block 的结果）。
- 前提一：必须用 `cudaLaunchCooperativeKernel` 启动 Kernel。
- 前提二：所有 Block 必须能**同时驻留**在 GPU 上（受 Occupancy 允许），否则未驻留的 Block 永远到不了屏障 → 死锁；因此 Grid 的大小有上限。
