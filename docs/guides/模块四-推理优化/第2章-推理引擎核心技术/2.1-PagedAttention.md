...
**1. 能说清连续分配下内部碎片和外部碎片各自的成因**

<details>
<summary>参考答案</summary>

- **内部碎片**：传统方式按"这个请求可能生成的最大长度"预留一整块连续显存。比如预留 2048 个 Token 的空间，实际只生成 100 个就遇到 EOS，剩下 1948 个 Token 的空间被该请求"锁着"却没用上，其它请求也进不来。
- **外部碎片**：不同请求预留的大块之间留下大小不一的空隙，加起来可能不小，但每一块都不够放下一个完整请求，等于全废。
- 碎片的根源在于**连续 + 预留**两个约束叠加——只要打破"必须连续"和"必须按最大长度预留"，碎片问题就迎刃而解，这正是 PagedAttention 的切入点。

（对应正文第 1 节）

</details>

**2. 能用"逻辑块 → 物理块"的映射解释 Block Table 的作用**

<details>
<summary>参考答案</summary>

- KV Cache 被切成固定大小的 KV Block（每块存 `block_size` 个 Token 的 Key/Value，vLLM 常见取值 16）。
- 一个请求的 KV Cache 在逻辑上是连续的"逻辑块"序列，物理上却可以散落在显存任意位置（如逻辑块 0/1/2 分别映射到物理块 #7、#2、#5，物理编号完全不必连续）。
- Block Table（块表）就是记录"该请求的第几个逻辑块对应哪个物理块"的映射表；Attention Kernel 计算时查块表把逻辑位置翻译成物理地址。
- 有了这层映射，显存不再要求连续，也不再需要提前按最大长度预留——这就是"KV Cache 的虚拟内存分页"。

（对应正文第 2、3 节）

</details>

**3. 能画出一个请求 Prefill 后的块表，并说明 Decode 时块如何按需增长**

<details>
<summary>参考答案</summary>

- 以 `block_size = 4`、Prefill 9 个 Token 为例：需要 $\lceil 9/4 \rceil = 3$ 个逻辑块，块表形如——逻辑块 0 → 物理块 #7（已满 4/4）、逻辑块 1 → #2（已满 4/4）、逻辑块 2 → #5（部分填充 1/4）。
- Decode 阶段每生成一个新 Token，就往当前逻辑块的空位里填：逻辑块 2 还剩 3 格，能再接 3 个 Token。
- 填满后（生成到第 12 个 Token）才**按需**再申请一个新物理块挂到逻辑块 3——用多少申请多少，绝不提前预留。

（对应正文第 3 节）

</details>

**4. 能解释 Attention Kernel 如何通过物理块号与块内偏移读取分散的 KV**

<details>
<summary>参考答案</summary>

- 传统 Attention 假设 K、V 连续排列，用基地址加偏移顺序扫过去；PagedAttention 打破连续性后，Kernel 必须先查块表，把"逻辑位置"翻译成"物理地址"。
- 核心是两个量：`physical_block_number`（物理块号，乘以块的跨度得到块基址）和 `physical_block_offset`（块内偏移，定位 Token 在块内第几格），再叠加 KV 头的偏移即可算出该 Token 的 Key 起始地址。
- 一个 Warp 在外层循环里逐块推进，每次迭代通过块号跳到下一个物理块——物理上东一块西一块的存储，被 Kernel 当成一段逻辑连续的上下文来访问。
- 间址查表的代价就藏在每次迭代的地址重算里，这是 PagedAttention 的主要开销来源之一。

（对应正文第 5 节）

</details>

**5. 能解释 PagedAttention 为什么能同时消除内部和外部碎片**

<details>
<summary>参考答案</summary>

- **消灭内部碎片**：不再按最大长度预留，每次只分配一个块、填满再要下一个。浪费被限制在"最后一个块内没填满的部分"，最多不到一个块（`block_size=16` 时最多浪费 15 个 Token 的空间），相比旧方式动辄浪费上千 Token 几乎可以忽略。
- **消灭外部碎片**：所有物理块**大小完全相同**，任何一个空闲块都能被任何请求使用，不存在"空间够但形状不对"的问题。
- 显存利用率因此接近最优，同一张卡能塞下更多并发请求，进而转化为吞吐提升。

（对应正文第 4 节）

</details>

**6. 能说明内存共享与 Copy-on-Write 如何支持前缀复用与并行采样**

<details>
<summary>参考答案</summary>

- KV 以块为单位、通过块表间接引用，所以多个请求的块表可以指向**同一个物理块**。典型场景：一批请求共享同一 System Prompt，或从同一 Prompt 并行采样多个回答（`n > 1`）——前缀 KV 完全相同，只存一份即可。
- 只读共享时靠**引用计数（ref_cnt）**记录有几个请求在用。
- 当某个请求要往被共享的块里写新内容（生成分叉）时，触发**写时复制（Copy-on-Write）**：先把该块复制成私有副本、在副本上写、再更新自己的块表指向副本，其他请求不受影响。
- 这套机制与 Linux `fork()` 后父子进程共享内存页如出一辙，也是 Prefix Cache（2.3 节）的物理基础。

（对应正文第 6 节）

</details>

**7. 能区分抢占恢复的两种方式（换出 vs 重计算）及各自的代价**

<details>
<summary>参考答案</summary>

- 触发时机：某一步 Decode 要为请求追加新块但空闲物理块不够，调度器挑选部分请求"踢出"释放物理块。被踢出请求的 KV 有两种处理方式：
- **换出（Swapping）**：把 KV Cache 从 GPU 显存拷到 CPU 内存暂存，恢复时再拷回。优点是不用重算、省算力；代价是占 CPU 内存，来回拷贝有 PCIe 传输开销。
- **重计算（Recomputation）**：直接丢弃 KV，恢复时把已生成的 Token 当成新 Prompt 重新 Prefill 一遍。优点是不占 CPU 内存、实现简单；代价是要重跑一次 Prefill、浪费算力。
- 抢占是保证不 OOM、请求不失败的兜底机制；日志里频繁出现 preemption 告警说明并发压得太满，是"系统在硬扛"的信号，不是常态。

（对应正文第 7 节）

</details>

**8. 能说出 `gpu_memory_utilization`、`block_size`、`enable_prefix_caching` 分别影响什么**

<details>
<summary>参考答案</summary>

- **`gpu_memory_utilization`**（默认约 0.9）：KV Cache 能用多少显存的总闸门——模型权重之外，剩余显存按这个比例划给 KV Cache 块池。调高能容纳更多并发、更少触发抢占，但留给激活值等的余量变小，太激进可能 OOM。
- **`block_size`**：分页的基本粒度（每块存多少 Token 的 KV）。调它要在"块表长度/管理开销"和"尾块内部碎片"之间权衡，绝大多数场景不需要手动改。
- **`enable_prefix_caching`**：开启后，共享前缀（如统一 System Prompt）的请求直接复用已缓存的物理块，省掉重复 Prefill。
- 日常真正常动的旋钮是 `gpu_memory_utilization`（压榨显存）和 `enable_prefix_caching`（有共享前缀就开）。

（对应正文第 8 节）

</details>

**9. 能说清 vLLM V1 中 `KVCacheManager` 与 `BlockPool` 的职责分工**

<details>
<summary>参考答案</summary>

- **`KVCacheManager`** 是**请求视角**的管理者，调度器每步通过它为请求申请、释放块：`get_computed_blocks` 查询前缀缓存命中；`allocate_slots` 为新增 Token 按需申请块，空闲块不够时返回 `None` 交给调度器触发抢占；`free` 在请求结束时**逆序**释放块（尾块先回收，因为头部块更可能被前缀缓存复用）。
- **`BlockPool`** 是**全局视角**的物理块仓库，不关心哪个请求，只管"哪些块空着、哪些块被缓存着"，通过 `get_new_blocks` / `free_blocks` / `touch` 服务上层。
- 读源码的主线：`KVCacheManager` 管请求、`BlockPool` 管物理块、`ref_cnt` 管共享、`FreeKVCacheBlockQueue` 管淘汰。

（对应正文第 9.1 节）

</details>

**10. 能解释空闲块为什么用双向链表而不是普通队列**

<details>
<summary>参考答案</summary>

- 空闲块要同时满足两种操作：分配时从头部快速取走 N 个（`popleft_n`）；前缀缓存命中时，把一个"空闲但仍缓存着有效内容"的块从链表**中间任意位置**摘出来重新激活（`remove`）。
- 普通队列没法 $O(1)$ 地从中间删除，双向链表可以——这就是 `FreeKVCacheBlockQueue` 用双向链表的原因。
- 背后的关键设计是"释放不等于清空"：`ref_cnt` 归零的块只是回到空闲链表，内容和哈希还在；后续请求前缀哈希命中时，`touch()` 直接把它从链表中"抢救"回来复用，这是 Automatic Prefix Caching 省掉重复 Prefill 的底层机制。

（对应正文第 9.2、9.3 节）

</details>

**11. 能分析 `block_size` 取值过大或过小分别带来什么问题**

<details>
<summary>参考答案</summary>

- **块太小**：块表变长、管理开销上升（要维护更多映射项，Kernel 查表次数也更多）。
- **块太大**：退化出内部碎片——最后一块没填满的浪费变多。
- vLLM 选择 16 这类中等值作为常见默认，就是在管理开销和碎片之间取平衡；默认值已在多数模型上调优过，一般无需手动修改。

（对应正文第 10 节，兼见第 8 节）

</details>

**12. 能把"省显存"和"提吞吐"通过 Memory Bound 的结论串联起来**

<details>
<summary>参考答案</summary>

- PagedAttention 把显存利用率推到接近最优，直接意味着**同一张卡能同时塞下更多并发请求**。
- 而 Decode 是 Memory Bound：瓶颈在搬权重而非计算，把更多请求拼进一个 Batch，一次权重搬运能服务更多 Token，系统吞吐显著提升。
- 所以链条是：分页消灭碎片 → 显存省下来 → 并发（Batch）变大 → Memory Bound 的 Decode 吞吐提升——PagedAttention 省下的显存最终转化成了实打实的吞吐。
