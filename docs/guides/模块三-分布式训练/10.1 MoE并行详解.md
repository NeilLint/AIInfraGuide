...
**1. 能写出 MoE 层的路由公式并说明稀疏激活解耦了什么**

<details>
<summary>参考答案</summary>

$p = \mathrm{softmax}(W_g \cdot x)$，$y = \sum_{i \in \mathrm{TopK}(p)} g_i \cdot \mathrm{Expert}_i(x)$，其中 $W_g$ 是 $h \times E$ 的小矩阵、$g_i$ 是归一化门控权重。稀疏激活把"参数量 ∝ 计算量"的绑定打破：参数量按专家数近似放大 $E$ 倍，单 token 计算量只放大约 $K$ 倍——DeepSeek-V3 因此能做到 671B 总参、37B 激活。注意路由是**逐 token** 的，同一序列相邻 token 完全可能去不同专家。（对应正文第 1.1 节；另见 Q2、Q8）

</details>

**2. 能说明共享专家与细粒度专家切分各解决什么问题、带来什么系统代价**

<details>
<summary>参考答案</summary>

共享专家对所有 token 恒定激活、不经路由，集中吸收语法等"公共知识"，让路由专家专注差异化知识；细粒度切分把每个专家的中间维切小为 $\frac{1}{m}$、专家数变 $mE$、Top-$K$ 提为 Top-$mK$，总计算量不变但组合数从 $\binom{8}{2}=28$ 暴涨到 $\binom{32}{8}\approx 10^7$ 量级。系统代价：单个专家的 GEMM 变得很小，专家内部再切 TP 不划算，每卡多专家要靠 GroupedGEMM 保持算力利用率。（对应正文第 1.2 节）

</details>

**3. 能从切分对象与通信性质两方面区分 EP 和 TP**

<details>
<summary>参考答案</summary>

TP 把**单个**权重矩阵切成 $t$ 片，每卡为所有 token 算 $\frac{1}{t}$ 份计算，用 AllReduce 合并部分和，通信量只看形状、与数据无关；EP 把专家**集合**按整份分配（专家本身不切），每卡只对被路由到本卡专家的 token 做完整计算，用 All-to-All 收发 token 的隐层向量本身，通信量是**数据依赖**的——发给谁由 Router 现场决定，每步每层都不同，还可能严重倾斜。（对应正文第 2 节）

</details>

**4. 能推导一次 dispatch 的通信量，并数出一个 MoE 层每 step 的 All-to-All 次数**

<details>
<summary>参考答案</summary>

路由均匀假设下 $V_{\text{dispatch}} = T \cdot K \cdot h \cdot b_{\text{dtype}} \cdot \frac{N_{EP}-1}{N_{EP}} \approx T K h \, b_{\text{dtype}}$，combine 与之对称同量级；代入 DeepSeek-V3 量级（$T=4096$、$K=8$、$h=7168$、BF16）单卡单层单方向约 470 MB。每个 MoE 层每 step 共 **4 次** All-to-All：前向 dispatch + combine，反向再来一对。它在关键路径上、数据依赖、常要跨节点走 IB（约 50 GB/s，仅为节点内 NVLink 约 160 GB/s 的三分之一），是 MoE 训练的头号瓶颈。（对应正文第 3 节）

</details>

**5. 能讲清路由塌缩的正反馈链条，以及为什么 loss 正常也可能已经退化**

<details>
<summary>参考答案</summary>

链条是"强者愈强"：初始随机差异 → 稍强的专家被更多 token 选中 → 得到更多梯度变得更强 → 被选得更多，最终少数专家承包绝大多数 token。模型侧 MoE 退化成"昂贵的小稠密模型"，其 loss 仍在正常下降，容量浪费从 loss 曲线上看不出来；系统侧路由倾斜即卡间负载倾斜，热点卡计算、通信、显存三重过载，而 All-to-All 是同步点、按最慢的卡计时。所以必须额外监控每层专家负载分布、路由熵、token drop 率。（对应正文第 4.1 节；另见 Q1、Q3）

</details>

**6. 能对比辅助损失与 DeepSeek-V3 aux-loss-free 两条负载均衡路线**

<details>
<summary>参考答案</summary>

辅助损失（GShard/Switch）：$\mathcal{L}_{\text{aux}} = \alpha \cdot E \cdot \sum_i f_i P_i$，$f_i$ 是硬统计（不可导）、$P_i$ 是软概率（梯度从这里流回 Router），Switch 取 $\alpha = 10^{-2}$；矛盾在于 $\alpha$ 太大伤模型质量、太小压不住倾斜。V3 把均衡挪到**非梯度通道**：偏置 $b_i$ 只参与 Top-K 选择、不进门控权重，每 step 按过载/欠载 $\mp\gamma$（$\gamma = 0.001$）调节，像反馈控制器；再加 $10^{-4}$ 量级的序列级均衡损失兜底，且训练推理都不丢 token。（对应正文第 4.2–4.4 节；另见 Q3）

</details>

**7. 能写出专家容量公式并说明容量因子的两难与 dropless 演进**

<details>
<summary>参考答案</summary>

$C = \text{CF} \times \frac{T \cdot K}{E}$，即在完全均匀所需容量上乘容量因子留余量（GShard 对 Top-2 取 2.0，Switch 实验 1.0～1.25）。CF 小则 drop 率高——超容 token 不经专家计算、只靠残差直通，伤质量；CF 大则 padding 浪费翻倍。drop 恰恰集中发生在热点专家门口，所以 drop 率本身就是负载均衡的监控指标。演进方向是 dropless：MegaBlocks 用块稀疏 GEMM 支持变长 token 数，DeepSeek-V3 靠均衡策略做到训练推理都不丢 token。（对应正文第 5 节）

</details>

**8. 能解释 MoE 模型里为什么存在两套 DP 通信组、两套梯度同步**

<details>
<summary>参考答案</summary>

非专家参数（注意力、Embedding 等）在全部 DP 副本间完全冗余，梯度走常规 AllReduce/ReduceScatter；专家参数每个只存在于 $\frac{N_{DP}}{N_{EP}}$ 个副本上，梯度只在"持有同一专家的卡"组成的**专家 DP 组**内同步——两套通信组独立构建、各走各的。另外专家内部是否再切 TP 看专家大小：Mixtral 那种中间维 14336 的大专家值得 ETP，DeepSeekMoE 中间维 2048 的细粒度小专家通常纯 EP 不切。（对应正文第 6 节）

</details>

**9. 能按"少发、快发、边发边算"三条线概括 All-to-All 优化**

<details>
<summary>参考答案</summary>

少发：node-limited routing 限定每个 token 最多发往 4 个节点（先选节点再选节点内专家），给跨 IB 流量确定上界；快发：DeepEP 两级转发——跨节点 RDMA 只发一份到目标节点的中转 GPU、节点内 NVLink 再分发，配高吞吐（训练/prefill）与纯 RDMA 低延迟（decode）两套 kernel，dispatch 方向用 FP8 让字节数减半；边发边算：单层内专家分块微流水，跨 micro-batch 用 DualPipe 把 All-to-All 藏进另一 micro-batch 的计算，训练只划约 20 个 SM 做通信。（对应正文第 7 节；另见 Q6）

</details>

**10. 能说明推理为什么要用大 EP，以及 prefill/decode 的 EP 策略差异**

<details>
<summary>参考答案</summary>

EP 越大每卡专家参数越少，省下的 HBM 直接变成 KV Cache 容量、能组更大 batch；同时全局大 batch 让每个专家聚到足够 token，GEMM 才打得满。推理路由随流量漂移，靠 EPLB 做**运行时**均衡：热点专家复制冗余副本摊流量、放置时同组专家收拢在节点内、周期性重排（训练则靠损失/偏置这类**学习时**均衡）。策略分化：prefill 吞吐导向，EP32 + 高吞吐 kernel；decode 延迟导向，EP320 + 纯 RDMA 低延迟 kernel，64 卡专门承载冗余与共享专家。推理无反向，每层只有 2 次 All-to-All。（对应正文第 8 节；另见 Q5、Q7）
