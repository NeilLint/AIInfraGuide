...
**1. 看到 `(B, S, H) @ (H, V)` 能立即写出输出 shape**

<details>
<summary>参考答案</summary>

- 先检查形状是否匹配：内维 $H$ 相同，运算合法。把前两维展平为 $(BS) \times H$，与 $H \times V$ 相乘得到 $(BS) \times V$，再 reshape 回 $(B, S, V)$。
- 代价：每个输出元素做 $H$ 次乘加，总计约 $2BSHV$ FLOPs（一次乘法和一次加法各计一个浮点操作）。
- 实现风险：权重和输出 logits 都很大，需要考虑数据类型、内存布局、分块、并行策略和 Softmax 的数值稳定性。（详见第 1.2 节）

</details>

**2. 能区分逐元素乘、点积、矩阵乘和 batched matmul**

<details>
<summary>参考答案</summary>

- 逐元素乘（$\odot$）：对应元素相乘，要求 shape 相同或可广播，输出 shape 不变。
- 点积：两个 $n$ 维向量的 $\sum_i x_i y_i$，输出标量，可理解为加权求和或方向相似性（$\lVert\mathbf{x}\rVert\lVert\mathbf{y}\rVert\cos\theta$）。
- 矩阵乘：`(M, K) @ (K, N) -> (M, N)`，每个输出元素是一行与一列的点积，中间维 $K$ 被归约。
- batched matmul：只对最后两维做矩阵乘，前面的维度（如 $B, N_h$）负责批处理和广播，例如 Attention 中的 $\mathbf{Q}\mathbf{K}^\top$。（详见第 2.3、2.4、2.6、2.7 节）

</details>

**3. 能根据 shape/stride 判断转置视图是否连续**

<details>
<summary>参考答案</summary>

- 行主序 $(2,3)$ 矩阵底层连续存储，stride 为 $(3,1)$，偏移是 $3i+j$；转置视图 shape 变为 $(3,2)$、stride 变为 $(1,3)$，通常无需复制数据。
- 判断连续性：连续布局要求最后一维 stride 为 1、且各维 stride 是后续维度大小的乘积；转置后按最后一维扫描时 stride 是 3 而不是 1，访问不再连续。
- `reshape` 是否复制取决于原 layout 能否用新 shape/stride 表示；看到 `view`、`transpose`、`contiguous` 时要同时思考逻辑维度和物理布局。（详见第 2.2 节）

</details>

**4. 能解释范数、秩、特征值和奇异值的直觉**

<details>
<summary>参考答案</summary>

- 范数衡量向量或矩阵的规模（L1、L2、L∞、Frobenius），工程上用于梯度裁剪、误差比较、正则化和归一化。
- 秩是矩阵中独立方向的数量，秩低意味着信息冗余，可用两个更小的矩阵近似（$\mathbf{A}\approx\mathbf{U}\mathbf{V}$）。
- 特征向量是矩阵变换下只缩放不改变方向的方向，特征值是缩放倍数；它帮助理解梯度经多层 Jacobian 连乘为何爆炸或消失，以及 PCA 的主方向。
- 奇异值来自 SVD（$\mathbf{A}=\mathbf{U}\mathbf{\Sigma}\mathbf{V}^\top$），保留前 $r$ 个最大奇异值得到最优 rank-$r$ 近似，小奇异值对应影响较弱、可舍弃的方向。（详见第 2.5、2.10–2.12 节）

</details>

**5. 能计算低秩分解和 LoRA 的参数量节省比例**

<details>
<summary>参考答案</summary>

- 低秩分解把参数量从 $MN$ 降到 $r(M+N)$，$r \ll \min(M,N)$。
- LoRA 冻结 $\mathbf{W}_0$，只训练 $\Delta\mathbf{W}=\frac{\alpha}{r}\mathbf{B}\mathbf{A}$，新增参数量为 $r(d_{in}+d_{out})$；例如 $d_{in}=d_{out}=4096$、$r=16$ 时，$16 \times 8192 / 4096^2 \approx 0.78\%$。
- 注意低秩只减少可训练参数和相应优化器状态，不自动消除基座权重、激活和前向计算成本。（详见第 2.10、2.13 节）

</details>

**6. 能从 block 矩阵乘法解释 GEMM tiling 的正确性**

<details>
<summary>参考答案</summary>

- 把矩阵沿 $M$、$N$ 和归约维 $K$ 切块后，$\mathbf{C}_{11}=\mathbf{A}_{11}\mathbf{B}_{11}+\mathbf{A}_{12}\mathbf{B}_{21}$：每个输出块沿 $K$ 方向累加若干局部乘积，数学上与整体相乘完全等价。
- 因此 GPU Kernel 可以取一小块 $\mathbf{A}$、$\mathbf{B}$ 搬到片上共享内存/寄存器，重复使用计算多个输出元素，沿 $K$ 迭代累加，最后写回 HBM——tiling 只改变计算顺序和数据搬运，不改变目标公式。
- 但浮点加法不严格满足结合律，不同分块的累加顺序可能导致结果不逐位一致，验证时要用适当的误差容限。（详见第 3.1、3.6 节）

</details>

**7. 能估算 GEMM 的 FLOPs、最小访存量和算术强度**

<details>
<summary>参考答案</summary>

- $(M,K)@(K,N)$：输出元素 $MN$ 个、每个做 $K$ 次乘加，FLOPs 约 $2MKN$。
- 理想情况下（FP16，每个输入矩阵只从 HBM 读一次、输出写一次），访存字节数约 $2(MK+KN+MN)$。
- 算术强度 $AI \approx \dfrac{2MKN}{2(MK+KN+MN)}$（FLOPs 除以传输字节数）。它不是精确性能模型，但能帮助判断算子更可能受算力还是带宽限制。（详见第 3.2、3.3 节）
...
- 期望：$\mathbb{E}[X]=\sum_x xP(X=x)$（连续变量用积分）；期望具有线性性质 $\mathbb{E}[aX+bY]=a\mathbb{E}[X]+b\mathbb{E}[Y]$，不要求独立。
- 方差：$\operatorname{Var}(X)=\mathbb{E}[(X-\mu)^2]=\mathbb{E}[X^2]-\mu^2$；若 $X,Y$ 独立则 $\operatorname{Var}(X+Y)=\operatorname{Var}(X)+\operatorname{Var}(Y)$。
- 协方差：$\operatorname{Cov}(X,Y)=\mathbb{E}[(X-\mu_X)(Y-\mu_Y)]$，标准化后得到相关系数 $\rho_{X,Y}$。
- 条件概率：$P(X\mid Y)=P(X,Y)/P(Y)$；自回归语言模型用链式法则把序列概率分解为 $\prod_t P(x_t\mid x_1,\ldots,x_{t-1})$。（详见第 4.2、4.5–4.7 节）

</details>

**9. 能写出稳定 Softmax 与 LogSumExp**

<details>
<summary>参考答案</summary>

- 依据是 Softmax 对统一平移不敏感：$\operatorname{softmax}(\mathbf{z}+c)=\operatorname{softmax}(\mathbf{z})$。
- 稳定 Softmax：令 $m=\max_j z_j$，则 $p_i = e^{z_i-m}/\sum_j e^{z_j-m}$，最大的指数输入为 0，防止正向溢出。
- 稳定 LogSumExp：$\operatorname{LSE}(\mathbf{z}) = m + \log\sum_j e^{z_j-m}$，于是 $\log\operatorname{softmax}(\mathbf{z})_i = z_i - \operatorname{LSE}(\mathbf{z})$。
- 若后续还要取对数，应直接用稳定的 `log_softmax`，避免小项先下溢为 0 再出现 `log(0)`。（详见第 5.1–5.3 节）

</details>

**10. 能说明交叉熵、熵和 KL 散度之间的关系**

<details>
<summary>参考答案</summary>

- 交叉熵 $H(\mathbf{q},\mathbf{p})=-\sum_i q_i\log p_i$；熵 $H(\mathbf{p})=-\sum_i p_i\log p_i$ 衡量分布自身的不确定性（均匀分布时最大，为 $\log V$）。
- 分解关系：$H(\mathbf{q},\mathbf{p}) = H(\mathbf{q}) + D_{KL}(\mathbf{q}\lVert\mathbf{p})$。训练时真实分布 $\mathbf{q}$ 固定，$H(\mathbf{q})$ 与参数无关，所以最小化交叉熵等价于最小化相应 KL 散度。
- KL 散度满足 $D_{KL}\ge 0$、两分布相同时为 0，但通常不对称，不是严格意义的距离。（详见第 5.5–5.7 节）
...
- 以 $a=xy$、$b=a+x$、$L=b^2$ 为例：从 $\partial L/\partial L=1$ 开始反向，$\partial L/\partial b=2b$；因为 $b=a+x$，$\partial L/\partial a=2b$。
- $x$ 有两条路径影响 $L$，梯度需要相加：$\partial L/\partial x = 2b + 2by$，而 $\partial L/\partial y = 2bx$。这解释了一个张量被多处使用时为什么要累积梯度。
- 概念上多层网络的梯度是 Jacobian 连乘，但实际自动微分不显式创建巨大 Jacobian，而是高效计算 vector-Jacobian product（反向模式）。（详见第 6.4、6.5 节）

</details>

**12. 能写出线性层对输入、权重和 bias 的梯度 shape**

<details>
<summary>参考答案</summary>

- 设 $\mathbf{Y}=\mathbf{X}\mathbf{W}+\mathbf{b}$，上游梯度 $\mathbf{G}=\partial L/\partial\mathbf{Y}\in\mathbb{R}^{B\times d_{out}}$，则：
- $\partial L/\partial\mathbf{X} = \mathbf{G}\mathbf{W}^\top$，shape 为 `(B, dout) @ (dout, din) -> (B, din)`；
- $\partial L/\partial\mathbf{W} = \mathbf{X}^\top\mathbf{G}$，shape 为 `(din, B) @ (B, dout) -> (din, dout)`；
- $\partial L/\partial\mathbf{b} = \sum_i \mathbf{G}_{i,:}$，对 batch 维归约得 `(dout,)`。
- 一个线性层的反向通常包含两次 GEMM 和一次归约，这也是训练算力显著高于单次前向的原因之一。（详见第 6.6 节）

</details>

**13. 知道 Softmax + 交叉熵对 logits 的梯度是 `p - y`**

<details>
<summary>参考答案</summary>

- one-hot 标签下损失 $L=-\log p_y$，与 Softmax 联合求导得 $\partial L/\partial z_i = p_i - y_i$：对正确类别梯度是 $p_y-1$，对其他类别是 $p_i$，预测越错，推动 logits 修正的力度越大。
- 这是深度学习最重要的简化之一；融合交叉熵 Kernel 可以共享最大值和归约结果、减少中间概率张量，并直接生成所需梯度。（详见第 6.8 节）
...
- 深层梯度包含许多 Jacobian 连乘，尺度长期小于 1 会指数衰减、长期大于 1 会指数增长（梯度消失/爆炸）。
- 残差块 $\mathbf{y}=\mathbf{x}+F(\mathbf{x})$ 的 Jacobian 是 $\mathbf{I}+\partial F/\partial\mathbf{x}$，恒等路径提供了一条不完全依赖 $F$ 的梯度通路。
- 它不保证永远不会梯度爆炸或消失，但显著改善深层网络的优化条件；其他缓解手段还包括合理初始化、归一化、梯度裁剪和合适学习率。（详见第 7.4、7.5 节）
...
- 浮点数位数分给符号、指数和尾数：指数位决定动态范围（可表示多大/多小的数），尾数位决定精度（相邻可表示数的精细程度），二者是两件事。
- overflow：结果绝对值超过最大有限值，常变成 $\pm\infty$；underflow：结果太接近 0，进入 subnormal 或被舍入为 0。
- 例如 FP16 尾数位（10 位）多于 BF16（7 位），同数量级下精度更细，但指数位只有 5 位，最大有限值仅 65504；BF16 有 8 位指数、动态范围接近 FP32，但舍入误差更明显。（详见第 8.1、8.2、8.5 节）

</details>

**16. 能说明 FP16 为什么常需要 Loss Scaling，而 BF16 通常不需要**

<details>
<summary>参考答案</summary>

- FP16 指数位少、动态范围窄，反向传播中的小梯度可能下溢为 0。
- Loss Scaling 把损失乘以缩放因子 $S$（$L'=SL$，则 $\nabla L'=S\nabla L$），先把梯度放大到 FP16 可表示范围，反向完成后再除以 $S$ 还原。
- 动态 Loss Scaling：每步检查梯度是否含 `inf`/`NaN`，溢出则跳过更新并减小 scale，连续稳定若干步后再尝试增大。
- BF16 指数位与 FP32 相同、动态范围大得多，通常不依赖 Loss Scaling，但仍可能因模型本身不稳定产生非有限值。（详见第 8.9 节）

</details>

**17. 能解释并行归约为何可能产生非逐位一致结果**

<details>
<summary>参考答案</summary>

- 浮点加法每步都舍入，不严格满足结合律：$\operatorname{fl}(\operatorname{fl}(a+b)+c) \ne \operatorname{fl}(a+\operatorname{fl}(b+c))$。
- 并行归约使用树形加法，不同的线程数、block 划分和原子操作顺序会改变累加顺序，因此结果可能略有差异；非确定性不一定代表实现错误，但必须在可接受误差内并符合复现性要求。
- 提高归约精度的手段：用更高精度累加、成对/树形求和、Kahan 补偿求和、先局部归约再合并部分和。（详见第 8.4 节）

</details>

**18. 能从 Attention shape 推导 $O(S^2)$ 中间量与显存成本**

<details>
<summary>参考答案</summary>

- $\mathbf{Q},\mathbf{K}\in\mathbb{R}^{B\times N_h\times S\times D_h}$，分数矩阵 $\mathbf{S}=\mathbf{Q}\mathbf{K}^\top/\sqrt{D_h}\in\mathbb{R}^{B\times N_h\times S\times S}$，随序列长度平方增长。
- 显式物化 FP16 的分数矩阵需要 $2BN_hS^2$ 字节：当 $B=1,N_h=32,S=32768$ 时就是 64 GiB，还没算概率矩阵和反向中间量。
- FlashAttention 的关键正是不把完整 $S\times S$ 中间矩阵写回 HBM，而是利用 Online Softmax 的可合并状态 $(m,l)$ 在片上分块完成稳定 Softmax 与 $\mathbf{P}\mathbf{V}$ 累积。（详见第 9.2、9.4 节）
