...
**1. 能用算术强度与 Roofline 平衡点，定量说明 Prefill 为什么 Compute Bound、Decode 为什么 Memory Bound**

<details>
<summary>参考答案</summary>

- 算术强度 $I = \dfrac{\text{FLOPs}}{\text{访存字节数}}$，与 GPU 平衡点 $I^* = \dfrac{\text{峰值算力}}{\text{显存带宽}}$ 比较：H100 SXM 约 989 TFLOPS / 3.35 TB/s，$I^* \approx 295$ FLOPs/Byte，低于此值的计算只能等数据。
- Prefill 的形态是矩阵乘 $[s, h] \times [h, h]$：FLOPs 约 $2sh^2$、访存约 $2h^2$ 字节（权重为主），$I \approx s$——prompt 上千 Token 时 $I \gg I^*$，权重读一次被 $s$ 个 Token 复用，Compute Bound，指标是 TTFT。
- Decode 的形态是向量乘 $[1, h] \times [h, h]$：$I \approx 1$，离平衡点差两个数量级；batch 到 $b$ 也只是 $I \approx b$，要数百并发才逼近平衡点——Memory Bound，指标是 TPOT，唯一正道是攒大 batch 让一次权重搬运服务更多请求。（对应第 1 节）

</details>

**2. 能描述混布下长 Prefill 如何造成 TPOT 尖刺，以及 TTFT 与 TPOT 的跷跷板关系**

<details>
<summary>参考答案</summary>

- Continuous batching 把 Prefill 和 Decode 混进同一迭代：一个几千 Token 的 Prefill 进入 batch，该步迭代时间从十几毫秒被拉长到几百毫秒，batch 里所有正在 Decode 的请求 TPOT 一起膨胀——用户体感"打字打得好好的突然卡一下"，P95/P99 TPOT 严重拖尾（可拖慢至 3~5 倍）。
- 反过来，为保护 TPOT 而限制 Prefill 的调度机会，TTFT 又会劣化——两个指标在混布架构里此消彼长，调度器只能在跷跷板上找折中。
- DistServe 把这类现象归纳为 prefill-decode interference，并指出还有第二重耦合损失：混布下两阶段被迫共享同一套并行策略与扩缩容决策。（对应第 2 节）

</details>

**3. 能说出 chunked prefill "只能缓解不能根治"的三个结构性原因（资源共享、chunk 两难、策略耦合）**

<details>
<summary>参考答案</summary>

1. **资源仍然共享**：chunk 与 Decode 同批执行，依旧抢同一份算力与显存带宽，TPOT 只是从"偶发尖刺"变成"持续温和的变慢"——干扰被平滑，没有被消除。
2. **chunk 大小两难**：chunk 大则单步干扰重；chunk 小则 Prefill 被切成多步、每步重读此前累积的 KV（attention 部分重复访存），Prefill 总效率下降、TTFT 变长，不存在两头都好的取值。
3. **耦合没有解开**：并行策略、GPU 型号、扩缩容仍是 P 和 D 一起定，永远只能取折中值。

一句话总结：chunked prefill 优化的是时间片划分，PD 分离改变的是资源划分；但它仍是单机小规模场景控制 TPOT 尖刺的最实用手段（vLLM 默认开启）。（对应第 2 节；另见面试题 Q7）

</details>

**4. 能定义 Goodput，并解释它与 raw throughput 的差别为什么是 PD 分离叙事的核心**

<details>
<summary>参考答案</summary>

- **Goodput = 单位时间内同时满足 TTFT SLO 和 TPOT SLO 的有效吞吐**（DistServe, OSDI'24 提出）。
- Raw throughput 会说谎：把 GPU 塞满的混布系统 QPS 可以很高，但一半请求 TTFT 超标、另一半 TPOT 超标，这些吞吐对用户无效。
- PD 分离牺牲一点账面利用率（两个池子各有闲时），换 SLO 达成率大幅提升：DistServe 报告在约 90% 请求满足延迟约束下可服务最高 7.4 倍请求量、或承受最高 12.6 倍更严格的 SLO；Splitwise 从成本视角报告相同功耗下最高 2.35 倍吞吐。
- 核心表述：PD 分离的收益不是"让 GPU 更忙"，而是"让每一份忙都花在满足 SLO 上"——设计目标从 utilization 换成 SLO attainment。（对应第 3 节）

</details>

**5. 能算一条 4K prompt 在 70B 模型上的 KV 传输体量，并说出按层流式 + 计算重叠如何把它藏起来**

<details>
<summary>参考答案</summary>

- 体量：Llama-3.1-70B（80 层、8 个 KV 头、head_dim 128、FP16）每 Token KV 为 320KB，4K prompt 即约 1.3GB。
- 裸传输时间：IB NDR 单端口（50 GB/s）约 26ms，NVLink 4（900 GB/s 聚合）约 1.5ms；对照 4K Prefill 本身数百毫秒的计算量，传输是小头，但串行执行就白白加进 TTFT（128K 长文在单端口 IB 下更是拖出近一秒）。
- 藏法：KV Cache 逐层产生，不必等 80 层全算完——第 $i$ 层 KV 一算完就异步发出，与第 $i+1$ 层计算重叠（layer-wise streaming）。Splitwise 报告经按层异步传输优化后，传输延迟基本被 Prefill 计算隐藏，对端到端延迟影响可忽略。
- 除了"藏"还能"减"：KV 量化到 FP8 传输量与 D 端显存同时减半；MLA 类架构的 latent KV 天然小一个量级。（对应第 4 节）

</details>

**6. 能列出 KV 传输的三层抽象代表：vLLM KV Connector、NIXL、Mooncake Transfer Engine**

<details>
<summary>参考答案</summary>

- **vLLM KV Connector**：引擎内建的可插拔连接器抽象，NCCL 系连接器（PyNcclConnector）、NixlConnector、LMCache 等后端可替换，以 kv_producer / kv_consumer 角色配置 P、D 实例。
- **NVIDIA NIXL**（Inference Xfer Library）：统一 NVLink/RDMA/存储等多种介质的点对点传输 API，是 Dynamo 的传输底座，vLLM/SGLang 均可对接。
- **Mooncake Transfer Engine**：专用高性能传输层，RDMA/GPUDirect 多网卡聚合、拓扑感知选路、按层流水，承载 P→D、池→P/D 之间所有 KV 搬运。

三者的共同职责：把介质选择、内存注册、分块、多路并发等传输细节封装成独立组件。（对应第 4、7 节）

</details>

**7. 能用 $x:y \approx T_P : T_D$ 的框架推演 xPyD 配比，并说出输入/输出长度比、SLO、缓存命中率如何拨动它**

<details>
<summary>参考答案</summary>

- 框架：两池各自的"单请求独占服务时间 × QPS"要匹配，即 $x : y \approx T_P : T_D$。示意推演（假设值）：输入 4K、输出 512 Token 的流量，$T_P = 0.4$s；D 端 512 Token × TPOT 30ms ≈ 15s，摊到 128 路并发后 $T_D \approx 0.12$s，故 $x:y \approx 3.3$，从 3P1D 量级起步再压测校准。
- 输入/输出长度比：输入长输出短（摘要、RAG 重排）P 多 D 少；输入短输出长（创作、CoT）P 少 D 多。
- SLO：TTFT 紧则 P 池多配冗余压排队延迟（TTFT 里排队是大头）；TPOT 紧则控制 D 单实例并发（batch 越大 TPOT 越高），等效多配 D。
- 缓存命中率：Prefix Cache 命中只削减 P 池负载，$T_P$ 要按未命中部分的计算量折算，命中率越高配比越向 D 倾斜。
- 流量时变还需动态调整：P/D 用同一份模型权重，节点切角色不需重新加载权重，只切调度身份与并行配置。（对应第 5、8 节）

</details>

**8. 能解释异构 PD"P 用算力卡、D 用带宽卡"的依据与工程前提**

<details>
<summary>参考答案</summary>

- 依据：Prefill 是 Compute Bound，旗舰算力卡（H100/H200 级）物尽其用；Decode 算力大量闲置，值钱的是 HBM 带宽与容量——Splitwise 论证把 Decode 放到上一代或功耗受限机器上，端到端损失有限而集群成本与功耗显著下降。
- 成本框架：$\text{Cost}_{\text{token}} = \dfrac{C_P}{\text{Throughput}_P} + \dfrac{C_D}{\text{Throughput}_D}$，同构部署两项分母都被"用错卡"打折；判断某张便宜卡值不值得进 D 池，只看 HBM 带宽/价格比（Decode 吞吐近似正比于带宽）和显存容量/价格比（决定 KV 池与并发上限），不必看算力。
- 工程前提：两种卡的 KV Cache 内存布局、量化格式、算子实现必须对齐或可转换，传输引擎要处理跨代际互联差异；收益模型还要计入两池各自过剩的部分——异构是放大收益的选项，不是入门配置。（对应第 6 节）

</details>

**9. 能概述 Mooncake 的三个支柱（KVCache 池化、Transfer Engine、以 KVCache 为中心的调度与早拒绝）**

<details>
<summary>参考答案</summary>

1. **KVCache 池化**：把集群闲置的 CPU DRAM 和 SSD 组织成分布式 KVCache 池（Mooncake Store），KV 从"某张卡的私有状态"升格为全局可寻址、可复用的一等资源，前缀缓存天然全局化。
2. **Transfer Engine**：RDMA/GPUDirect 多网卡聚合、拓扑感知选路、按层流水的专用传输层，负责 P/D/池之间所有 KV 搬运。
3. **KVCache 中心调度（Conductor）**：选节点时同时权衡前缀命中收益与负载均衡；过载时做基于预测的早拒绝——预判赶不上 SLO 的请求在入口即拒，把资源留给能达标的请求。

技术报告称长上下文模拟场景吞吐最高提升 525%、真实负载下可多承接约 75% 请求。视角关键在"KVCache-centric 而非 PD-centric"：KV 成为一等资源后，PD 分离只是自然推论。（对应第 7、8 节；另见面试题 Q6）

</details>

**10. 能列举至少三个不该用 PD 分离的场景并说明理由**

<details>
<summary>参考答案</summary>

1. **输入短、输出也短**：Prefill 耗时与一步 Decode 同量级，混布干扰本来就小，chunked prefill 足以兜底；分离新增的传输、调度、跨实例排队开销反而净亏。
2. **小规模部署（2~4 卡）**：劈成两池后各自失去规模效应——D 池 batch 拼不大、P 池闲时无事可做，而混布时这些资源互相填谷。
3. **没有高速互联（纯 PCIe/TCP）**：KV 传输慢到无法被计算重叠覆盖，甚至逼近"D 端重算一遍 Prefill"的耗时。
4. **离线批处理**：没有 TTFT/TPOT SLO 就没有 Goodput 焦虑，混布 + chunked prefill 的账面利用率最大化正是最优解。
5. **流量极度不稳且无力运维动态配比**：xPyD 配错方向时分离劣于混布。

实用判断链：P95 输入是否上千 Token → 是否同时约束 TTFT/TPOT → 每池能否 ≥ 4~8 实例 → 有无 RDMA/NVLink，四问全过才进方案候选。（对应第 9 节）
