...
**1. 能从两个端点条件推出非对称量化的 $s$ 与 $z$，并说出对称量化是它的什么特例、工程上为何"权重用对称"**

<details>
<summary>参考答案</summary>

由 $x_{\min} \mapsto q_{\min}$、$x_{\max} \mapsto q_{\max}$ 联立解出 $s = \frac{x_{\max}-x_{\min}}{q_{\max}-q_{\min}}$、$z = \mathrm{round}(q_{\min} - x_{\min}/s)$；量化 $q = \mathrm{clamp}(\mathrm{round}(x/s)+z)$、反量化 $\hat{x} = s(q-z)$。对称量化强制 $z=0$、范围取 $[-\max|x|, +\max|x|]$，INT8 时 $s = \max|x|/127$。权重分布天然近似零对称，用对称量化省掉含 $z$ 的交叉项、GEMM 内核最干净；明显偏斜的激活才考虑非对称。（对应正文第 1.1 节）

</details>

**2. 能解释量化误差为什么随粒度细化而下降，以及 scale 为什么只能挂在 GEMM 的外侧维度**

<details>
<summary>参考答案</summary>

均匀量化舍入误差上界 $s/2$，而 $s \propto \max|x_{\text{group}}|$——组切得越小，组内动态范围越窄、误差越小，这是 INT4 必配 per-group（常见 $g=128$）的原因。scale 只能挂在 $X$ 的 token 维（per-token）或 $W$ 的输出通道维（per-channel）：挂在内侧归约维时，累加过程中每个乘积项的 scale 都不同、无法从求和号里提出来，硬件 INT8 GEMM 就废了——这也是激活无法 per-channel 量化的根源（另见 Q1）。（对应正文第 1.2 节）

</details>

**3. 能区分量化加速的两条收益路径，并据此判断 W4A16 与 W8A8 各该用在什么场景**

<details>
<summary>参考答案</summary>

路径一：memory-bound 的小 batch Decode，每步时间 $\approx$（权重 + KV Cache 字节数）/ HBM 带宽，W4A16 把权重字节数除以 4，哪怕计算仍用 FP16 做也接近线性提速——反量化花的是过剩的算力，省的是稀缺的带宽。路径二：compute-bound 的大 batch Prefill / 高并发吞吐，必须让计算本身跑在 INT8/FP8 Tensor Core 上（约 FP16 的 2 倍算力），激活也得量化即 W8A8/FP8；此时 W4A16 无算力收益、反量化成净开销（另见 Q4、Q5）。（对应正文第 1.4 节）

</details>

**4. 能描述 Activation Outlier 的系统性特征，并说出它与硬件约束构成的"核心矛盾"**

<details>
<summary>参考答案</summary>

对 6.7B 以上的 LLM：少数固定通道的幅值比其余大 1~2 个数量级、持续出现在相同通道位置、随模型规模加剧，但同一通道内跨 token 方差不大。per-tensor 的 scale 被 outlier 撑大，普通通道只剩几个有效码点、精度崩塌；理想解法是激活 per-channel，可惜激活的 channel 维恰是 GEMM 内侧归约维、硬件做不了——激活难量化的方向正好是硬件量化不了的方向。（对应正文第 2.1 节）

</details>

**5. 能写出 SmoothQuant 的恒等变换与迁移强度公式，并说出 $\alpha$ 的典型取值与离线折叠**

<details>
<summary>参考答案</summary>

$Y = XW = (X\,\mathrm{diag}(s)^{-1})(\mathrm{diag}(s)\,W)$：激活第 $j$ 通道除以 $s_j$ 压平 outlier，对应权重行乘 $s_j$，难度搬进原本好量化的权重。$s_j = \frac{\max(|X_j|)^{\alpha}}{\max(|W_j|)^{1-\alpha}}$，$\alpha=0.5$ 是论文默认甜点，outlier 更严重的模型（如 GLM-130B）用 $0.75$。$\mathrm{diag}(s)$ 可离线折叠——权重侧直接改权重文件、激活侧折进前一个 LayerNorm 的仿射参数，运行时零开销；变换后激活权重都能用硬件友好的粗粒度量化到 INT8（另见 Q2）。（对应正文第 2.2 节）

</details>

**6. 能说出 GPTQ 的优化目标、误差补偿思想与三个工程化改造**

<details>
<summary>参考答案</summary>

目标是逐层重构：$\min\|WX - \hat{W}X\|_F^2$（让本层输出变化最小而非权重变化最小），二阶信息是 Hessian $H = 2XX^\top$。继承 OBQ：量化掉某个权重后，对尚未量化的其余权重施加最优补偿，把这次 round 的输出扰动"补"回来。三个改造使其撑起百亿参数：固定列序（Hessian 逆全层共享一份）、Lazy Batch 以 128 列为块延迟批量更新、Cholesky 预分解保数值稳定。画像：极限比特（3-bit）上限高，但依赖校准集、有过拟合风险（另见 Q3）。（对应正文第 3.2 节）

</details>

**7. 能解释 AWQ 为什么"看激活挑权重"，以及等价缩放为什么是净赚的**

<details>
<summary>参考答案</summary>

约 0.1%~1% 的权重通道贡献绝大部分精度，判断显著性看的不是权重自身大小、而是对应激活的幅值（激活大的输入通道，权重误差被放大得最厉害）。为避免"1% FP16 + 99% INT4"搅乱 kernel，AWQ 对显著通道权重乘 $s>1$、激活除以 $s$：只放大 1% 通道时组内 $\max|w|$ 大概率不变、量化步长 $\Delta' \approx \Delta$，显著权重相对误差近似缩小为 $1/s$。$s$ 参数化为 $s_X^{\alpha}$ 网格搜索，不做重构、只统计激活幅值——校准依赖弱、泛化好、量化快。（对应正文第 3.3 节）

</details>

**8. 能说出 KIVI 对 Key/Value 采用不同量化维度的分布依据，以及 residual 窗口存在的原因**

<details>
<summary>参考答案</summary>

Key cache 存在少数固定 outlier 通道（K 本就是激活 $XW_K$ 的产物），只有 per-channel（跨 token 共享 scale）才能把 outlier 隔离在自己的通道组里；Value cache 无明显 outlier，且被注意力权重逐 token 加权求和，per-token 量化让每个 token 的误差独立有界、不跨 token 串扰。配置为 2-bit、组大小 32。per-channel 需沿 token 维攒满一组才能算 scale，而自回归一次只来一个 token，故维护 FP16 residual 滑动窗口（默认 128 token）攒组批量落盘，同时保住"最近 token 注意力权重通常最高"的关键信息。（对应正文第 4.3 节）

</details>

**9. 能对比 MXFP4 与 NVFP4 的块大小和 scale 格式，并解释 NVFP4 的两处改进为何都冲着精度去**

<details>
<summary>参考答案</summary>

两者元素格式都是 FP4 E2M1（全部码点仅 $\pm\{0, 0.5, 1, 1.5, 2, 3, 4, 6\}$）。MXFP4（OCP 标准）：块 32、块 scale 用 E8M0（8-bit 纯指数、只能取 2 的幂），平均 4.25 bit；NVFP4：块缩到 16——组内动态范围更窄，块 scale 换成带尾数的 FP8 E4M3——能精确对准块内最大值而不是向上取整到最近的 2 的幂，再加每张量一级 FP32 scale 兜住 E4M3 自身的表示范围，约 4.5 bit。两者均为 Blackwell Tensor Core 原生支持。（对应正文第 5.2 节）

</details>

**10. 能复述量化选型决策树的主干，并说出上线前两类精度验收的互补性**

<details>
<summary>参考答案</summary>

主干按卡问起：Blackwell → NVFP4；Hopper/Ada → FP8 W8A8；Ampere 及更早按主要矛盾分——显存装不下 / 小 batch 低延迟 → W4A16（AWQ/GPTQ + Marlin），高并发吞吐 → INT8 W8A8（SmoothQuant 平滑激活）；长上下文再叠加 KV 量化（首选 FP8 KV，极限压缩看 KIVI）。验收先 PPL 后下游任务：PPL 便宜敏感，但"PPL 几乎不变 ≠ 下游能力无损"；用 lm-eval（vLLM 后端）跑业务任务集，GSM8K 这类敏感任务当金丝雀，FP16 基线同框架同配置对比，不达标按"排查误差层 → 排除敏感层 → 换校准集 → 细化粒度 → 退位宽"处置。（对应正文第 6~7 节）
