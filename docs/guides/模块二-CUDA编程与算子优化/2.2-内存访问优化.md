...
<img src="/AIInfraGuide/images/合并访存与非合并访存对比.png" alt="合并访存与非合并访存对比" style="max-width: 70%; display: block; margin: 0 auto;" />
...
**1. 能判断给定的内存访问模式是否满足合并访问条件**

<details>
<summary>参考答案</summary>

- 核心原则：让 Warp 内的**连续线程访问连续的内存地址**。全局内存访问以 Warp 为单位合并成事务，32 个线程连续访问 32 个 float（共 128B）且对齐时最理想，恰好落在 1 条 cache line 内。
- 跳跃访问不合并：stride=2 时带宽利用率约 50%，stride=32（每线程地址相差 128B）或完全随机时需 32 个事务、利用率仅约 3%。
- 还需检查对齐：`cudaMalloc` 保证 256B 对齐，但手动指针偏移（如 `data[tid + 1]` 偏移 4B）会破坏 128B 对齐，可能多出一个事务。
- 二维数组（行主序）按行遍历（连续线程访问同一行的连续列）是合并的；按列遍历 stride=cols，极差。

（对应正文第 2.1~2.3、2.5 节）

</details>

**2. 能计算一个 Warp 的全局内存访问需要多少个 32B 事务**

<details>
<summary>参考答案</summary>

- 每个内存事务的粒度为 32 Bytes（一个 cache line sector）；L1 cache line 为 128B，由 4 个 32B sector 组成。
- 计算方法：把 Warp 内 32 个线程实际触及的地址映射到 32B sector 上，覆盖了多少个不同的 sector 就需要多少个事务。
- 例：32 线程连续对齐访问 32 个 float（128B）→ 覆盖 4 个 32B sector，即最少事务数；stride=32 的 float 访问，每个线程地址相差 128B，各落在不同 sector → 需 32 个事务，是连续访问的 32 倍。

（对应正文第 2.1、2.2 节）

</details>

**3. 能将 AoS 数据结构改写为 SoA 以实现合并访问**

<details>
<summary>参考答案</summary>

- AoS（Array of Structures，如 `struct Particle { float x, y, z, w; }` 的数组）：连续线程访问所有 `x` 字段时地址间隔 16B（stride=4），不利于合并。
- SoA（Structure of Arrays）：把每个字段放进独立数组（`float x[N], y[N], z[N], w[N]`），访问所有 `x` 时地址完全连续（stride=1），可以合并。
- 在 GPU 编程中 **SoA 几乎总是优于 AoS**，因为 Warp 并行模式天然适合连续线程访问连续字段。

（对应正文第 2.4 节）

</details>

**4. 能解释共享内存 Bank 的编址方式及 Bank Conflict 的产生原因**

<details>
<summary>参考答案</summary>

- 共享内存划分为 32 个 Bank（与 Warp 大小一致），每个 Bank 宽 4 Bytes；连续的 4B 字轮流分配到连续 Bank：地址 0~3 → Bank 0，4~7 → Bank 1，……124~127 → Bank 31，从 128 起循环回 Bank 0。
- 每个时钟周期每个 Bank 只能服务一次读或写请求；同一 Warp 内多个线程访问**同一 Bank 的不同地址**时必须串行化，N-way Conflict 需要 N 个周期。
- 例外是广播：多个线程读同一 Bank 的**同一地址**时硬件将该值广播给所有请求线程，1 个周期完成，不算冲突。

（对应正文第 3.1、3.2 节）

</details>

**5. 能对存在 32-way Bank Conflict 的代码添加 Padding 解决冲突**

<details>
<summary>参考答案</summary>

- 典型场景：`__shared__ float tile[32][32]` 按列读取（`tile[threadIdx.x][ty]`）时，每个线程地址相差 32×4B，Bank 全部等于 `ty % 32` → 32-way Conflict。
- 解决：声明改为 `tile[32][32 + 1]`（33 列），地址变为 `(row * 33 + col) * 4`，Bank = `(row * 33 + col) % 32`，各线程的 Bank 依次错开一位 → 无冲突。
- 代价是每行多浪费 4 Bytes 共享内存，性能收益通常远超这点开销；不想浪费空间可用 Swizzle（如 `col ^ row` 的 XOR 变换），但索引计算更复杂，CUTLASS 等高性能 GEMM 中广泛使用。
- 验证：用 Nsight Compute 的 `l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_ld/st` 计数器确认归零。

（对应正文第 3.3~3.6 节）

</details>

**6. 能正确使用 `float4` 向量类型实现向量化加载和存储**

<details>
<summary>参考答案</summary>

- 用 `reinterpret_cast<float4*>` 把 `float*` 转换为 `float4*`，一条指令读写 16B（4 个 float），即 4 次 `LDG.32` 变为 1 次 `LDG.128`。
- 索引变化：每个线程负责 4 个元素（`idx = tid * 4`），以 `float4` 为单位取第 `tid` 个，并做好边界判断（`idx + 3 < N`）。
- 收益：总加载指令数减少 75%、单次事务传输更多有效数据、减少调度开销；同类类型还有 `float2`（8B）、`int4`、`double2`（均 16B）。
- 适用场景：Elementwise、大规模拷贝、带宽受限且地址可保证对齐的 Kernel；复杂 gather/scatter 索引则不适合。

（对应正文第 4.1~4.3、4.6 节）

</details>

**7. 能处理向量化加载中的对齐问题和尾部元素**

<details>
<summary>参考答案</summary>

- 对齐要求：`float4` 必须 16B 对齐。`cudaMalloc` 的 256B 对齐天然满足，但指针偏移（如 `base + 3`，偏移 12B）会破坏对齐，直接向量化访问是不安全的。
- 头部处理：先用标量处理前 `(4 - offset % 4) % 4` 个未对齐元素，再从对齐位置开始向量化处理主体。
- 尾部处理：N 不是 4 的倍数时，主体以 `float4` 处理 `N / 4` 组，剩余元素从 `vec_N * 4` 开始用标量循环补齐。

（对应正文第 4.4、4.5 节）

</details>

**8. 能使用共享内存将非合并的全局内存写入转化为合并写入**

<details>
<summary>参考答案</summary>

- 以矩阵转置为例：朴素版本读取是合并的，但写入 `out[col * height + row]` 的 stride = height，完全不合并。
- 优化思路：以 32×32 的 tile 为单位，先按行**合并读取**全局内存写入共享内存；`__syncthreads()` 后交换 block 的 x/y 索引，从共享内存按转置方向读出，再按行**合并写入**输出。
- 共享内存按列读取会引入 Bank Conflict，配合 `[TILE_DIM][TILE_DIM + 1]` 的 Padding 消除。
- 本质：共享内存充当"中转站"，把全局内存层面的非合并访问转化为共享内存内部的行列交换。

（对应正文第 5.1 节）

</details>

**9. 能用 Nsight Compute 识别内存效率相关的性能指标**

<details>
<summary>参考答案</summary>

- 全局内存效率：`sm__sass_average_data_bytes_per_sector_mem_global_op_ld`（正文标注理想值 128B）与 `l1tex__t_sectors_pipe_lsu_mem_global_op_ld`（实际事务数），对比判断是否存在非合并访问。
- 带宽利用：`gpu__compute_memory_throughput`（实际吞吐）、`dram__bytes_read + dram__bytes_write`（实际传输量）。
- Bank Conflict：`l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_ld / _st`，两个计数器不为零即说明存在共享内存冲突。

（对应正文第 3.6、6.2 节）

</details>

**10. 能对矩阵转置 Kernel 应用完整的内存访问优化链**

<details>
<summary>参考答案</summary>

- ① 定性：转置不含计算，是纯 Memory-Bound，性能完全取决于内存访问效率；朴素版本的瓶颈是写入 stride = height 非合并。
- ② 引入共享内存 tile（TILE_DIM=32），使全局内存的读和写都变成合并访问。
- ③ tile 声明加 `+1` Padding（`[32][32 + 1]`）消除按列读 tile 的 32-way Bank Conflict。
- ④ 用 BLOCK_ROWS=8 让每线程处理 TILE_DIM / BLOCK_ROWS = 4 个元素，减少 Block 数与调度开销。
- ⑤ 按优化决策流程用 Nsight Compute 验证：合并访问 → Bank Conflict → 向量化 → 是否达到带宽上限。
