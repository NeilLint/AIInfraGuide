...
**1. 能说清 Prefill 和 Decode 两阶段在输入规模、并行度、计算瓶颈上的差异**

<details>
<summary>参考答案</summary>

- **输入规模**：Prefill 一次性处理整段 Prompt（几十到几千 Token）；Decode 每步只输入上一步刚生成的 1 个 Token。
- **并行度**：Prefill 的 Token 间可并行，矩阵乘又大又密集；Decode 步与步之间串行（第 $t+1$ 个 Token 必须等第 $t$ 个算完）。
- **计算瓶颈**：Prefill 是 Compute Bound（算力受限），Decode 是 Memory Bound（访存受限）。
- **决定的指标与执行次数**：Prefill 决定 TTFT、只执行 1 次；Decode 决定 TPOT、执行"输出长度 - 1"次。
- 类比：Prefill 像一口气读题，Decode 像一笔一画答题。

（对应正文 1.2、1.3 节）

</details>

**2. 能解释为什么需要 KV Cache，以及为什么只缓存 K、V 而不缓存 Q**

<details>
<summary>参考答案</summary>

- 若什么都不缓存，每生成一个新 Token 都要把前面所有 Token 重新过一遍网络算出 K、V（生成第 100 个 Token 时要重算 99 个），计算量随序列长度平方级膨胀。
- KV Cache 把已算过的 Key、Value 存起来，下一步直接复用、绝不重算，是典型的"用空间换时间"：牺牲显存，换掉大量重复计算。
- 只缓存 K、V 不缓存 Q 的原因是不对称性：每一步 Attention 只用**当前 Token 的 Query**，而历史 Token 的 **Key、Value 每步都要全量用到**，所以只有 K、V 值得缓存。

（对应正文 2.1 节）

</details>

**3. 能手算一个给定配置模型每 Token 的 KV Cache 显存开销**

<details>
<summary>参考答案</summary>

- 公式：$\text{per\_token\_bytes} = 2 \times L \times H_{kv} \times d_{head} \times \text{bytes}$，其中 2 是 K 和 V 两份、$L$ 是层数、$H_{kv}$ 是 KV 头数（不是 Query 头数）、$d_{head}$ 是每头维度、bytes 是精度字节数（FP16/BF16 为 2）。
- 总占用再乘序列长度和并发数：$\text{total\_bytes} = \text{per\_token\_bytes} \times \text{seq\_len} \times \text{batch\_size}$。
- 例：32 层、32 个 KV 头、$d_{head}=128$、FP16 → $2 \times 32 \times 32 \times 128 \times 2 = 524288$ 字节 ≈ 0.5 MB/Token；上下文 4096 Token 的一个请求约 2 GB，50 个并发仅 KV Cache 就要 100 GB，单张 80GB GPU 直接爆掉。

（对应正文 2.3 节）

</details>

**4. 能说明 GQA/MQA 为什么能省 KV Cache，以及大致能省多少**

<details>
<summary>参考答案</summary>

- $H_{kv}$ 在 KV Cache 公式中是线性因子，GQA/MQA 让多个 Query 头共享一组 KV 头，直接线性减小 $H_{kv}$，KV Cache 随之等比例缩小。
- 例：32 个 Query 头共享 8 个 KV 头（GQA），KV Cache 直接砍到原来的 $1/4$，而模型表达能力几乎不受影响。
- 正因为这笔账，现代大模型几乎清一色采用 GQA。

（对应正文 2.3 节）

</details>

**5. 能准确区分 TTFT、TPOT/ITL、Throughput、尾延迟和 Goodput 的含义与适用场景**

<details>
<summary>参考答案</summary>

- **TTFT**：提交请求到收到首 Token 的时间，由排队 + Prefill + 网络延迟构成，决定聊天/流式场景"响应快不快"的第一印象。
- **TPOT/ITL**：Decode 阶段平均每个输出 Token 的时间，决定吐字速度（TPOT 50ms ≈ 每秒 20 字）；两者多数语境同义，细微差别在是否把首 Token 算进去（GenAI-Perf 的 ITL 排除首 Token）。
- **Throughput**：系统 Token/s 衡量集群产能与成本，单用户 Token/s ≈ $1/\text{TPOT}$，RPS 衡量并发承载能力；吞吐与延迟通常是一对矛盾。
- **尾延迟**：P50 是中位数，P95/P99 才是用户投诉的来源——平均值会把少数慢得离谱的请求藏起来。
- **Goodput**：只统计满足 SLO（如 TTFT < 1s 且 TPOT < 50ms）的请求的吞吐，比 Raw QPS 更贴近真实价值。

（对应正文 3.1~3.3 节）

</details>

**6. 能用 E2E 延迟公式判断某业务场景是 TTFT 主导还是 TPOT 主导**

<details>
<summary>参考答案</summary>

- 公式：$\text{E2E} = \text{TTFT} + \text{TPOT} \times (\text{输出 Token 数} - 1)$，短输出场景 TTFT 主导，长输出场景 TPOT 主导。
- 代入验证（TTFT = 500 ms、TPOT = 40 ms）：生成 20 Token 时 E2E = 1260 ms，TTFT 占约 40\%——首字慢用户就先炸了；生成 800 Token 时 E2E ≈ 32460 ms，TTFT 只占约 1.5\%——此时优化 TPOT 才是正事。
- 结论：先测清楚自己业务的输入输出长度分布，再决定优化重心，这是铁律。

（对应正文 3.1 节）

</details>

**7. 能用算术强度和 Roofline 解释为什么 Decode 是 Memory Bound**

<details>
<summary>参考答案</summary>

- 算术强度 = FLOPs / 访存字节数，即每搬 1 字节数据能顺带做多少次计算；Roofline 上低于"平衡点"（峰值算力 ÷ 显存带宽）就落在被带宽压住的斜坡区（Memory Bound）。
- 平衡点举例：H100 SXM 的 BF16 稠密算力约 990 TFLOPS、HBM3 带宽 3.35 TB/s，平衡点 ≈ 295 FLOP/Byte。
- Decode 每步只算 1 个 Token，却要把整个模型权重从显存完整搬一遍：FP16 下搬约 $W$ 字节权重只做约 $W$ FLOP，算术强度 ≈ 1 FLOP/Byte，比平衡点低两个数量级，算力利用率理论上不到 1\%——死死卡在带宽墙上。
- 对比 Prefill：一次并行处理成百上千 Token，权重搬一次被所有 Token 共享，算术强度高，落在 Compute Bound 区。

（对应正文 4.1~4.3 节）

</details>

**8. 能从"Decode 卡在搬权重"推导出 Batching 提吞吐、Weight-only 量化加速 Decode 的原因**

<details>
<summary>参考答案</summary>

- 前提：Decode 慢不是算不过来，而是"搬不过来"——GPU 计算单元大部分时间在空等权重从显存加载，闲置算力充足。
- **Batching**：把 $B$ 个请求的 Decode 拼成一个 Batch，权重仍只搬一次却服务 $B$ 个 Token，算术强度放大 $B$ 倍、逼近平衡点，把闲置算力利用起来——所以能大幅提升吞吐却几乎不增加延迟，这正是 Continuous Batching 的理论依据。
- **Weight-only 量化**：瓶颈在搬权重，把权重从 FP16 压到 INT4，要搬的字节数直接减少到 $1/4$，Decode 延迟随之大幅下降。

（对应正文 4.3 节）

</details>

**9. 能画出推理全链路并指出每个环节的计算特性与潜在瓶颈**

<details>
<summary>参考答案</summary>

链路：Tokenize → Prefill → Decode Loop（循环内含 Sampling）→ Detokenize。

- **Tokenize**（文本 → Token ID）：CPU 任务，长文本或高并发时 CPU 成瓶颈。
- **Prefill**：GPU、Compute Bound，Prompt 很长时拉高 TTFT。
- **Decode Loop**（每步读全量 KV、追加新 KV）：GPU、Memory Bound，显存带宽决定 TPOT。
- **Sampling**（按 Temperature/Top-p 采样）：GPU、轻量，复杂采样或约束解码略增开销。
- **Detokenize**（Token ID → 文本，流式返回）：CPU 任务，与主循环争抢 CPU。
- 隐藏坑：GPU 优化到位后，Tokenize/Detokenize 这些 CPU 环节可能成为新瓶颈——vLLM V1 引擎用多进程把它们和 GPU 核心循环重叠正是为此。
