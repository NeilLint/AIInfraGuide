...
**1. 能解释 Warp 的大小为什么是 32 以及 Warp 的形成规则**

<details>
<summary>参考答案</summary>

- Warp 是 GPU 硬件调度和执行的最小单位，固定包含 **32 个连续线程**——同一 Warp 的线程在同一时钟周期执行相同的指令（SIMT），32 是硬件设计定死的粒度。
- 形成规则：Block 被分配到 SM 后，硬件按 `threadIdx` 顺序自动分组——线程 0~31 为 Warp 0，32~63 为 Warp 1，以此类推。划分是硬件行为，程序员无法干预。
- 一个 256 线程的 Block 会被拆成 8 个 Warp；若 Block 大小不是 32 的整数倍（如 48 线程），最后一个 Warp 的多余位置会被"填充"但不执行有效工作，浪费硬件资源。
- 每个 SM 配备若干 Warp Scheduler（现代架构通常 4 个），Warp 间切换零开销（寄存器状态常驻 SM，无需保存/恢复），借此在等待内存（约 400 cycles）时切换到其他就绪 Warp，实现延迟隐藏。

（对应正文第 1.1、1.2 节）

</details>

**2. 能说明 SIMT 与 SIMD 的核心区别**

<details>
<summary>参考答案</summary>

- 编程视角：SIMD（如 AVX-512）要求程序员显式操作向量寄存器、手动向量化；SIMT 让程序员写单线程的标量代码，硬件把相同指令广播到 32 个线程上并行执行。
- 线程状态：SIMT 中每个线程（逻辑上）拥有独立的程序计数器和栈，可以独立寻址；SIMD 的所有 lane 通常访问连续数据。
- 分支处理：SIMD 用掩码（mask）跳过特定 lane；SIMT 由硬件自动序列化不同分支路径。
- 代价：SIMT 大幅降低了并行编程门槛，但引入了 Warp Divergence 这一性能陷阱。

（对应正文第 2.1 节）

</details>

**3. 能识别代码中的 Warp Divergence 并估算其性能影响**

<details>
<summary>参考答案</summary>

- 成因：同一 Warp 内的线程遇到条件分支（`if/else`、`switch`、循环次数不同）走不同路径时，硬件同一时刻只能发射一条指令，必须串行执行所有分支路径，不走该路径的线程被暂时禁用（masked off）。
- 估算：总耗时约等于各条路径耗时之和。典型例子 `if (tid % 2 == 0)` 使 Warp 内一半线程走 `if`、一半走 `else`，耗时约为无分支情况的 2 倍；最坏情况 32 个线程走 32 条不同路径，性能退化可达 32 倍。
- 量化指标：Branch Efficiency = 非分歧分支数 / 总分支数 × 100%；用 Nsight Compute 可观察每条分支指令的 Warp 活跃线程占比，理想为 100%（32/32）。

（对应正文第 3.1~3.3 节）

</details>

**4. 能将含有 `if/else` 的 Kernel 改写为无分支或 Warp 对齐的版本**

<details>
<summary>参考答案</summary>

- 策略一（Warp 对齐分支）：让分支条件在 Warp 粒度上一致，如把 `if (threadIdx.x % 2 == 0)` 改为按 `warpId = threadIdx.x / 32` 分支，使同一 Warp 内所有线程走相同路径。
- 策略二（算术替代分支）：如 ReLU 写成 `y = x * (x > 0)`（利用 bool→int 隐式转换）或 `y = fmaxf(x, 0.0f)`。
- 策略三（数据重排）：预处理时把走同一路径的数据聚集到连续位置（如正数、负数分开存放），保证处理它们的 Warp 无分歧。
- 策略四（统一循环）：把"各线程循环次数不同"改为统一循环到块内最大次数，循环体内用 `if (i < array_length[tid])` 做 predication。

（对应正文第 3.4 节）

</details>

**5. 能正确使用四种 Warp Shuffle 指令（`shfl`/`shfl_up`/`shfl_down`/`shfl_xor`）**

<details>
<summary>参考答案</summary>

- `__shfl_sync(mask, var, srcLane)`：从指定 lane 读取，常用于 Warp 内广播；`__shfl_up_sync`：从 `laneId - delta` 的低编号 lane 读取；`__shfl_down_sync`：从 `laneId + delta` 的高编号 lane 读取；`__shfl_xor_sync`：与 `laneId ^ laneMask` 配对的 lane 交换（蝶形模式）。
- `mask` 是参与线程掩码，通常用 `0xFFFFFFFF` 表示全部 32 个线程参与；`width` 参数（可取 2、4、8、16、32）把 Warp 逻辑划分成子 Warp。
- 本质是让同一 Warp 内的线程直接读取彼此的寄存器值，不消耗共享内存，也无需显式同步屏障。

（对应正文第 4.1、4.2 节）

</details>

**6. 能用 Warp Shuffle 实现 Warp 级归约（sum/max/min）**

<details>
<summary>参考答案</summary>

- 蝶形归约：`offset` 从 16 开始每步减半（16→8→4→2→1），每步执行 `val += __shfl_down_sync(0xFFFFFFFF, val, offset)`，$\log_2(32) = 5$ 步完成。
- 求 max/min 时把加法换成 `fmaxf`/`fminf` 即可，归约结构不变。
- 结束时只有 lane 0 持有正确的归约结果，需要时可再用 `__shfl_sync` 广播给全 Warp。

（对应正文第 4.5、4.4 节）

</details>

**7. 能解释 `width` 参数的作用并实现子 Warp 归约**

<details>
<summary>参考答案</summary>

- `width` 把一个 32 线程的 Warp 逻辑划分为多个独立子 Warp（宽度可为 2、4、8、16、32），Shuffle 的源 lane 计算被限制在各子组内部。
- 例如 `width=8` 时 Warp 被视为 4 个独立的 8 线程子组：归约循环从 `offset=4` 开始减半，`val += __shfl_down_sync(0xFFFFFFFF, val, offset, 8)`。
- 结束后 Lane 0、8、16、24 分别持有各自子组的累加和，适合"一个 Warp 同时处理多个小问题"的场景。

（对应正文第 5.4 节）

</details>

**8. 能结合 Shuffle 和共享内存完成 Block 级归约**

<details>
<summary>参考答案</summary>

- 三步结构：① 每个 Warp 先用 Shuffle 做 Warp 内归约；② 每个 Warp 的 lane 0 把部分和写入共享内存数组 `warp_sums[32]`（最多 32 个 Warp = 1024 线程 / 32），然后 `__syncthreads()`；③ 第一个 Warp 把这些部分和读进来再做一次 Warp 归约，thread 0 写出本 Block 结果。
- 相比纯共享内存逐步折半归约（1024B 共享内存、8 次 `__syncthreads()`、约 64 条指令，256 线程 Block），Shuffle 版本只需 128B 共享内存（32 个 float）、1 次 `__syncthreads()`、约 13 条指令。
- 优势来源：寄存器到寄存器通信延迟极低，且大幅减少共享内存占用和同步屏障开销。

（对应正文第 6.1、6.2 节）

</details>

**9. 能使用 `__ballot_sync` 实现 Warp 级的条件筛选**

<details>
<summary>参考答案</summary>

- `__ballot_sync(mask, predicate)` 返回一个 32-bit 位图，每个 bit 对应一个 lane 的 predicate 值，让全 Warp 知道"哪些线程满足条件"，这是 Warp 级条件筛选的基础。
- 同族的 Vote 函数：`__all_sync`（全部为真才返回 true）、`__any_sync`（任一为真即返回 true），可做集体决策。
- 典型用法是提前退出优化：先 `__any_sync` 判断 Warp 内是否有线程命中目标，全 Warp 都未命中就整体跳过后续处理，只有命中时才进入具体分支处理。

（对应正文第 5.1 节）

</details>

**10. 能说明 Volta 独立线程调度对编程的影响**

<details>
<summary>参考答案</summary>

- Volta 起每个线程拥有真正独立的程序计数器和调用栈：分支分歧时硬件可以更灵活地交错执行不同路径，Warp 内线程可以位于不同代码位置。
- 编程影响一：Warp 不再天然 lockstep，Warp 内通信前需要显式 `__syncwarp()`（Volta 之前它通常是空操作），并可用 `__activemask()` 获取当前活跃线程掩码。
- 编程影响二：新增 Warp Match 函数（`__match_any_sync`/`__match_all_sync`），可按值自动分组线程。
- 关键提醒：独立线程调度**并不消除** Divergence 的性能代价，只是让调度更灵活，仍应尽量避免同一 Warp 内的分支分歧。
