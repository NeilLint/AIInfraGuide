...
- **端侧（Device）**强调最终设备（手机、汽车、摄像头、可穿戴、IoT 终端），资源约束通常最强；**边缘侧（Edge）**还可能包括基站、门店服务器和边缘网关；**云端（Cloud）**拥有更充足的算力、显存和散热条件。
- 上端的四个常见动机：低延迟（不能等网络往返）、离线可用、隐私（原始数据留在本地）、成本（高频轻量请求本地完成）。
- 代价是工程约束从"主要考虑吞吐和成本"扩展为"同时考虑延迟、内存、包体积、功耗、温度、精度和兼容性"。（对应第 1 节）

</details>

**2. 能区分模型格式、Runtime 与 Backend/Delegate**

<details>
<summary>参考答案</summary>

- **模型格式 / IR**：描述计算图和权重，如 ONNX、`.pte`、`.tflite`、`.mlpackage`。
- **Runtime**：负责加载模型、管理内存并发起执行，如 ExecuTorch、ONNX Runtime、ncnn。
- **Backend/Delegate**：负责真正执行一部分计算，如 XNNPACK、Core ML、Vulkan、Qualcomm QNN。
- Runtime 不一定亲自计算每个算子：图分区器把支持的子图委托给硬件后端，剩余算子留给默认 CPU Kernel（图分区 / 委托机制）。因此"启用了 NPU"不等于"整个模型都运行在 NPU 上"，端侧优化首先要确认实际分区结果。（对应第 3 节）

</details>

**3. 能说明 CPU、GPU、NPU、DSP 的主要特点**

<details>
<summary>参考答案</summary>

- **CPU**：算子覆盖和调试体验最好、兼容性最强的兜底路径；优化靠 NEON/SVE 等 SIMD、线程数与大小核调度、Cache 与内存布局、XNNPACK 等优化 Kernel。
- **GPU**：并行度高（Vulkan/Metal/OpenCL），但小算子摊不薄调度开销、CPU-GPU 数据转换与同步有成本，不保证比高度优化的 ARM CPU 快。
- **NPU**：针对矩阵、卷积和低精度推理，能效高；约束是算子覆盖、Tensor Layout、动态形状和量化规则，出现不支持的节点就会图切分、回退。
- **DSP**：算力未必最高，胜在低功耗下持续处理规则数据流，适合音频、传感器等常驻任务。
- 没有"永远最快"：先用 CPU 建立准确性基线，再分别实测 GPU/NPU/DSP 的覆盖率、端到端延迟和稳态功耗。（对应第 4 节）

</details>

**4. 能估算模型权重和 LLM KV Cache 的内存占用**

<details>
<summary>参考答案</summary>

- 权重下界：$M_{weights} \approx P \times \frac{b}{8}$（$P$ 参数量、$b$ 每参数位数）。3B 模型：FP16 约 6GB、INT8 约 3GB、INT4 约 1.5GB——这只是理论下界，还要加量化尺度、文件对齐、激活和工作区。
- KV Cache：$M_{KV} \approx 2 \times L \times B \times T \times H_{kv} \times D \times s$（2 代表 K 和 V；$L$ 层数、$B$ batch、$T$ 缓存 Token 数、$H_{kv}$ KV Head 数、$D$ 每 Head 维度、$s$ 每元素字节数）。
- 两个推论：权重能加载不代表能稳定支持长上下文（KV 随 $T$ 线性增长）；操作系统还限制单 App 可用内存，物理内存 8GB 不代表模型能用满 8GB。（对应第 2.1 节）
...
- 量化把权重或激活从 FP32/FP16 降到 INT8/INT4，但加速的前提是硬件有对应的低精度 Kernel；如果没有命中，或计算图中频繁执行量化与反量化，延迟可能不降反升。
- 还要分清几组概念：PTQ（成本低、快速部署）与 QAT（训练时模拟量化误差、精度更好）；Weight-only（LLM 常用）与权重+激活量化（压缩更充分但更依赖硬件支持）；Per-tensor 与 Per-channel。
- 因此评估量化必须同时报告模型大小、延迟、内存和精度变化四项，不能只看体积。（对应第 6.2 节）

</details>

**6. 能识别异构执行中的后端回退和数据搬运问题**

<details>
<summary>参考答案</summary>

- 委托机制下，不支持的算子会回退 CPU，图被切碎后后端间频繁切换、数据搬运成本上升——这是"号称使用 NPU、实际大量回退 CPU"误判的来源，Benchmark 记录里要保留 delegate_coverage（委托子图比例 / 未支持算子）。
- "NPU 比 CPU 还慢"的排查顺序：NPU 实际覆盖了多少算子；是否出现 CPU 与 NPU 之间的频繁数据转换；输入规模是否太小、摊不薄编译与调度开销；Benchmark 是否把模型加载、首次编译或预热算进了推理时间；量化类型是否真的命中了 NPU Kernel。
- 图优化的重点也在于此：不是节点越少越好，而是减少内存访问、调度开销和后端切换。（对应第 3、6.3、9.3 节）

</details>

**7. 能设计包含冷启动、尾延迟、内存、功耗和精度的 Benchmark**

<details>
<summary>参考答案</summary>

- **环境记录**：设备/SoC/系统版本、Runtime 与模型版本（含文件哈希）、精度与量化方法、实际执行后端与图分区结果、线程数与输入尺寸。
- **指标**：冷启动与模型加载时间；预热后的 P50/P90/P99 延迟；吞吐；峰值与稳定运行内存；包体积；功耗、温度与持续运行后的性能变化；任务精度与误差。LLM 另加 TTFT、TPOT、Prefill/Decode 各自 tokens/s、最大稳定上下文长度。
- **协议要点**：固定输入样本与随机种子；先保存原始框架精度基线；加载与推理分开计时；预热后采集至少数十次；同时报中位数与尾延迟；连续运行观察温度和降频；检查每个子图实际后端；保存脚本、配置与原始结果。
- 比较必须控制变量：不同输入尺寸、线程数、量化精度或后端覆盖率的数字不能直接横向比。（对应第 7 节）

</details>

**8. 能描述 PyTorch 模型导出为 ExecuTorch `.pte` 的基本流程**

<details>
<summary>参考答案</summary>

- 三步：`torch.export.export` 捕获计算图（模型先 `.eval()` 避免 Dropout/BatchNorm 处于训练行为，示例输入定义输入形状与类型）→ `to_edge_transform_and_lower` 配 `XnnpackPartitioner` 标记并降低 XNNPACK 支持的子图 → `to_executorch()` 生成 `.pte` 写入文件。
- 换目标后端（Qualcomm NPU、Apple Neural Engine、Vulkan GPU）整体流程不变，只需替换 Partitioner 并按后端文档准备 SDK 与设备环境；第一次练习建议先用 XNNPACK 建 CPU 基线，把"导出问题"和"NPU 工具链问题"分开排查。
- 生成文件不等于部署完成，还要验证：导出前后输出误差、哪些算子被委托、`.pte` 与 Runtime 包体积、真机上的加载时间/延迟/内存、持续运行稳定性。（对应第 8 节）
