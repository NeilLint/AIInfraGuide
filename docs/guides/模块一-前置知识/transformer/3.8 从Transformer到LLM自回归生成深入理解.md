...
**1. 能解释自回归生成的数学本质，写出联合概率的链式分解公式**

<details>
<summary>参考答案</summary>

- 语言模型本质是建模条件概率分布 $P(x_t \mid x_1, \ldots, x_{t-1})$：给定前面所有 token，输出下一个 token 在整个词表上的概率分布。
- 整句话的联合概率是各位置条件概率的连乘（链式分解）：$P(x_1, \ldots, x_T) = \prod_{t=1}^{T} P(x_t \mid x_1, \ldots, x_{t-1})$。
- 工程上：序列过完所有 Decoder Block 后，取最后一个位置的隐藏状态，经 LM Head 线性层映射为 $vocab\_size$ 维 logits，再 Softmax 归一化成概率分布。
- 自回归生成就是一个循环：每步基于已有序列预测下一个 token，采样后追加到序列末尾，直到遇到 EOS token、达到 `max_new_tokens` 上限或命中停止字符串。

（对应正文第 1、2 节）

</details>

**2. 能说清 Prefill 和 Decode 两阶段的计算特性差异，以及为什么一个是 Compute Bound 另一个是 Memory Bound**

<details>
<summary>参考答案</summary>

- **Prefill**：一次性并行处理整个 prompt，计算并缓存所有 prompt token 的 K/V，输出第一个生成 token。核心操作是大矩阵乘法（如 $Q(1000, 128) \times K^T(128, 1000)$），算术强度高，Tensor Core 可以满载，瓶颈在算力——**Compute Bound**，其耗时决定 TTFT。
- **Decode**：每步只处理 1 个新 token，矩阵乘法退化为矩阵-向量乘法（$Q(1, 128) \times K^T(128, 1001)$）。计算量很小，但仍要把整个 K 缓存和权重矩阵从 HBM 搬到计算核心，数据搬运量没有按比例减少，GPU 大部分时间在等数据，瓶颈在显存带宽——**Memory Bound**，其每步耗时决定 TPOT。
- 两者对硬件需求截然不同，这也是 Prefill/Decode 解耦（DistServe、Splitwise）的出发点。

（对应正文第 4、5 节）

</details>

**3. 能用通用公式计算给定模型配置下的 KV Cache 显存占用**

<details>
<summary>参考答案</summary>

- 通用公式：$\text{KV Cache 显存} = 2 \times L \times n_{kv} \times d_h \times N \times B \times b_e$。
- 各项含义：$2$ 为 K 和 V 各一份；$L$ 为层数；$n_{kv}$ 为 KV 头数（MHA 等于 num_heads，GQA 等于 num_kv_groups）；$d_h$ 为每头维度；$N$ 为序列长度；$B$ 为批大小；$b_e$ 为每元素字节数（FP16 为 2 字节，FP8 为 1 字节）。
- 由公式可直接看出优化抓手：GQA 减少 $n_{kv}$（32 头减到 8 头即缩小 4 倍）、量化减少 $b_e$、滑动窗口限制 $N$。
- 注意 Batch 放大效应：同时服务 16 个请求，单请求数字要乘 16。

（对应正文 6.4 节）

</details>

**4. 能估算 LLaMA-2-7B 在 4096 序列长度、batch_size=16 下的 KV Cache 显存（约 32 GB）**

<details>
<summary>参考答案</summary>

- LLaMA-2-7B（MHA）配置：$L=32$ 层、$n_{kv}=32$ 头、$d_h=128$、FP16 每元素 2 字节。
- 每 token KV Cache $= 2 \times 32 \times 32 \times 128 \times 2 = 512$ KB。
- 4096 序列长度：$512 \text{ KB} \times 4096 = 2$ GB / 请求。
- batch_size=16：$2 \text{ GB} \times 16 = 32$ GB——KV Cache 就能吃掉大半张 GPU，这解释了为什么长上下文和大 batch 是"显存杀手"。

（对应正文 6.4 节）

</details>

**5. 能解释 PagedAttention 的核心思想，以及它如何解决显存碎片化问题**

<details>
<summary>参考答案</summary>

- 碎片化的根源：不同请求 KV Cache 大小不一且动态增长，传统做法按最大序列长度预分配连续显存，大量浪费；请求来去之后显存出现"空洞"——总空闲够但没有连续大块。
- PagedAttention（vLLM 核心创新）借鉴操作系统虚拟内存分页：把显存划分为固定大小的 Block（如每 Block 存 16 个 token 的 KV），按需逐步分配。
- 同一请求的 KV 不要求连续，可分散在不连续的 Block 中，用 Block Table（类似页表）记录映射；请求完成后立即释放 Block 供他人复用。
- 效果：KV Cache 显存利用率从约 20-40% 提升到接近 100%，相同显存下支持 2-4 倍并发请求。

（对应正文 6.5、7.1 节）

</details>

**6. 能区分 Temperature、Top-K、Top-P 三种采样策略的作用和适用场景**

<details>
<summary>参考答案</summary>

- **Temperature**：softmax 前将 logits 除以 $T$，控制分布"尖锐程度"——$T<1$ 分布变尖（保守），$T>1$ 分布变平（更冒险），$T \to 0$ 退化为 greedy，$T \to \infty$ 退化为均匀分布。
- **Top-K**：只保留概率最高的 $K$ 个 token 重新归一化后采样。问题是 $K$ 固定：分布集中时 $K=50$ 引入太多噪声，分布分散时又太限制。
- **Top-P（Nucleus）**：按概率降序累加，直到超过阈值 $P$（如 0.9），只在这个"核"内采样。优势是自适应——分布集中时候选集自动缩小，分散时自动扩大。
- 适用场景：greedy/低温适合代码补全等确定性输出；实际最常用 Temperature + Top-P 组合，如 `temperature=0.7, top_p=0.9`。

（对应正文第 3 节）

</details>

**7. 能解释 Speculative Decoding 如何在不牺牲质量的前提下加速生成**

<details>
<summary>参考答案</summary>

- 动机：自回归 Decode 的根本瓶颈是串行，每步必须等上一步完成。
- 流程：小模型（Draft Model，如 7B 对应的 1B 蒸馏版）快速生成 $K$ 个候选 token（如 $K=5$）→ 大模型（Target Model）把这 $K$ 个候选一次性并行验证 → 一致的 token 直接接受，第一个不一致处由大模型重新采样、后续候选全部丢弃。
- 加速原理：若小模型猜测准确率较高（如 70-80% 的 token 被接受），一次验证确认多个 token，等效一步生成多步，Decode 吞吐提升数倍；本质是把 Memory Bound 的串行 Decode 换成一次并行验证。
- 不牺牲质量的关键约束：从概率分布意义上，生成结果与大模型独立生成完全一致。

（对应正文 8.2 节）

</details>

**8. 能说出 TTFT、TPOT、Throughput 三个指标的含义和它们之间的关系**

<details>
<summary>参考答案</summary>

- **TTFT**（Time To First Token）：从请求发出到收到第一个输出 token 的时间，由 Prefill 阶段耗时决定，典型目标 < 500ms。
- **TPOT**（Time Per Output Token）：每个输出 token 的生成间隔，由 Decode 单步耗时决定，典型目标 < 100ms（50-100ms 以内即有"实时输出"的流畅感）。
- **Throughput**（吞吐）：系统视角指标，如 Tokens/s（每秒处理的总 token 数）和 Requests/s，靠增大 batch_size、Continuous Batching 等提升。
- 关系：`E2E Latency = TTFT + TPOT * (output_length - 1)`；延迟与吞吐存在权衡——增大 batch 提升吞吐但会拉高单请求 TPOT，推理调优的本质是在延迟 SLA 约束下最大化吞吐。

（对应正文第 4.3、5.3 节与第 9 节）
